...
| `Hive::PatrolFix` | `lib/hive/patrol_fix/` | Source-neutral workflow core for strict source snapshots, durable semantic admission, idempotent task materialization, task manifests, strict Inbox/Fix/Review reports, exact local worktree generations, bounded validation receipts, stable stage/route-transition journals, exactly-once coding successors, and common projection. Candidate-set and independent-review decisions are digest-bound and revalidated after unlocked provider calls. Materialization intent is durable before task capture; task/evidence binding precedes source acknowledgement; retry repairs interrupted creates, route actions, reciprocal successor links, and provenance updates. The core depends downward on `AgentGitGate` and lower-level `TaskCapture`, but has no dependency on either Patrol product, Daemon, UI, GitHub authentication, issue creation, or remote publication. |
