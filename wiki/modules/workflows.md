...
- agent/council stages may declare `agent`, `model`, and `effort`, which override project stage config. They may also declare `budget_usd` and `timeout_sec` resource defaults; project stage config takes precedence over those defaults. Budgets accept positive finite numbers, while timeouts require positive integers.
