...
| `queue`    | Read-only inspection of the dispatch-request queue the bot writes and the daemon consumes (AN-1/2/3). Runs in the CLI process (no daemon contact); reads the same `<state_home>/dispatch_requests/` directory. `list` (default) prints each pending request with `request_id  age  project/slug  verb` plus `[EXPIRED]` / `[NOT-ALLOWLISTED]` flags and any malformed files. `show <id>` dumps one request's full payload (errors with exit 1 if the id is unknown; missing id is a USAGE error). `prune` removes expired + malformed request files (the daemon also does this lazily on its own tick) and reports the count. With `--json`, emits a `hive-daemon-queue.v1` envelope (`action`, `requests[]`, `request`, `malformed[]`, `pruned_count`). Claimed in-flight requests (`*.json.claimed`) are intentionally not listed — they are daemon-managed; see [[modules/daemon]] §"At-most-once dispatch via atomic claim". |
...
| `child_timeout_sec` | 7200 | Per-child wall-clock cap (R-02). A child past its deadline gets SIGTERM then SIGKILL. `0` disables (children run unbounded — the historical behaviour). Default is generous so the 5-review autonomous loop (~90 min lock) is never killed mid-pass. Min 0. |
