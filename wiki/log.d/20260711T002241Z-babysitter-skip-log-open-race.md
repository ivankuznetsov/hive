...
**Result:** Existing logs now require the opened descriptor's device/inode to match the preflighted file. Missing logs use `File::CREAT | File::EXCL`; `EEXIST` restarts the full existing-file preflight. The existing no-follow, nonblocking, regular-file, owner, and link-count checks remain in place.
