...
| `modules/migration/{occurrence_journal,effect_delivery_collaborators}_test.rb`, `daemon/patrol_scheduler_test.rb`, `patrol/effect_gateway_test.rb`, `patrol/state_store_effect_intents_test.rb`, `patrol/pr_opener_test.rb`, `patrol/review_handoff_test.rb` | Ordinary effect recovery and publication — one canonical occurrence journal with no legacy StateStore effect maps or persisted sender liveness, process-local keyed mutex plus stable `0600` never-unlinked flock serialization (including a forked contender), uncertainty persisted before invocation, crash recovery without blind redispatch, gateway-owned local retry-safe absence, remote absence refusal, store-minted byte-stable terminal receipts, nonterminal-finalization and finalized-dispatch refusal, concurrency-safe durable schedule-attempt generations, reconcile-only adoption of an exact existing hosted PR, exact base/head and clean-worktree binding, verified remote publication, final remote base/head recheck before task publication, stale-base failed-handoff retry refusal, durable `reconciliation_pending` URL/patch receipts across lookup/auth failures, fail-closed exact-diff/title/body secret scanning, closed results, proof handoff, and exact-patch handoff retry. |
