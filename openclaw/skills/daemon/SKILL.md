...
Prefer `--json` for `status`, `stop`, `reload`, `install`, `enable`, and `disable` when you need structured output. Before `install --force`, `disable --all`, or `stop`, restate the effect and get explicit user confirmation.
