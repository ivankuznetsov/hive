...
The same `web` job installs `web/Gemfile` in frozen mode before any Rails
tests. Because `web/Gemfile` depends on the root gem as
`gem "hive-cli", path: ".."`, every root `hive.gemspec` runtime dependency
change must be reflected in `web/Gemfile.lock`; otherwise `ruby/setup-ruby`
fails during setup with Bundler's "gemspecs for path gems changed" error.
