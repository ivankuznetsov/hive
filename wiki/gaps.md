...
3. **Hive skills package publishing is still a companion-package follow-up.** Claude/Codex/Pi marketplace install commands are documented in `install.md` and [[operating]], but agents must not run those commands until `ivankuznetsov/hive-skills` is actually published. OpenClaw skill files now live in `openclaw/skills/` with an umbrella `/hive` skill plus shortcuts (`/plan`, `/work`, `/ce-review`, etc.); ClawHub publication and listing evidence remain external maintainer steps.
