...
| `Hive::Modules::Migration::PatrolEffectIndex` | `lib/hive/modules/migration/patrol_effect_index.rb` | Bounded immutable run-wide projection over verified effect receipts. Exact receipt replay is counted but idempotent; distinct committed/reconciled effects that collide by intent or idempotency key become qualification findings, and externally observable sinks also collide by semantic target/scope identity. Local `job`, `discovery`, and `action` transitions may legitimately repeat a target across fenced recovery generations, so their JobStore transition identities remain authoritative. `attempted` and `unknown` effects remain visible and block qualification, while denied, known-not-sent, and failed receipts remain non-effects. |
