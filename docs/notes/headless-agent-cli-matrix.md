...
| CE skill invocation syntax (prompt-side) | `/ce-code-review` (legacy `/compound-engineering:ce-code-review` config is normalized before rendering) | `$compound-engineering:ce-code-review` (Codex plugin skills are dollar-prefixed; Claude-style `/ce-code-review` is rejected) | `/skill:ce-code-review` after loading CE skills via `--skill <path>` (path can point at the Codex CE plugin's skills/ directory; pi has no native CE plugin marketplace install but accepts external skill dirs). **Skill invocation NOT verified by test-spawn** because pi is not logged in on this machine. | **No CE plugin available for opencode.** Hive must inline the CE skill prompt content into the message arg. Workaround: load the skill markdown from `~/.codex/plugins/cache/compound-engineering-plugin/compound-engineering/3.1.0/skills/ce-code-review/SKILL.md` and pass it as the prompt. Alternative: `opencode agent create` to register a custom agent that mirrors the CE skill — heavier setup, plugin-update drift. |
...
- Skill invocation in the prompt body: `/ce-code-review` for Hive reviewers. Older namespaced CE config values are normalized before rendering. The newer official `/code-review` command plugin posts PR comments directly and does not satisfy Hive's structured reviewer-file contract.
...
- **CE plugin v3.11.1 in codex** is exposed through dollar-prefixed prompt invocations such as `$compound-engineering:ce-code-review`. Claude slash commands like `/ce-code-review` are rejected by Codex even when the plugin is installed. Hive keeps accepting old config values but renders the Codex prompt with `$plugin:skill`.
...
codex exec \
  ...
  '<rendered prompt invoking $compound-engineering:ce-code-review>'
...
2. **Hive's default reviewer set is now claude + codex (not 4 CLIs).** Pi and opencode are profile-able but opt-in. U2 ships:
   ...
   - `codex-ce-code-review` (codex profile + `$compound-engineering:ce-code-review` skill)
