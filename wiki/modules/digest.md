...
waiting. Pairing-store read failures degrade to no reminder, because pairing
counts are informational and should not abort a daily digest.
...
successful. Rendering groups rows by repository and uses the existing
MarkdownV2 escaping/link helpers from `Digest::Renderer`.
