  ...
  module Tui
    ...
    module Help
      BINDINGS = [
        ...
        { mode: :grid, key: "r", action: :review,        description: "run hive review; on a max_passes-hit REVIEW_STALE row this submits coordinator recovery after you've edited the focal escalations file (Enter on the same row opens that file in $EDITOR)" },
        ...
        { mode: :grid, key: "Enter",     action: :open_contextual,    description: "left pane: focus right. right pane: open red-status detail on ERROR rows; wall_clock REVIEW_STALE keeps direct coordinator retry, max_passes REVIEW_STALE opens focal escalations; non-red contextual behavior is unchanged" },
