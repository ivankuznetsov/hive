...
| `:review_error phase=reviewers reason=reviewer_partial_failure` | `next: inspect <folder>/reviews/errors-NN.md, then refresh operational status and invoke the task's workflow.retry action`. |
| `:review_ci_stale` | `next: fix CI, edit reviews/ci-blocked.md, then refresh operational status and invoke workflow.retry` |
| `:review_stale` | `next: inspect or edit the highest-pass review files, then refresh operational status and invoke workflow.retry` |
...
| `:review_error` | `next: investigate <reason>, then refresh `hive status --operational --json` and invoke the task's workflow.retry action`. JSON: `next_action.kind = "review_error"` with `phase`, `reason`, and the full marker `attrs` surfaced so polling agents can branch without re-parsing the marker. Raises `Hive::TaskInErrorState` → exit 3 (`TASK_IN_ERROR`) after the JSON payload is emitted. |
| `:error` | raises `Hive::TaskInErrorState` → `bin/hive` rescues → exit 3 (`TASK_IN_ERROR`). JSON mode emits the full payload first, then raises — dual signal. **Reason-gated branch:** `reason=ensure_clean_on_exit_failed` emits `next_action.kind="edit"` with `target=worktree_path` (falls back to `task.folder`), `residue_paths` parsed from the comma string into an array, instructions to repair the worktree and then use the fresh workflow.retry action, `markers_to_clear=["error"]`, and `rerun_with` set to the stage's friendly command. `Hive::TaskAction#suggested_next_action_payload` emits the exact generation-guarded `hive act workflow.retry ... --observation TOKEN` command for this and every other real `ERROR`, so bot, TUI, status consumers, and the daemon share the durable retry contract. |
