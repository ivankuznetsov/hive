...
class RefactorPatrolPolicyTest < Minitest::Test
  ...
  def test_capture_persists_every_action_policy_input
    ...
    assert_equal 8, policy.dig("action", "caps", "max_files")
  ...
  def test_effective_policy_intersects_caps_and_thresholds_in_the_stricter_direction
    ...
    assert_equal 8, result.config.dig("refactor_patrol", "caps", "max_files")
    assert_equal 400, result.config.dig("refactor_patrol", "caps", "max_diff_lines")
  ...
  def test_current_policy_can_narrow_a_snapshot
    snapshot = Hive::RefactorPatrol::Policy.capture(config, now: T0)
    current = config
    current["refactor_patrol"]["caps"]["max_files"] = 3
    current["refactor_patrol"]["min_confidence"] = "high"
    current["refactor_patrol"]["issue_filing"]["min_leverage_score"] = 0.8

    result = Hive::RefactorPatrol::Policy.intersect(snapshot, current)

    assert_equal 3, result.config.dig("refactor_patrol", "caps", "max_files")
