...
## [2026-05-13T22:00:00Z] review — embed `plan.md` inline in every reviewer system prompt

**Action:** Wired the task's `plan.md` into every reviewer spawn so reviewers can ground findings against the plan's stated scope, not just the worktree they see in isolation. New `Hive::Reviewers::PlanContext.render(task_folder)` helper reads `<task_folder>/plan.md`, wraps it in a clear instructional block ("authoritative on scope" header + `--- BEGIN plan.md ---`/`--- END plan.md ---` markers + an explicit anti-finding rule: "if a candidate finding would flag a deliberate plan-level scope boundary, drop the finding"), and returns the resulting string. `Hive::Reviewers::Agent#render_prompt` passes the rendered section into `TemplateBindings` as `plan_context_section`, and all three reviewer ERB templates (`reviewer_claude_ce_code_review.md.erb`, `reviewer_codex_ce_code_review.md.erb`, `reviewer_pr_review_toolkit.md.erb`) interpolate it between the `Pass:` header line and the `Behavior:` instruction block. Missing/empty/unreadable `plan.md` falls back to a fixed `PlanContext::ABSENT_NOTE` so the prompt stays well-formed and the reviewer flags missing-plan in its review-output header. Motivated by the xbookmark `create-proper-readme-md-for-260513-2ba1` task: the plan deliberately deferred `bin/xbookmark` to a separate downstream task, but reviewers (who only saw the worktree) kept escalating "README references files that don't exist" pass after pass, driving the task into `REVIEW_STALE pass=4` because the fixer couldn't resolve a plan-by-design contradiction. Five `PlanContext` unit tests pin the four shapes (present/absent/empty/unreadable + the trailing-whitespace rstrip); two new agent tests pin the embed-in-all-three-templates contract.
...
7. **TUI marker-pass+reason match on 5-review save.** `BubbleModel#review_marker_still_open?` now requires `marker.pass` AND `marker.attrs[reason]` to match the row snapshot taken on Enter. A stale editor session for pass 4 that saves AFTER a concurrent process advanced the task to pass 5 no longer dispatches.
...
- [[commands/tui]] — 5-review auto-continue paragraph updated to mention the pass+reason match requirement on save.
