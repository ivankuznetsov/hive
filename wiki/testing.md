...
| `patrol/effect_gateway_test.rb`, `patrol/state_store_effect_intents_test.rb`, `patrol/pr_opener_test.rb`, `patrol/review_handoff_test.rb` | Ordinary effect recovery and publication — one canonical occurrence journal with no legacy StateStore effect maps, durable intent before authorization, one sender CAS/lease, reconciliation-only expired uncertainty, reconcile-only adoption of an exact existing hosted PR, terminal canonical receipt replay/outbox binding, exact base/head and clean-worktree binding, leased/verified remote publication, final remote base/head recheck before task publication, stale-base failed-handoff retry refusal, durable `reconciliation_pending` URL/patch receipts across lookup/auth failures, fail-closed exact-diff/title/body secret scanning, closed results, proof handoff, and exact-patch handoff retry. |
...
| `managed_directory_test.rb`, `modules/migration/{bounded_file_inventory,evidence_store,shadow_comparator,shadow_decision_migration}_test.rb`, `integration/module_migration_test.rb` | Patrol migration storage — linked managed-component, file, and lock refusal; descriptor/inode-bound bounded reads and atomic writes; filesystem-order-independent lexicographic cursors bound to one store and high-water inventory; mutation and overflow invalidation; bounded receipt-index repair; early shadow-history excess rejection before body reads; streaming report aggregation; and no-follow bounded v1 archive collision handling. |
