...
| `status`   | Reports running / not running. Exit code 0 if running, 1 if not. With `--json`, emits a `hive-daemon-status` envelope with `running`, `pid`, `uptime_sec`, `pid_file`, `log_file`, plus the autostart-service state `service_installed`, `service_enabled`, and `unit_path` (read-only probe) so an agent can tell whether `hive daemon install` has run without a mutating call. The JSON envelope is produced by `Hive::Daemon::StatusReport`, which also feeds the web dashboard, and reports `installed_binary`, `expected_binary`, `installed_binary_version`, `cli_version`, and `binary_drift` (`none`, `path`, `version`, `unparseable`, `unreadable`, or `not_applicable`). Its `job_store_resets` field is a read-only row for every registered project: `fresh`, `current`, `reset_required`, `reset_incomplete`, `conflict`, or isolated `error`, with any archive and receipt paths. The whole producer is observation-only: it reads a detected legacy project registry in place and skips update-state temporary-file cleanup. Status never converts or archives v2 jobs; `reset_required` needs the explicit confirmed command documented in [[commands/refactor-patrol-reset]]. |
