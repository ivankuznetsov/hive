...
locations. Each candidate's `--version` probe runs in a dedicated process group
with a five-second watchdog so forked helpers cannot hold the probe open or
survive its cleanup. Use `HIVE_BIN_OVERRIDE` for a custom Hive CLI install path. RubyGems
