...
**TLDR**: Minitest for unit/integration coverage, plus opt-in outer e2e, eval, native package/bootstrap, install-smoke, release-verify, and Hivebox image smoke layers. `test/unit/` covers modules, including the fenced language-neutral architecture-patrol lifecycle; `test/integration/` covers command/stage behaviour in-process, `test/e2e/` drives the real `bin/hive` subprocess plus tmux for TUI scenarios, `test/eval/` evaluates the Telegram bot signal contract, and CI layers RuboCop, Brakeman, bundler-audit, installer checks, Rails tests, and Docker smoke tests on top.
