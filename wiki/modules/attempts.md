...
the private supervisor argv adapter starts the owner wrapper, and three
read-only compatibility adapters plus `TaskClosure`'s active-attempt
verification open the canonical store. These sites are not alternate admission
producers. The component-boundary test pins each file/constant pair and rejects
the same construction from any newly listed file even while Attempts remains a
