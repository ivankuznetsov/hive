...
| `Hive::Patrol::Finding` | `lib/hive/patrol/finding.rb` | Durable finding record with delivery metadata (`scope`, contract, impact, root cause, reproduction, validation, computed alpha score), exact target SHA, selected validation key, and explicit active/resolved/rejected/superseded lifecycle while retaining backward-compatible v1 loading. |
| `Hive::Patrol::FindingRegistry` | `lib/hive/patrol/finding_registry.rb` | Reconciles existing finding records with merged/open fingerprint state and dismissals, suppresses terminal or same-target semantic duplicates before persistence, and supersedes an older active record only when a newer target is admitted. |
...
- Each semantic root maps to at most one durable active/resolved/rejected lineage. Same-target and terminal semantic duplicates are suppressed before another finding file is written; a newer target supersedes an older active record. An open PR still blocks additional variants from the same feature, and one feature normally supplies at most one fix per cycle.
