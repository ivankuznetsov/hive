...
| `Bubbletea::WindowSizeMessage` | ✅ Present | Auto-sent on terminal resize via SIGWINCH trap installed by Runner. **Note Runner installs its own SIGWINCH handler** (`runner.rb#setup_resize_handler`) and chains a previous handler when one was set. As of 2026-06-05, `Hive::Tui::App.run_charm` installs its own WINCH hook first so the app model also receives a typed `Messages::WindowSized` from `STDOUT.winsize`; Bubble Tea still owns renderer sizing. |
...
- **The bubbletea-ruby Runner installs its own SIGWINCH handler** (`runner.rb:99-117`). It stores and calls the previous handler when the previous handler is a `Proc`, so `Hive::Tui::App.run_charm` now installs a small app-level WINCH hook before `Runner#run`: Hive dispatches `Messages::WindowSized` into the model, then Bubble Tea sets `@resize_pending` and resizes the renderer. Cleanup restores Bubble Tea first, then Hive restores the pre-Hive handler.
