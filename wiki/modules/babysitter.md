...
| `Hive::Babysitter::Worktree` | `lib/hive/babysitter/worktree.rb` | Recreates `<resolved-hive-state>/babysitter/worktrees/<pr>/` from a force-refreshed internal PR-head ref. Relative custom state roots are resolved against the project root through `Config.project_hive_state_path`. |
...
- Cross-repository PRs remain outside the repair boundary, but the status
  precheck now runs first: a fork PR that is already green and not `BEHIND`
  returns `already_green` without applying `needs-human`. Other fork PRs follow
  the existing `fork_pr` skip and label path instead of being mutated.
...
- Skip-log FIFO and concurrency hardening: dry-run setup creates the private regular audit log before agent subprocesses launch, so concurrent first writes cannot race on exclusive creation. Both stubs still preflight existing targets with `File.lstat`, open with `File::NOFOLLOW | File::NONBLOCK`, keep the post-open regular-file/owner/identity checks, and warn while still skipping when a FIFO, symlink, device, hard link, non-owned path, or direct-stub creation/replacement race is configured.
