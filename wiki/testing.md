...
| `honeycomb/{reference,catalog,registry,manifest,package,security_report,lockfile,installation,transaction,diff}_test.rb`, `commands/workflow_honeycomb_test.rb` | Honeycomb immutable reference/catalog resolution, last-known-good bare cache, adversarial tree/manifest/descriptor validation, derived permission and shell-risk reporting, deterministic lock ownership inspection, journal rollback/recovery, semantic/full instruction diffs, pin/downgrade/no-op rules, approval/collision behavior, offline/remote/outdated listing, multi-package updates, partial removal, and all four JSON schemas. |
...
| `honeycomb_workflow_management_test.rb` | Hermetic real-Git honeycomb acceptance — publishes tagged package/catalog revisions to a local bare registry, initializes real projects, and proves inventory-only install plus nested discovery/lock/commit, network-free local list, permission-escalating full-diff update, equal-SHA no-op, invalid-package/pre-approval preservation, clean/dirty/forced removal, and non-zero missing-lock best-effort cleanup. |
