...
| `cli_version_test.rb` | `bin/hive` wrapper contract — top-level `--version`, command-local help after option-bearing invocations (`hive approve --from 2-brainstorm --help`), leading/trailing `--json=true` equivalence for `status`, and pre-target-resolution rejection of unsupported `--json=1` / `--json=yes` spellings. |
