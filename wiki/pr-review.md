...
Command admission, durable worker validation and queued dispatch all resolve review/archive stages through workflow-aware `Workflows.for_verb`; standalone workers must not validate against coding stage numbers. Reviewer sub-spawns preserve the task metadata's workflow identity and stage directory, so session observations stay in the same journal stream.
