...
class AgentSkillsCanonicalSkillTest < Minitest::Test
  ...
  def test_current_format_policy_omits_retired_daily_activity_surface
    files = Hive::AgentSkills::CanonicalSkill.new.rendered_canonical_files
    route = files.fetch("SKILL.md")

    refute files.key?("references/daily-digest.md")
    refute_includes route, "what happened today, yesterday"
    refute_includes route, "hive digest --json"
    refute_includes route, "hive answer-digest"
  end
