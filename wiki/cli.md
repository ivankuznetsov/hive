...
Before Thor dispatch, `bin/hive` handles three wrapper-level cases itself:
top-level `--version` / `-v` prints `Hive::VERSION`; leading JSON class
options are moved after the first subcommand so `hive --json status` and
`hive --json=true run` behave like command-local `--json`; and command-local
`--help` / `-h` is rewritten to `help <cmd>`. Leading truthy JSON spellings
`--json`, `--json=true`, and `--json=t` are canonicalized to `--json` before
dispatch, mirroring Thor's boolean grammar; `--json=1` and `--json=yes` are
not normalized. Leading negations `--json=false`, `--json=f`, `--no-json`, and
`--skip-json` are moved after the command as negations. The help rewrite
preserves any leading dash-prefixed
arguments that appear before the subcommand, then drops the command-local
arguments after the subcommand so calls such as
