...
| `babysitter/dry_run_env_test.rb` | `Hive::Babysitter::DryRunEnv` and its Git/GitHub stubs — default-deny classification, Git 2.45+ lazy-fetch preflight, hosts-only per-invocation `gh` auth views, prompt suppression, environment and executable-path scrubbing, working-tree diff/verbose-status/filter guards, `gh api` value-consuming option parsing, deterministic skip-log creation/replacement race handling, and concurrent oversized skip-log record serialization. |
