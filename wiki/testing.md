...
| `patrol_command_test.rb`, `unit/patrol/token_budget_test.rb`, `unit/usage_db_test.rb` | `hive patrol` — JSON envelope, dry-run behavior, language-neutral default component mapping, globally ranked selection, immutable selection-audit recording, SHA pinning across default advancement after a cursor-zero review error, scan-state recording, inbox non-interference, retry/backoff outcomes, schema validation with fake mapper/reviewer/fixer/PR opener collaborators, shared per-cycle/day token and launch ceilings, unmetered launch accounting, and ordinary plus architecture patrol attribution. |
