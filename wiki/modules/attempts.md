...
| `Supervisor` | Claim, first-heartbeat, spawn the existing Hive command, heartbeat, frame output, enforce timeout/cancellation, validate one child diagnostic frame, bind its exact private log reference, and terminalize only after appending the diagnostic output reference. When every output pipe is at EOF but worker exit or a lingering group is still pending, it sleeps the computed wait budget instead of busy-spinning on `WNOHANG`. |
| `Client` | Tail frames read-only and replay a terminal result, consuming the repository's single `read_log` contract instead of sniffing store capabilities or resolving frame paths. It does not own or reap the supervisor or worker. It performs one final drain after observing a terminal or lost record so frames published during the decisive record fetch are not dropped. Interrupt means detach; it never signals the owner group. |
...
The private supervisor route is selected before the public CLI loads Thor, and
the detached wrapper scrubs inherited Bundler/Ruby toolchain variables before
it re-enters Hive. This keeps its startup bound to the invoked Hive checkout
rather than a caller bundle or transient test home, while the capability and
handshake descriptors remain the only inherited launch authority.
