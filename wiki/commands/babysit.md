...
Skip logging itself is guarded: both stubs open `HIVE_BABYSITTER_DRY_RUN_LOG` (or the default `.babysitter-dry-run-skipped.log`) with `File::NOFOLLOW`, create it as mode `0600`, require the opened target to be a regular file owned by the current uid, and warn if the log cannot be written. The skipped command still exits successfully after that warning so dry-run behavior remains default-deny even when the audit sink is unsafe or unavailable. Logged and stderr-rendered argv escapes ASCII control characters as `\xHH`, so a skipped argument containing a newline cannot forge extra log lines.
