...
`suggested_next_action` remains diagnostic guidance only. Retryable red rows expose `{kind: "retry", command: null}`; surfaces compose a generation-scoped `hive retry manual` intent from the journal projection. Rows requiring human repair expose `{kind: "manual_fix", command: null}`. `TaskAction` never constructs a marker-clear/stage-run retry command, computes a deadline, or decides retry eligibility.
...
- **`:error`** → always `error` at the status/action layer. `TerminalErrorRegistry` supplies bounded evidence/guidance and `FailureReporter` registers the terminal attempt; the exact optional retry projection determines cooldown/readiness. Dead-PID and legacy terminal markers use the same coordinator path, with no healer-owned clear or same-stage queue exception.
