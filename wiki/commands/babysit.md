...
Every allowlisted `git` passthrough also carries the global `--no-lazy-fetch` option and `GIT_NO_LAZY_FETCH=1`, preventing a partial-clone object read from contacting a promisor remote or materializing the missing object. Git 2.45 and newer honor this client-side guard; older Git releases reject the unknown global option before dispatching the read, so babysitter dry-run fails closed while Hive's non-dry-run Git minimum remains 2.40.
