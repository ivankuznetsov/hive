...
4. **The current ClawHub audit is understood, but v0.1.3 publication evidence is pending.** `openclaw/skills/hive/` is the only public OpenClaw skill source and installs as `@ivankuznetsov/hive-cli`; its slash command remains `/hive`. The live v0.1.2 audit reports clean static and VirusTotal results but a behavioral `Review`: the high actionable finding is Arch package installation with `--noconfirm`, while expected-risk notes cover daemon auto-advance and persistent runtime/service repair instructions. Checked-in v0.1.3 removes the unattended flag, direct installed-runtime patches, direct service override writes, and the embedded polling program; it separates `setup --no-init` core provisioning from interactive project enrollment, binds auto-advance to previously approved enrollment, keeps agent-backed patrol dry-runs consent-gated, and adds current Hive 0.6.4, managed-skill, Honeycomb, patrol-tier/token, digest/bench, TUI/web, and consent guidance. Until v0.1.3 is published and the immutable version's audit completes, no checked-in artifact proves the live listing moved from `Review` to `Pass`. A real installed OpenClaw invocation of `/hive wiki compile-log --check` also remains unrecorded.
