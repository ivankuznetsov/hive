...
type: observability
source: lib/hive/usage_db.rb, lib/hive/patrol/token_budget.rb, lib/hive/agent_profiles/usage_extractors.rb, lib/hive/tui/views/token_stats.rb, lib/hive/tui/bubble_model.rb
created: 2026-05-24
updated: 2026-07-16
...
`Hive::UsageDb.patrol_activity` is the runtime enforcement view for the current UTC day. It returns input/output/cached and their conservative sum, total patrol agent launches, and unmetered launches. `Hive::Patrol::TokenBudget` checks that durable project-wide view before each default ordinary or architecture patrol spawn, while also retaining an in-process cycle counter if SQLite recording fails. Cached tokens count toward the ceiling. Architecture stages default to 2x the ordinary cycle token/launch limits and per-agent budget-equivalent guard, but they do not get a separate daily pool. One child can finish above the remaining token allowance because a CLI reports usage only after exit; the independent per-agent runaway guard, launch caps, and wall-clock timeouts bound that final overshoot. The CLI calls the guard a USD budget; on subscription-backed providers it does not mean Hive makes an additional payment.
