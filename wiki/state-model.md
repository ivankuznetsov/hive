...
title: State Model
type: data-model
source: lib/hive/task.rb, lib/hive/task_meta.rb, lib/hive/task_closure.rb, lib/hive/task_journal.rb, lib/hive/task_projection.rb, lib/hive/work_ledger.rb, lib/hive/terminal_outcome.rb, lib/hive/completion_time.rb, lib/hive/archive_filter.rb, lib/hive/markers.rb, lib/hive/config.rb, lib/hive/attempts/*, lib/hive/runtime_control_plane/*, lib/hive/lock.rb, lib/hive/process_kill.rb, lib/hive/commands/drop.rb, lib/hive/worktree.rb, lib/hive/metrics.rb, lib/hive/usage_db.rb, lib/hive/bot/*, lib/hive/patrol/*, lib/hive/patrol_fix/*, lib/hive/refactor_patrol/*, lib/hive/daemon/refactor_patrol_merge_*.rb, lib/hive/web/status_feed.rb, web/app/models/status_broadcaster.rb, web/app/javascript/status_stream_source.js
created: 2026-04-25
updated: 2026-09-09
...
The internal scheduler/UI projection exposes two task timestamps from this layout: `mtime` is the current stage state-file mtime (or the folder mtime fallback when the state file is missing), while `folder_mtime` is always the task folder's own `File.mtime`. Daemon edit-resume decisions continue to use `mtime`; consumers that need directory-level aging can use `folder_mtime` without re-walking the filesystem. The projection also exposes `pr_url` from `pr.md` frontmatter for coding tasks at `5-open-pr` and later and for generic workflows whose descriptor declares `handoff: draft_pr`, returning `null` before verified PR metadata exists or when the sidecar is absent/unparseable. See [[commands/status]].
...
Outcome-evidence implementation rework is an audited backward transition, not a
new stage or a marker clear. A strict `current.json` pointer at
`status=rework` binds the rejected generation and digest; the controller rearms
`7-artifacts` to `4-execute` through `Commands::Approve` and appends one of two
mode-0600 receipts under `<task>/outcome-evidence/reworks/`. Each receipt binds
the reviewed base/head, targets, and reasons. The evidence generation remains
immutable, and both future receipt slots are protected while the implementer
runs, including before the first receipt exists. The same custody manifest
protects every representation and project-provider manifest named by the
rejected attempts. Unrecognized siblings are not controller receipts and cannot
poison the bounded inventory. Exact replay is idempotent; a third distinct
authorization is impossible. Execute compares the reviewed and resulting
repository trees through the hardened Git read boundary, so an empty descendant
commit does not count as implementation progress.
...
| `<!-- ERROR reason=ensure_clean_on_exit_failed failure_kind=<scope_violation\|safety_violation\|git_failed> residue_paths=<rel,paths> marker_id=<hex16> -->` | `Hive::Stages::CleanExit` could not safely commit known worktree residue because it was outside `review.fix.auto_commit.scope_check`, contained a staged symlink or oversized blob, added content matching a shared secret detector, a bounded Git operation failed, or auto-commit config was invalid. The marker preserves bounded `detail`, a typed `failure_kind`, display-safe `residue_paths`, and a complete recovery identity (`residue_paths_count` plus `residue_paths_sha256`). Small non-sensitive exact lists use `residue_paths_b64`; large or secret-shaped lists use the integrity-checked owner-private `.clean-exit-residue-paths.json` sidecar so the marker stays redacted and cannot exceed `MAX_MARKER_SCAN_BYTES`. CleanExit captures exact NUL-delimited dirty paths before mutation, so failures without a path diagnostic still remain recoverable. Review's pre-fix snapshot uses this same marker with `origin=review_pre_fix phase=fix pass=N`; it stops before fix-agent launch, unlike an initial unreadable Git status, which remains `REVIEW_ERROR reason=fix_status_check_failed`. Inspect or repair with `hive worktree status\|commit-residue\|discard-residue`; repair leaves the marker in place until a fresh generation-guarded retry. | `Hive::Stages::Base#enforce_clean_exit!`, `Stages::Finalize` entry backstop, `Stages::Review` pre-fix snapshot |
| `<!-- ERROR reason=dirty_worktree marker_id=<hex16> -->` | the execute agent left durable uncommitted progress in its Hive-owned worktree; the daemon resumes it through ordinary generation-guarded recovery. Historical `EXECUTE_WAITING reason=dirty_worktree` is upgraded only when a durable `attempt_id` proves agent ownership. | `Stages::Execute#run!`, `Daemon::StaleAgentHealer` |
| `<!-- EXECUTE_WAITING reason=no_worktree_changes\|missing_research_output\|branch_mismatch\|head_not_descendant -->` | impl spawn exited cleanly but needs plan/output/branch repair before completion | `Stages::Execute#run!` |
...
stages:                  # stage-runner-level invariants enforced by `Hive::Stages::Base.with_stage_events`
  ensure_clean_on_exit: true            # default true; `Hive::Stages::CleanExit` runs as a post-yield hook on every WORKTREE_OWNING stage (`4-execute`, `6-review`, `8-finalize`). Clean worktree → no-op. `git add -A` means enabled projects must keep `.gitignore` strict. Residue must pass the configured filename scope, staged-symlink rejection, and added-line secret scan before auto-commit. Rejection or git failure writes `ERROR reason=ensure_clean_on_exit_failed`. PAUSE_MARKERS (`:execute_waiting`, `:review_waiting`) skip enforcement. Finalize also runs CleanExit as an entry backstop. The resolved boolean is published under the internal task graph's project `config_summary`.
```
