...
2. **Resolve workflow default** — explicit `--workflow NAME` wins. Without a flag, TTY init prompts for a registered workflow with `coding` as the default; non-TTY init silently selects `coding`. The selected default is validated through [[modules/workflows]].
3. **Already-initialized handling** — if `hive/state` exists and no workflow was explicitly selected, raise `Hive::AlreadyInitialized` (exit 2). If a workflow was selected, attach/validate the existing state worktree, update `<project>/.hive-state/config.yml`, and warn when changing the default would re-resolve in-flight field-less tasks.
