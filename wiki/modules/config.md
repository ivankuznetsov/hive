...
**Patrol is opt-in.** `resolve_patrol_mode!` runs on the raw YAML before `merge_defaults` and only derives/injects mode knobs when `mode:` is **explicitly present** in the raw config (`return unless nested_key?(data, "patrol", "mode")`). A config with **no patrol section** — or a patrol section that omits `mode:` — injects nothing and falls through to `DEFAULTS["patrol"]["enabled"] = false`, so patrol stays **disabled**. `medium` is the default offered by the `hive init` *prompt*, never a config-resolution default. The explicit modes are `ultrapatrol` (`timer`/30m, 800k tokens and 10 launches per cycle, 2.4m/36 per UTC day, $100 per agent), `high` (`timer`/2h, 400k/6 per cycle, 1.2m/18 per day, $50), `medium` (`timer`/4h, 200k/3 per cycle, 600k/8 per day, $25), `low` (`new_commits`, 100k/1 per cycle, 200k/2 per day, $10), and `off` (`enabled: false`). Token and launch limits are shared by ordinary and architecture patrol for the project; the launch limit is the provider-independent backstop when token counts are absent. The mode never changes finding/PR caps, per-feature diversity, confidence, or alpha gates. Explicit granular knobs always win over a set mode and survive the deep-merge.
