...
| Diagnostics | `hive status`, `hive circuits`, `hive doctor`, `hive rebase-status`, `hive markers clear`, `hive metrics rollback-rate` | Inspect task/provider state, validate configured stage/reviewer skills, check whether the next run would auto-rebase, clear a recovery marker or provider circuit, or report fix-agent rollback rate. See [docs/cli.md#diagnostics](docs/cli.md#diagnostics). |
