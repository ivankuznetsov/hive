...
- `bin/hive` catches `Thor::Error` around the main CLI dispatch. Without `--json`, it prints Thor's usage error and exits 64. With `--json`, it emits a single error document on stdout before exiting 64; known parse-error commands are routed to their registered schema (`hive-run`, `hive-stage-action`, `hive-findings`, etc.) through `JSON_PARSE_ERROR_ENVELOPES`, including the schema-required `verb` extra for stage-action verbs. The integration suite round-trips every parse-error-raising mapped command against its published schema, and pins `hive pr --json` to the canonical `verb: "open-pr"`. An unknown command has no versioned schema, so it falls back to an unversioned `{ok:false, error_kind:"usage", error_class:"UsageError", ...}` shape.
