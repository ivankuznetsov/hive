...
For other `REVIEW_STALE` rows (max_passes hit with completed passes), clearing the marker without first editing or renaming the highest-pass review files just re-enters the same stale state. Enter therefore flashes the manual pass-cleanup instruction and leaves the marker intact.
