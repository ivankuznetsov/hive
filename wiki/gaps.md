...
36. **Resolved 2026-07-20: root README/FAQ no longer claims there is no built-in web UI.** Maintained onboarding now presents native Hive web first and reserves Hivebox for the container distribution. The old wording remains relevant only as historical evidence in this gap record.
...
4. **The v0.1.3 ClawHub publication is verified; only a real OpenClaw invocation remains unrecorded.** `openclaw/skills/hive/` is the only public OpenClaw skill source and installs as `@ivankuznetsov/hive-cli`; its slash command remains `/hive`. The previous v0.1.2 audit had clean static and VirusTotal results but a behavioral `Review`, led by Arch package installation with `--noconfirm` and expected-risk notes about daemon auto-advance and persistent repairs. Published v0.1.3 removes the unattended flag, direct installed-runtime patches, direct service override writes, and the embedded polling program; it separates `setup --no-init` core provisioning from interactive project enrollment, binds auto-advance to previously approved enrollment, keeps agent-backed patrol dry-runs consent-gated, and adds current Hive 0.6.4, managed-skill, Honeycomb, patrol-tier/token, digest/bench, TUI/web, and consent guidance. ClawHub accepted immutable version ID `k9755za2qn954070sytzmw4e958awza0` with fingerprint `c172cc4a4b71ec712512c3ae64ba0c92f36d1432e29096df7b08ddb37b8357f0`; explicit scan `w176fz5cgtewgc335nmbcc32018ax5qh` wrote back clean static analysis, zero Skillspector issues (`SAFE`), and a benign ClawScan verdict. After staged pre-publication finalization, exact-version inspection reported `latest: 0.1.3`, a clean moderation verdict with no reason codes, and one published `SKILL.md` whose SHA-256 is `f7a0981a6058a7b5e7fa0a385589edb558905cca295fa041c673d46a40395202`. Clean temporary pinned and unversioned installs both selected v0.1.3 and matched that reviewed source byte-for-byte. A real installed OpenClaw invocation of `/hive wiki compile-log --check` remains unrecorded.
