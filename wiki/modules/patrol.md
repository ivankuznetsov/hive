...
The managed repository worktree is not edited by fixes. `Fixer` uses [[modules/worktree]] to create a branch named `hive-patrol/<feature-id>-<fingerprint8>` under the project's worktree root. When `patrol.review_prs` is enabled (default), that worktree is kept after PR creation and referenced by a synthetic `6-review` task with display name `Patrol: <finding title>`. The synthetic task's `idea.md` is derived from the original patrol finding rather than the PR body: description, recommendation, and optional evidence become the markdown body, and the identical body is intentionally duplicated into frontmatter `original_text` so [[commands/tui]] info panels can show the same source context they show for `hive new` tasks. When disabled, the successful local worktree is removed after the branch is pushed and the PR opens.
