...
| `Hive::PatrolFix` | `lib/hive/patrol_fix/` | Source-neutral workflow core for strict source snapshots, durable semantic admission, idempotent task materialization, task manifests, receipts, and common projection. Candidate-set decisions are digest-bound and revalidated after unlocked provider calls. Materialization intent is durable before task capture; task/evidence binding precedes source acknowledgement; retry repairs interrupted creates and provenance updates. The core has no dependency on either Patrol product, Commands, Daemon, or UI. |
