...
| `test/fixtures/incidents/topgreendeals-pr-295/` | Sanitized, digest-pinned offline lifecycle replay: stable handoff, restart/takeover, stale-head invalidation, open-list disappearance, closed-unmerged negative case, and exact `MERGED` observation at `2026-07-16T23:05:50Z`. `test/support/finalization_replay.rb` rejects every un-scripted GitHub operation. |
...
| `task_journal_test.rb`, `conditions/*_test.rb`, `task_projection*_test.rb`, `task_action_conditions_test.rb` | Generation-scoped conditions — strict journal durability/attempt integrity, registry/evidence lifecycle, generation/HEAD reconciliation, deterministic supersession, snapshot validation/replay, gate/migration/shadow modes, execute boundary ordering, canonical TaskAction/status consumption, and sanitized task-1849 replay across missing/corrupt/stale snapshots with live-observation sentinels. |
...
| `finalization/{event,projection,reconciler,outcome_validator,archive_cleanup}_test.rb`, `babysitter/{job,job_store,job_runner,project_tick}_test.rb`, `daemon/dispatcher_test.rb` | Durable finalization authority — typed/idempotent journal events, lifecycle/head/attempt folding, deterministic job identity and inactive activation, leases/fences/restart takeover, exact-PR observation, closed-unmerged/GitHub-outage blockers, reconciler-only archive eligibility, guarded no-PR outcomes/re-arm, and exact idempotent local cleanup. Dispatcher coverage also pins digest scheduler dispatch/completion, dry-run digest pseudo-child reaping, and fatal-log isolation when `DigestScheduler#complete` raises. |
...
| `patrol/pr_opener_test.rb`, `patrol/review_handoff_test.rb` | `Hive::Patrol::PrOpener` / `ReviewHandoff` — exact base/head and clean-worktree binding, leased/verified remote publication, created/existing PR reconciliation, durable `reconciliation_pending` URL/patch receipts across lookup/auth failures, fail-closed exact-diff/title/body secret scanning, closed results, proof handoff, and exact-patch handoff retry. |
| `patrol/feature_batch_test.rb`, `patrol/candidate_selector_test.rb`, `patrol/reviewer_test.rb`, `patrol/fingerprint_test.rb`, `patrol/fixer_test.rb` | Ordinary patrol alpha policy — exact detached SHA-bound review rotation across default advances and first-batch errors, observable partial reviews, bounded/atomic source-matching evidence, immutable finding ids, legacy/current and same-run deduplication, retryable pending publication, model-label-light alpha ranking, strict fresh-base creation, language-neutral declared regression paths, guardrails, nonblocking no-follow regular-file proof reads, normal regression-identified fail-before/pass-after proof, and non-suppressing fixer rejection. |
...
| `run_finalize_test.rb` | `hive run` of `8-finalize/` — exact PR/head resolution, stable inactive reservation, append-once handoff/adoption, activation/repair across injected crash boundaries, summary-as-presentation semantics, replacement rejection while an old PR is open/ambiguous, and already-merged PR handoff without direct terminal authority. |
| `run_done_test.rb` | `hive run` of `9-done/` — `archive_ready` guard, exact registered worktree/local-branch cleanup, crash resumption, wrong-target/newer-generation refusal, and one durable cleanup receipt before complete. |
| `run_stage_action_test.rb` | Workflow verbs — reconciler-only archive admission, `--force` refusal, exactly-once stage move, and retrying moved `9-done` cleanup until its receipt exists. |
| `finalization_lifecycle_replay_test.rb` | Entirely offline PR 295 incident-shape replay using production JobStore/JobRunner/ProjectTick/Reconciler/ArchiveCleanup: exact merge timestamp, restart claim takeover, stale-head reset, list-disappearance non-authority, closed-unmerged negative case, resume at persisted boundaries, exactly one terminal effect, fixture tamper detection, and legacy-summary repair guidance. |
...
| `patrol_command_test.rb`, `unit/patrol/token_budget_test.rb`, `unit/usage_db_test.rb` | `hive patrol` — JSON envelope, dry-run behavior, language-neutral default component mapping, globally ranked selection, immutable selection-audit recording, SHA pinning across default advancement after a cursor-zero review error, scan-state recording, inbox non-interference, retry/backoff outcomes, schema validation with fake mapper/reviewer/fixer/PR opener collaborators, shared daily token/launch ceilings, the architecture cycle/per-agent multiplier, unmetered launch accounting, and ordinary plus architecture patrol attribution. |
| `refactor_patrol_command_test.rb`, `refactor_patrol/job_query_test.rb`, `refactor_patrol/job_query_index_test.rb`, `refactor_patrol/job_store_test.rb` | `hive refactor-patrol` — v1 compatibility plus PR-scoped/manual v2 manifest consumption, immutable checkout/scope, command-level failure that releases the claim and persists no results when discovery mutates tracked source, terminal replay ledgers, partial feature resume, exhaustive zero/error envelopes, action projection, recursive suppression, exact-registration/duplicate-owner blocking before manual metadata resolution, bounded read-only job inspection even while discovery is disabled, immutable sequence/high-water cursor membership across same-time inserts and clock rollback, selected-job-only parsing, honest namespaced and legacy-flat publication truncation metadata, claim-generation-ordered current blockers, pre-index migration, both membership/job crash windows, rebuild/write fencing, failed-rebuild generation retention, explicit index rebuild/stale-cursor rejection, selector-less jobs-v1 usage errors, and dry-run proof that current consent/ownership is evaluated while state, refs, worktrees, and external collaborators remain unchanged. |
