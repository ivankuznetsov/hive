...
| `/status [project]` | Renders actionable rows from the bot's internal status projection as `Title… — Stage`; when a project name is supplied, filters to that project. Telegram is human-only: `/status --json` directs automated callers to the bounded `hive status --json` liveness contract or `hive status --operational --json` workflow contract over authenticated CLI access. |
