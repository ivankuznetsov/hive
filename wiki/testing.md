...
| `commands/{bot,daemon}/service_installer_test.rb`, `commands/service_installer/base_test.rb` | Shared service installer behavior — systemd/launchd rendering, cycle-free `WantedBy=default.target` daemon ordering, force-upgrade backups/restarts, unchanged already-loaded launchd idempotency, unsupported-platform service-state probes, Homebrew stable-binary selection, macOS launchd ProgramArguments `$0` parsing for the daemon binary, and shared launchd path substitution used by daemon/web plists. |
...
| `digest/london_window_test.rb`, `digest/repo_resolver_test.rb`, `digest/collector_test.rb`, `digest/stats_test.rb` | Canonical digest collection — exact before/at/after London-midnight membership on both 2026 DST transitions under mismatched host timezones, poisoned task/stage/pairing read seams, registered host-qualified target/filter resolution, stable live pagination, repository-atomic file-backed body/diff/file evidence, independent rename quoting, explicit collection outcomes, safety ceilings, secret redaction, and independently optional metrics with same-slug/different-host and partial/all-missing totals. `digest/window_test.rb` remains the separate host-local AnswerDigest contract. |
| `digest/changelog_generator_test.rb`, `digest/renderer_test.rb`, `digest/run_test.rb`, `digest/sender_test.rb`, `digest/delivery_checkpoint_store_test.rb` | Digest generation/delivery — exact evidence-to-fact-to-bullet and host-qualified project/PR identity coverage, semantic implementation/fix/release/migration generation, prompt-injection fencing, controller-owned policy and whole writable-root cleanup, structural-ID/generated-text redaction, deterministic project/PR links and warnings, MarkdownV2 escaping plus entity-aware chunk boundaries, a two-project/five-PR multi-chunk payload with footer, honest empty/partial/failure orchestration, dry-run token bypass, Telegram boundary redaction/preflight, owner-private atomic delivery checkpoints, durable next-unsent resume without accepted-prefix duplication, and one bounded HTML recovery for Telegram entity-parse rejection. `test/digest/e2e_test.rb` is the opt-in real-agent/Telegram semantic proof; unit seams make no live API call. |
| `daemon/digest_scheduler_test.rb` | `Hive::Daemon::DigestScheduler` — first-run no-history guard, Europe/London due calculation, one-day-at-a-time catch-up, catch-up cap logging, non-zero retry/backoff behavior, typed permanent-delivery blocking without cursor advance or redispatch, disabled mode, and both real London DST transitions. |
...
| `daemon/stale_agent_healer_test.rb` | `Hive::Daemon::StaleAgentHealer` — stale `AGENT_WORKING` healing, wedged `REVIEW_WORKING` lock cleanup, marker-id/live-owner guards, and unbounded retry for every `ERROR` / `REVIEW_ERROR`. Coverage pins the shared cooldown for every reason, retries beyond legacy budgets and fresh marker ids, pre-clear dispatch-baseline seeding, and `3-plan` requeue / failure traces. |
| `daemon/stale_agent_healer_test.rb`, `daemon/auto_retry_safety_test.rb`, `daemon/logger_test.rb`, `events_test.rb`, `daemon/dispatcher_test.rb`, `operational_status_test.rb`, `commands/status_test.rb`, `integration/daemon_auto_retry_test.rb` | Universal durable-error retry — one healer owner for every `ERROR` / `REVIEW_ERROR`, the shared cooldown, temporary current-work-area safety deferral, project/global enable gates, scheduler-owned operational status, unbounded repeated clears, generation guards, durable `3-plan` continuation across temporary config removal, and no exhaustion outcome. |
...
| `gemspec_test.rb`, `install_script_test.rb` | RubyGem/install packaging — `hv` stays out of `spec.executables` so RubyGems does not create a broken Ruby binstub for the bash launcher; the bash installer writes its own `hv` wrapper and does not expect a gem-installed `hv` shim. Fault-injected installer executions also pin exact wrapper/shim byte and mode restoration across gem failure, missing output, shim staging, wrapper write, and chmod failures. |
...
| `test/unit/web/status_feed_test.rb`, `web/test/channels/status_channel_test.rb`, `web/test/models/{board,status_broadcaster}_test.rb`, `web/test/integration/status_test.rb`, `web/test/system/{kanban_board,pipeline_flow}_test.rb` | Hive web status views and broadcasting — descriptor-ordered project/workflow board bands, unknown-stage retention, visible degraded-project/unavailable-workflow states, side-effect-free Board/Grid GET routes plus signed-preference mutation, shared native card actions, URL-addressed server-rendered project filtering with canonical ghost-filter rejection, unrelated-markup exclusion, view-switch preservation, permanent-composer context, and live activity reordering, stable card focus/band scroll across reorder morphs, long-name mobile board containment, registered-project snapshots, one shared scan per poll tick, pinned five-second cadence and zero-subscriber scans, first-request page-snapshot priming without a duplicate connect scan, competing-prime convergence, claim release before poller start plus join-window claim fencing, accepted/rejected-channel lifecycle with barrier-paused setup teardown, first-poller startup rejection/recovery without queued stream work, late adapter-registration cleanup after teardown, deferred adapter-failure lease release/reconnect, per-channel locking, and exactly-once concurrent cleanup, barrier-controlled shutdown during lazy feed installation, emit-on-connect for independent subscribers, canonical semantic page tokens stable across independent feeds, Ruby processes, hash key order, and unrelated `Hive::Digest` constants while ignoring volatile-only ticks and reusing unchanged-token hashes, confirmed-channel stale-page catch-up with one reconciliation GET per token and URL under continuous cross-worker lag, one-shot live-element handoff across same-URL Turbo moves, and reset on real disconnect, pending/failed cross-navigation, or source-less history restoration, atomic render-before-single-send broadcasts, refresh-only view reconciliation including failed-idea deferred replay, card-action mutation survival from a task-page-wide submission owner, redirect-handoff suppression, delayed Cable setup cancellation, poisoned lazy-consumer cache recovery, partial-registration cleanup, transport-scoped reconnect teardown, bounded never-confirmed transport cleanup, bounded retry and disconnect cancellation, failed-send retry on the next confirmation, and missed-broadcast catch-up on both status and task pages after direct or history-restored reconnect, deterministic server-side catch-up completion for one-request fresh Turbo navigation, poller survival after snapshot errors, failed-broadcast retry across last-subscriber shutdown, and deterministic browser coverage that a competing page refresh cannot abort an in-flight mutation or preserve a duplicate-ready permanent draft. |
