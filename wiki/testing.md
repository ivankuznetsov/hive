...
The live shipped-digest E2E is `test/digest/e2e_test.rb` and is intentionally
outside the default `rake test` glob. It requires `HIVE_TELEGRAM_BOT_TOKEN` and
`HIVE_DIGEST_TEST_CHAT_ID` (optional `HIVE_DIGEST_TEST_AGENT`), builds fixture
`9-done` tasks with injected ship times, then runs the real
`Hive::Digest.run` path with a real agent categorizer and real Telegram
`sendMessage`. Missing env vars fail the test rather than skipping.
