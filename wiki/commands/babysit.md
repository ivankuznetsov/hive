...
The current `git` stub read-only allowlist is `branch` (bare, `--show-current`, or `--contains`), `cat-file`, `describe`, `diff`, `grep`, `log`, `ls-files`, `ls-tree`, `merge-base`, `remote` only for listing, `show [-n]`, and `get-url` forms, `rev-list`, `rev-parse`, `show`, `status`, and `config` when paired with `--get`, `--get-all`, or `--list`. Before any pass-through, it skips executable-affecting Git inputs: unsafe `-c` / `--config-env` keys (`core.askPass`, `core.editor`, `core.fsMonitor`, `core.hooksPath`, `core.pager`, `core.sshCommand`, `diff.external`, `gpg.program`, `sequence.editor`, `ssh.variant`, `uploadpack.packObjectsHook`, `alias.*`, `credential.*`, `filter.*`, `gpg.*`, `hook.*`, `hooks.*`, `pager.*`, `protocol.ext.*`), global pager/exec-path controls (`-p`, `--paginate`, `--exec-path`), and `git grep` pager escapes (`--open-files-in-pager`, `-O...`). The `-O` grep guard scans bundled short-option clusters and stops at value-consuming short options (`e`, `f`, `m`, `A`, `B`, `C`) so values are not treated as flags. Before execing an allowlisted read-only command, the stub deletes executable/config-injection Git environment variables (`GIT_ASKPASS`, `GIT_CONFIG`, `GIT_CONFIG_COUNT`, `GIT_CONFIG_GLOBAL`, `GIT_CONFIG_PARAMETERS`, `GIT_CONFIG_SYSTEM`, `GIT_EDITOR`, `GIT_EXTERNAL_DIFF`, `GIT_PAGER`, `GIT_PROXY_COMMAND`, `GIT_SEQUENCE_EDITOR`, `GIT_SSH`, `GIT_SSH_COMMAND`, `GIT_CONFIG_KEY_*`, `GIT_CONFIG_VALUE_*`). The current `gh` stub read-only allowlist is `api` with no mutating method and no payload flags, `api --method GET ...` / `api -XGET ...`, `auth status`, `pr checks/diff/list/status/view`, `run list/view/watch`, `repo view`, and `workflow list/view`.
...
- `test/unit/babysitter/*_test.rb` covers interval parsing, dispatcher ticks, PR filtering, context building, PR fixing, GitHub ops, worktree materialization, and dry-run PATH wrappers, including executable-affecting Git option/config/grep-pager blocking and the `gh api` implicit-POST payload flag guard.
