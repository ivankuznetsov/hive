...
| `managed_directory_test.rb`, `modules/migration/{bounded_directory_entries,migration_repository,qualification_checkpoint_verifier,qualification_process_custody,qualification_scenario_evidence_collector,qualification_scenario_orchestrator,qualification_scenario_process,qualification_lane_runner}_test.rb` | Compressed Patrol host qualification — entry caps apply while enumerating rather than after allocation; target reads are no-follow and descriptor-stable; checkpoint events require the complete canonical semantic envelope; host evidence collection is byte-for-byte non-mutating; process failures after spawn retain only an 8 KiB typed digest projection, reap the owned foreground process, preserve exact orchestrator type, and publish a receipt-less failed-generation row; terminal lane files are prevalidated, staged as one exact private tree, and installed with a no-replace atomic directory rename; injected interruption after every staged write and immediately before install leaves no visible partial lane and retries safely with different evidence. |
