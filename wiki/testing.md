...
| `agent_limit_test.rb` | `Hive::AgentLimit` — provider-limit classifier for Claude usage-credit menus and common quota/rate-limit API errors, the tight live-pane `live_limit_menu?` / `live_limit_line` detector, the narrow AgentLimit wire-format `from_limit?` recognizer, retry-cooldown timestamp helpers, and the shared quota-held display/JSON helpers (`held?`, provider extraction, UTC retry display, label text, and `held` field shape), with runnable false-positive guards for source line numbers, quoted live-menu panes, ordinary "missing rate limit" findings, and UI feature text like scroll/window limits. |
...
| `run_review_test.rb` | `hive run` of `6-review/` — pre-flight states, reviewer/triage/fix/browser branching, no-fix suppression convergence and negative cases, manual wait and stale/error recovery, auto-commit/guardrail boundaries, provider-limit marker behavior for reviewers, triage/CI/fix limit vs non-limit failures, bounded transient triage retry recovery including wall-clock handoff to `REVIEW_STALE`, and `message=` surfacing on terminal phase-agent (triage and fix) errors. |
