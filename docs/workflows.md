...
- `workspace: worktree` opts the final `kind: agent` stage into a
  controller-owned Git worktree. `handoff: draft_pr` composes that workspace
  with controller-owned draft-PR delivery and requires an explicit
  `deliverable:` for validation and PR evidence. Both fields are closed enums,
  are rejected on inert, council, or nonterminal stages, and do not embed an
  agent, model, or effort choice. No other workspace or handoff values are
  supported.
...
Stages that omit `workspace:` and `handoff:` retain task-folder execution and
