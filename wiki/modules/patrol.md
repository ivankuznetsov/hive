...
| `Hive::PatrolFix` | `lib/hive/patrol_fix/` | Source-neutral workflow core for strict source snapshots, durable semantic admission, idempotent task materialization, task manifests, strict Inbox/Fix reports, exact local worktree generations, bounded validation receipts, stable stage-transition journals, and common projection. Candidate-set decisions are digest-bound and revalidated after unlocked provider calls. Materialization intent is durable before task capture; task/evidence binding precedes source acknowledgement; retry repairs interrupted creates and provenance updates. The core depends downward on `AgentGitGate` for closed local Git reads but has no dependency on either Patrol product, Daemon, UI, GitHub authentication, or remote publication. |
