...
`bin/hive` is a thin runner that loads `lib/hive` and calls `Hive::CLI.start(ARGV, debug: true)`, catching `Hive::Error` to render `hive: <message>` to stderr with the error's `exit_code` (default `ExitCodes::GENERIC = 1`). It also catches Thor usage errors (missing positionals/options, unknown options) and maps them to `Hive::InvalidTaskPath` / EX_USAGE (64). For JSON-command argv shapes whose command method is not reached, the entrypoint emits the matching error envelope for `hive run --json`, `hive approve --json`, and `hive markers clear --json`.
