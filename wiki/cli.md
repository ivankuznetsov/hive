...
| `hive daemon SUBCOMMAND` | Auto-advance pipeline dispatcher. Subcommands: `start` / `stop` / `status` / `reload` / `tail` for the daemon process; `install [--force]` to (re)write the platform-native unit file; `enable PROJECT \| --all` and `disable` for per-project enrollment; `queue [list \| show <id> \| prune]` for dispatch-request inspection. Polls status, reconciles current journal terminal evidence to `archive_ready`, and fires only eligible workflow verbs; it does not query GitHub to infer pipeline-task merge state. The dispatcher also runs bounded stale/recoverable-error healers. Daemon autostart is installed globally via `hive daemon install`; new projects opt in at `hive init`, and existing ones via `hive daemon enable`. | `Hive::Commands::Daemon` | [[commands/daemon]] |
| `hive babysit SUBCOMMAND [PROJECT]` | Durable PR watcher/remediator. Subcommands: `start` / `stop` / `restart` / `status` / `reload` / `tail`, plus `--once PROJECT` or `--once --all`. It claims handed-off exact PR jobs before legacy open-PR discovery, fences observations/mutations by task/job/head generation, and records explicit merge evidence without pressing merge. | `Hive::Commands::Babysit` | [[commands/babysit]] |
...
| `hive finalize TARGET [--from STAGE]` | Promote completed artifacts to finalization, or retry/repair the durable exact-PR babysitter handoff | `Hive::Commands::StageAction` → approve/run | [[commands/stage_action]], [[stages/finalize]] |
| `hive finalize-outcome TARGET approve\|rearm --outcome TYPE --reason TEXT` | TTY-only audited exceptional no-PR approval (`abandonment`, `superseded`, `direct_landing`) or append-only re-arm; current evidence is revalidated immediately before exact confirmation | `Hive::Commands::FinalizeOutcome` | [[stages/finalize]] |
| `hive archive [TARGET] [--from STAGE]` | With no target, list every `9-done` task through Status archive mode. With a target, advance only a current reconciler-authored `archive_ready` task and run crash-resumable exact local cleanup; force and marker state cannot bypass eligibility. | no target: `Hive::Commands::Status`; target: `Hive::Commands::StageAction` → approve/run | [[commands/status]], [[commands/stage_action]] |
