...
| `incident_provider_limit_retry` | Opens one account-scoped circuit from trusted transport evidence, proves one charged fallback successor, runs AE2-AE8 through `Attempts::Dispatcher`, durable attempt ownership and receipts, restart reopening, a real `Attempts::Supervisor`, recovery reuse, operator/corruption handling, and the real no-pool `limits_reached` path. |
