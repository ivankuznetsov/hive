...
Signature-verifying history reads are skipped too: `git log`, `git show`, and `git rev-list` reject `--show-signature` plus signature-capable `--format` / `--pretty` values, because those reads can invoke repo-local `gpg.program`. The format/pretty check is **default-deny**, not a literal `%G` scan: a glued `--pretty=<value>` / `--format=<value>` passes only when provably signature-free — a builtin format (`oneline`, `short`, `medium`, `full`, `fuller`, `reference`, `email`, `raw`, `mboxrd`), a fully visible inline format / `format:`/`tformat:` template with no `%G`, or empty. Any other bare value is treated as a repo-local `pretty.<name>` alias from `.git/config` whose expansion can carry a `%G*` placeholder the argv never shows, so opaque `--pretty=<alias>` / `--format=<alias>` names are skipped (a git config key cannot contain `%`, so a `%`-bearing value is always a visible inline format, never an alias). Only the glued `=<value>` selects the format; a separate following word is a revision (`git log --pretty oneline` treats `oneline` as a bad revision), so a bare `--pretty` / `--format` stays allowed.
...
- Signature-verification coverage signs a temp commit with a fake repo-local `gpg.program`, then proves the dry-run git stub skips `--show-signature` and `%G` pretty-format reads before that helper can run.
