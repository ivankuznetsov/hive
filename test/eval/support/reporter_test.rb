...
class HiveEvalReporterTest < Minitest::Test
  ...
  def test_cli_accepts_positional_scenario
    Dir.mktmpdir("hive-eval-report") do |dir|
      report = File.join(dir, "s1-positional.json")
  ...
  def test_cli_rejects_unexpected_extra_arguments
    Dir.mktmpdir("hive-eval-report") do |dir|
      report = File.join(dir, "unexpected.json")

      _out, err, status = Open3.capture3(
        { "HIVE_EVAL_NO_JUDGE" => "1" },
        "bin/hive-eval", "s1_status", "extra", "--no-judge", "--report", report
      )

      assert_equal 64, status.exitstatus
      assert_match(/unexpected arguments: s1_status extra/, err)
      refute File.exist?(report)
    end
  end
  ...
  def test_cli_rejects_positional_and_scenario_flag_conflict
    Dir.mktmpdir("hive-eval-report") do |dir|
      ...
      assert_equal 64, status.exitstatus
      assert_match(/either a positional .* or --scenario, not both/, err)
      refute File.exist?(report)
    end
  end
end
