...
| `thor_usage_json_test.rb` | `Hive::ThorUsageJson` — direct unit coverage for every parse-error routing-table entry, schema-registration guard, flag-order-independent command/`--json` detection, schema-less unknown-command fallback, EPIPE swallow, and JSON-generator warning branch. |
| `patrol/pr_opener_test.rb`, `patrol/review_handoff_test.rb` | `Hive::Patrol::PrOpener` / `ReviewHandoff` — PR creation, fingerprint mapping, optional synthetic `6-review` tasks, worktree pointer contents, `patrol.review_prs: false` cleanup behavior, `idea.md` seeding from the original finding, and sparse finding rendering (nil title fallback, nil evidence, omitted empty sections, location-only evidence, symbol-keyed evidence, exact body render). |
| `commands/status_test.rb`, `archive_filter_test.rb`, `tui/schema_correspondence_test.rb`, `tui/snapshot_test.rb`, `tui/views/archive_pane_test.rb` | Status/TUI archive boundary — required `hive-status` task keys match `Status#task_payload`, `Snapshot::Row` has a field for every emitted task key, `folder_mtime` is preserved, old archives are hidden from daily text/grid views by row `mtime` when present and `folder_mtime` as fallback regardless of marker state, no-target `hive archive` filters to `9-done`, and explicit archive views remain age-unfiltered. |
| `tui/views/format_test.rb`, `tui/views/tasks_pane_test.rb` | TUI terminal-cell formatting — direct `Unicode::DisplayWidth` helper tests plus render-level assertions keep wide CJK/emoji graphemes from shifting downstream columns or overflowing fixed cell budgets. |
...
| `cli_parse_error_test.rb` | `bin/hive` executable parse-error boundary — every parse-error-raising mapped command under `--json` emits a schema-valid usage envelope before command construction, stage-action aliases carry the canonical `verb` (including `pr` → `open-pr`), unknown-flag Thor paths for bare `status`/`prune` validate against their schemas, `daemon` unknown-subcommand stays on the command-local stderr path, unknown JSON commands emit an unversioned `usage` envelope, and unknown text commands exit 64 with Thor's text error. |
