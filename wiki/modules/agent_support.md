...
- The Pi root owns its message protocol, native model rules, credential
  location/token schema, capture interface, and plan-review exceptions. Its
  capture facet gives non-Hive visual evidence a controller-owned
  `evidence_server` capability only when a project server is required; the
  generic toolkit remains the mailbox, sandbox, and lifecycle owner.
