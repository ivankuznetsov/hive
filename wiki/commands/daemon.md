...
discovery/action resumes. Before constructing those Architecture Patrol
surfaces, startup runs the shipped v2-to-v3 JobStore converter across every
project in this daemon user's exact config/state profile and holds only failed
rows. Machine-wide coverage is owned by the separate privileged all-user
coordinator; AUR's system timer retries inactive profiles hourly. Generated
daemon units persist only the exact HIVE_HOME/XDG roots needed to keep custom
profile retry and runtime resolution stable across login/reboot.
