...
| `HIVE_QMD_NPM_PACKAGE` | Override the pinned QMD version; must be an exact `@tobilu/qmd@X.Y.Z` spec and defaults to `@tobilu/qmd@2.5.3`. Arbitrary packages, URLs, tags, and ranges fail before npm runs. |
| `HIVE_QMD_NPM_INTEGRITY` | Required `sha512-...` integrity when overriding the default QMD version. The default version carries a checked-in integrity pin. |
...
sha512 digest against Hive's expected value, installs it into a sibling staging
tree, and opens an in-memory SQLite database before activation. A download,
install, `npm rebuild better-sqlite3`, startup, native-health, timeout, or
activation failure leaves the previous managed tree and link intact.
