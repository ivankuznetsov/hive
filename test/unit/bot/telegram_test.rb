...
class HiveBotTelegramTest < Minitest::Test
  ...
  def test_markdown_v2_chunks_do_not_split_inside_entities_or_escapes
    # The only newline before Telegram's limit is inside the bold entity.
    # The old newline-aware splitter cut there, leaving chunk 1 with an open
    # entity and chunk 2 with its unmatched closing marker.
    bold = "*#{'b' * 3_900}\n#{'c' * 140}*"
    suffix = " tail #{'z' * 120} _italic_ `code\\`tail` " \
             "[link](https://example.com/a\\)b) escaped\\_text"
    text = "#{bold}#{suffix}"

    chunks = telegram(FakeApi.new).message_chunks(text, parse_mode: :markdown_v2)

    assert_equal 2, chunks.size
    assert chunks.all? { |chunk| chunk.length <= Hive::Bot::Telegram::MAX_MESSAGE_CHARS }
    assert_equal text, chunks.join
    assert_match(/\A\*.*\*/m, chunks.first)
    assert_includes chunks.last, "_italic_"
    assert_includes chunks.last, "`code\\`tail`"
    assert_includes chunks.last, "[link](https://example.com/a\\)b)"
    assert_includes chunks.last, "escaped\\_text"
    assert_equal 2, chunks.first.scan(/(?<!\\)\*/).size
    assert_equal 0, chunks.last.scan(/(?<!\\)\*/).size
  end

  def test_send_message_uses_markdown_aware_split_when_parse_mode_is_markdown_v2
    api = FakeApi.new
    text = "*#{'b' * 3_900}\n#{'c' * 140}* #{'z' * 120}"

    telegram(api).send_message(chat_id: 12345, text: text, parse_mode: :markdown_v2)

    sends = api.calls.select { |kind, _| kind == :send_message }
    assert_equal 2, sends.size
    assert sends.all? { |_, params| params[:parse_mode] == "MarkdownV2" }
    assert_equal text, sends.map { |_, params| params.fetch(:text) }.join
    assert_equal 2, sends.first.last.fetch(:text).scan(/(?<!\\)\*/).size
    assert_equal 0, sends.last.last.fetch(:text).scan(/(?<!\\)\*/).size
  end

  def test_markdown_v2_to_html_preserves_supported_entities_and_escapes
    source = "*bold* _italic_ __underline__ ~strike~ ||spoiler|| " \
             "`code\\`tail` [link](https://example.com/a\\)b) escaped\\_text"

    html = telegram(FakeApi.new).markdown_v2_to_html(source)

    assert_equal(
      "<b>bold</b> <i>italic</i> <u>underline</u> <s>strike</s> " \
      "<tg-spoiler>spoiler</tg-spoiler> <code>code`tail</code> " \
      "<a href=\"https://example.com/a)b\">link</a> escaped_text",
      html
    )
  end

  def test_markdown_v2_pre_custom_emoji_and_blockquote_paths_are_validated
    bot = telegram(FakeApi.new)
    source = "```\ncode\\`tick\n```\n![🙂](tg://emoji?id=1)\n>quote"

    chunks = bot.message_chunks(source, parse_mode: :markdown_v2)
    html = bot.markdown_v2_to_html(source)

    assert_equal [ source ], chunks
    assert_includes html, "<pre>\ncode`tick\n</pre>"
    assert_includes html, "<a href=\"tg://emoji?id=1\">🙂</a>"
    assert_includes html, "&gt;quote"
  end

  def test_markdown_v2_rejects_unbalanced_or_unescaped_entities_before_send
    bot = telegram(FakeApi.new)
    invalid = [
      "_unclosed",
      "dangling\\",
    ...
    invalid.each do |source|
      assert_raises(Hive::Bot::Telegram::MarkdownV2SplitError, source) do
        bot.message_chunks(source, parse_mode: :markdown_v2)
      end
    end
  end

  def test_markdown_v2_rejects_an_entity_larger_than_one_telegram_message
    source = "*#{'x' * (Hive::Bot::Telegram::MAX_MESSAGE_CHARS + 1)}*"

    error = assert_raises(Hive::Bot::Telegram::MarkdownV2SplitError) do
      telegram(FakeApi.new).message_chunks(source, parse_mode: :markdown_v2)
    end

    assert_match(/entity exceeds Telegram/, error.message)
  end
