...
23. **Archive hiding/listing is unit-pinned but not live-smoked.** Commits `6d10e386` through `bfa4b590` add required `tasks[].folder_mtime`, the shared `Hive::ArchiveFilter`, default text/TUI hiding for clean `9-done` rows older than 3 days, an unfiltered TUI Archive pane, and no-arg `hive archive` listing. Focused unit/integration tests cover the policy, status text/JSON boundaries, TUI projection/cursor behavior, and CLI routing. This refresh did not create an in-tree live artifact from a real registered project showing the full operator workflow (`hive status`, `hive tui` -> `z`, `hive archive --json`) after aged done folders exist.
