...
`recover_review` rows show the observed recovery cause in the status column (for example `triage_failed` from `<!-- REVIEW_ERROR phase=triage reason=triage_failed pass=2 -->`) instead of the generic `Needs recovery` label. Pressing `Enter` sequences the documented CLI recovery flow: first `hive markers clear <folder> --name REVIEW_ERROR|REVIEW_STALE|REVIEW_CI_STALE`, guarded with one observed `--match-attr` when available, then `hive run <folder>` in the normal background-spawn path. If the marker clear fails because a concurrent writer changed the marker, the rerun is skipped and the captured stderr is flashed.
