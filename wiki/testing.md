...
| `commands/daemon_test.rb` | `Hive::Commands::Daemon` and `Hive::Daemon::StatusReport` — lifecycle command routing, PID-file ownership/status handling, detached start/re-exec behavior, service install/enable/disable output, daemon-status service/binary fields, `safe_payload` web degradation, `binary_drift` classification including `unreadable`, queue inspection, and start-path wiring of daemon/update/digest config into the dispatcher. Start tests stub all three global config blocks so operator-local Telegram digest defaults cannot change unit expectations. |
...
| `daemon/retry_coordinator_test.rb`, `daemon/retry_record_test.rb` | Durable error-agnostic state machine — schedule escalation and indefinite final value, duplicate idempotency, absolute restart deadlines, attempt/generation fencing, same-stage success retention, reset boundaries, audited abandon/re-arm, and byte-equivalent journal replay. |
| `daemon/stale_agent_healer_test.rb`, `daemon/recoverable_error_healer_test.rb`, `daemon/auto_retry_safety_test.rb` | Legacy helper demotion — dead PID/grace/terminal observations emit one canonical report and never own marker clearing, counters, delay computation, provider probes, or dispatch. |
| `daemon/retry_authority_contract_test.rb`, `terminal_error_registry_test.rb`, `diagnostic_evidence_test.rb` | Architectural/evidence boundary — coordinator-only retry journal writes and authorization creation, retired authority names rejected, complete canonical registry inventory, coordinator-only routes, and bounded secret-redacted evidence. |
...
| `daemon_stale_agent_healing_test.rb` | Status-to-observer integration — real `hive status --json` rows feed `Hive::Daemon::StaleAgentHealer`, pinning stale `AGENT_WORKING` classification, one-shot failure reporting, live-lock isolation, and `daemon.agent_marker_grace_sec` as observation grace only. |
| `retry_error_family_matrix_test.rb`, `daemon_retry_coordinator_test.rb`, `retry_surface_consistency_test.rb` | All terminal families share one ladder; restart/capacity/current-environment behavior is durable; CLI/TUI/web/bot copy the same optional status-v4 retry object. |
