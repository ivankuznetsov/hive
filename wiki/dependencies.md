...
| Gem | Version | Purpose |
|-----|---------|---------|
| `fiddle` | `>= 1.1` (locked 1.1.8) | Development/test-only FFI used by the independent outer Linux OpenClaw creator containment root to call `prctl(PR_SET_CHILD_SUBREAPER)` and verify it with `PR_GET_CHILD_SUBREAPER` before it launches the expendable owner and fault-injected worker. Declared because Fiddle stops being a default gem in Ruby 4; it is not part of the shipped `hive-cli` runtime. |
