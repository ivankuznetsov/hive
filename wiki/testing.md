...
| `user_service/user_service_test.rb`, `commands/{bot,daemon,web,babysit}/service_installer_test.rb`, `commands/service_installer/base_test.rb`, `commands/uninstall_test.rb` | `Hive::UserService` and thin Hive adapters — tri-state manager inspection, real-home canonical-target ownership, durable directional replay, stale-holder identity proof, exclusive backups, applied receipts, exact observation revalidation, drift refusal, fail-closed evidence, verified removal, and compatible systemd/launchd outcomes without accepting desired bytes as a terminal partial state. |
