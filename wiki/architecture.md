...
- **Decoder reset on cancel** — `PasteAwareRunner` watches for transitions out of `:new_idea` / `:filter` and calls `InputDecoder#reset!` so an orphan paste held mid-prompt cannot dump into the next prompt.
