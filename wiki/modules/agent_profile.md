...
| `check_version!` | Delegates the bounded 30-second process-group version probe to the package and caches its result per `(bin, min_version)` pair. The deadline tolerates CLI startup on a loaded parallel runner while remaining finite. Raises `Hive::AgentError` on missing/un-runnable binary, parse failure, timeout, or version below minimum. |
