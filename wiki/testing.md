...
| `cli_usage_error_json_test.rb` | `bin/hive` pre-dispatch usage errors under `--json` — wrapper mappings for missing required targets (`run`, `approve`, `markers`, `drop`, `findings`, finding toggles), workflow verb `hive-stage-action` envelopes (`brainstorm`, plus `pr` normalized to `open-pr`), `patrol` / `status` Thor usage failures, and `prune` usage errors before command object dispatch. |
