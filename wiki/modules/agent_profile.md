...
For implementation identity, Claude translates normalized values to `--model <model> --effort <effort>` and Codex to `--model <model> -c model_reasoning_effort=<effort>`. Pi and Grok can resolve and optionally pin a concrete provider-native model but declare effort unsupported. The profile-level identity value can still record unsupported requested effort for durable identity compatibility; a new explicit runtime invocation request fails closed instead of silently omitting that requested capability.
