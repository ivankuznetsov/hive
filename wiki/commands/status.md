...
The public JSON/text callers use the full stage union above. `Status#json_payload` also has an additive internal seam for the TUI: optional `stages:` restricts `collect_rows` to a subset, and optional `extra_dependency_tasks:` supplies identity-only rows (keyed by project path) for dependency resolution. With no kwargs, the output stays the normal full snapshot; the TUI uses the seam to parse active stages while serving `9-done` rows from its archive cache. Archived identities are folded into dependency resolution so an active row that depends on a completed archived prerequisite does not become falsely unresolved.
