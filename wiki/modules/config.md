...
10. **`validate_daemon!`** — daemon numeric bounds, booleans, and nested hashes are checked before the daemon starts. Branch `make-the-hive-daemon-automatically-260629-223d` adds a nested `daemon.auto_retry` shape check and requires `daemon.auto_retry.enabled` to be boolean when present.
