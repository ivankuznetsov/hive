...
| `hive setup [--json] [--no-service] [--no-bootstrap] [--no-init]` | Normal native provisioning: diagnostics, authenticated QMD/web-bundle bootstrap, daemon service install, optional project enrollment, and default managed loopback web-service install/readiness. `--no-service` opts out of web mutation; `--no-bootstrap` is diagnose-only. JSON is `hive-setup.v1`. | `Hive::Commands::Setup` | [[commands/setup]] |
