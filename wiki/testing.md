...
| `babysitter/dry_run_env_test.rb` (config lifecycle regression) | A successful allowlisted `gh` passthrough records the hermetic `HOME`/`GH_CONFIG_DIR` selected by `DryRunEnv.with_env` and asserts that directory no longer exists after the block returns. |
