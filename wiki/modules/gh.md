...
| `digest_merged_pr_candidates(repository:, host:, window_start:, cfg:)` | Exhaustively paginate closed PR REST rows by descending `updated_at`, validate monotonic pages, require consecutive identical complete snapshots so live page shifts fail closed, detect traversal caps, and return complete candidates for London-window filtering without the Search API's 1,000-row cap. |
| `digest_repository_metadata` / `digest_pr_detail` / `digest_pr_diff_to_file` / `digest_pr_files` | Fetch explicit-host repository metadata plus canonical PR body/identity/optional metrics, stream the raw GitHub diff directly to a private byte-limited file, and fetch complete paginated file identities. These fail loudly so [[modules/digest]] can make required evidence repository-atomic. |
