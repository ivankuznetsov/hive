...
For other `REVIEW_STALE` rows (max_passes hit with completed passes), Enter now routes to a browse-only handler — `BubbleModel#open_review_stale_file` — that opens the highest-pass `reviews/escalations-NN.md` (derived from `marker.attrs["pass"]`) in `$VISUAL` / `$EDITOR` / `vi` via the same `foreground_takeover_command` machinery `o` and `Enter`-on-`needs_input` use. The marker is NOT cleared and no `hive run` dispatch fires — clearing remains a deliberate `hive markers clear` round-trip (or a future TUI gesture) so the operator owns the resolve decision after reading the unresolved findings. If `escalations-NN.md` is missing for the recorded pass, the resolver falls back to the `reviews/` directory; if neither exists, the handler flashes `no review files for <slug>` and refuses.
