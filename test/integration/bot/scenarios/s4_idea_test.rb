...
class HiveBotScenarioIdeaTest < Minitest::Test
  def test_s4_idea_project_picker_collects_then_done_commits
    projects = [
    ...
    assert_equal :reply, collect.action
    assert_match(/Send any files now/, collect.text)
    done_button = collect.reply_markup.flatten.detect { |button| button[:text] == "Done" }
    refute_nil done_button, "collector keyboard should expose Done"

    result = router.handle(update(callback_data: done_button[:callback_data]))
