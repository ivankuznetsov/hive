...
| `completion_time_test.rb`, `completed_at_backfiller_test.rb`, `commands/{approve,run}_test.rb` | Completion-clock durability — inert versus active terminal stamping, first-completion immutability, metadata/move/commit/interrupt rollback, earliest credible workflow-aware Git event before state-file/folder mtime fallbacks, bounded lock-aware legacy backfill, concurrency, and fail-open warnings when discovery or persistence cannot provide a durable clock. |
...
| `test/unit/web/status_feed_test.rb`, `web/test/channels/status_channel_test.rb`, `web/test/models/{project,board,status_broadcaster}_test.rb`, `web/test/integration/{status,tasks}_test.rb`, `web/test/system/{kanban_board,pipeline_flow}_test.rb` | Hive web status/archive views and broadcasting — descriptor-ordered board bands, ordinary producer filtering and linked hidden summaries, separate lossless archive snapshots that do not replace the ordinary feed baseline, project-scoped archive navigation, expired-task detail lookup through explicit archive source, hidden-count-only semantic changes, unknown-stage/degraded-project visibility, Board/Grid preferences and filtering, one shared scan per poll tick, first-request priming, canonical semantic tokens, confirmed-channel catch-up, broadcast lifecycle/error recovery, mutation survival, and real-browser Board-to-Archive-to-task navigation. |
