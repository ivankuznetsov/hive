...
Claude/tmux fix spawns have a narrow completion fallback for the known interactive Stop-hook failure shape: the launcher may return `status: :timeout` with `error_message: "claude stop hook did not signal completion"` even though the turn visibly ended. `handle_fix_completion_fallback` suppresses that terminal error only when `Hive::ClaudeCompletionFallback` accepts launcher evidence and review-phase facts together: the tmux session was still alive, the fix artifacts exist, unresolved escalations are zero, the worktree was readable, the error is not a missing-output failure, and code-change evidence exists. Code-change evidence is either a new fix commit, a dirty worktree that Hive's post-fix auto-commit will land, or a whole-pass no-change disposition where every finding was checked `RESOLVED/NO-FIX:` with no unapplied `[x] AUTO-FIX:` line remaining. When the fallback fires it emits `claude_completion_fallback` with the concrete evidence (`commit:<before>-><after>`, `dirty_worktree`, or `whole_pass_no_change`) and then falls through to the normal post-fix path; it does **not** write `fix-success-NN.md` early, so the fix guardrail can still pause the pass before the success sentinel is written. If the audit event cannot be recorded, suppression is refused and the task stays on the recoverable `REVIEW_ERROR`.
