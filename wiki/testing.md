...
| `workflow_package/*_test.rb`, `commands/workflow_{install,list,remove,update,publish}_test.rb`, `workflow_lifecycle_schema_test.rb` | Honeycomb package canonicalization/security, exact/wildcard domain and shell/path-hook enforcement, registry ancestry/ref plus catalog/manifest binding, immutable store transaction/task-pin concurrency, protected policy material, headless/tmux environment parity, lifecycle preview/consent/ownership, closed JSON envelopes, deterministic packaging, and injected fork/PR seams. |
...
| `honeycomb_workflow_lifecycle_test.rb` | A local immutable git registry fixture drives real catalog/source ancestry through install, no-write dry-run, update, old-task generation pinning, list selected/retained dimensions, remove, and offline execution of the retained task. |
