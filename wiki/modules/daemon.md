...
| `Hive::Daemon::PatrolFixOperationalProjection` | `lib/hive/daemon/patrol_fix_operational_projection.rb` | Sole source-composition edge for the bounded project-level Patrol Fix projection. It reads ordinary and Architecture discovery, admission, task workflow, migration, successor, publication, post-merge, and existing usage telemetry authorities once per daemon tick. Source failures are isolated to an unavailable section plus a redacted diagnostic; token counts are observation only and never feed scheduling. |
