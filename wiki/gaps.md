...
4. **OpenClaw skill bundle ships in-tree; ClawHub publication is still external.** OpenClaw skill files now live in `openclaw/skills/` with an umbrella `/hive` skill plus shortcuts (`/plan`, `/work`, `/ce-review`, etc.) — local install from the checkout works today. ClawHub publication and marketplace listing evidence remain external maintainer steps.
