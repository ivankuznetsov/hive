...
| `hv_test.rb` | `bin/hv` — refuses unsafe Apache Hive fallback paths (`/usr/bin/hive`, `/opt/hive/bin/hive`), skips Apache-style Homebrew fallback binaries whose `--version` output is not bare semver, and verifies `HIVE_BIN_OVERRIDE` can point at a custom Hive CLI install path. |
