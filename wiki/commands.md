...
want the same first-run shape as hivebox without Docker: it first previews and
provisions the bundled Hive operating skill for supported agents, then emits a
`hive-setup` phase report, installs/repairs Hive-owned QMD and managed web
bundle assets, installs the daemon, enrolls the project unless disabled, and
can install the web service. One interactive confirmation covers the run;
JSON/non-TTY requires `--yes` or performs no mutation. See
[[commands/setup]].
...
the skill does not use unattended Arch install flags.
