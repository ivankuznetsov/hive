...
class BabysitterDryRunEnvTest < Minitest::Test
  ...
  def test_stubs_skip_unknown_and_mutating_commands_but_allow_read_only_commands
    with_tmp_dir do |dir|
      ...
      assert_stubbed env, "git", "-p", "log"
      assert_stubbed env, "git", "--paginate", "show"
      ...
      safe_git = "real-git --no-pager -c core.fsmonitor=false -c diff.external="
      ...
      assert_includes real_invocations, "#{safe_git} -C #{dir} status --short"
      assert_includes real_invocations, "#{safe_git} diff --no-ext-diff --no-textconv --name-only"
      assert_includes real_invocations, "#{safe_git} grep needle"
      assert_includes real_invocations, "#{safe_git} config --get remote.origin.url"
      assert_includes real_invocations, "#{safe_git} remote"
      assert_includes real_invocations, "#{safe_git} remote -v"
      assert_includes real_invocations, "#{safe_git} remote show -n origin"
      assert_includes real_invocations, "#{safe_git} remote get-url --push origin"
    end
  end

  def test_git_passthrough_ignores_repo_local_external_diff
    with_tmp_git_repo do |repo|
      ...
      run!("git", "-C", repo, "config", "diff.external", "#{external_diff} #{marker}")
