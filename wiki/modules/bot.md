...
| `ConversationStore` | `lib/hive/bot/conversation_store.rb` | In-memory per-chat active answer/Codex state with TTL. No sidecar file; `brainstorm.md` stays canonical. `active_for_slug?` is prune-aware and intentionally checks any chat, so the notification dispatcher can suppress a proactive `needs_input` push while an operator is already answering that slug. |
