...
4. **OpenClaw skill bundle ships in-tree; ClawHub publication is still external.** OpenClaw skill files now live in `openclaw/skills/` with an umbrella `/hive` skill plus shortcuts (`/plan`, `/work`, `/ce-review`, etc.) — local install from the checkout works today. Collision-prone shortcuts use prefixed names such as `/hive-new`, `/hive-approve`, and `/hive-status` because OpenClaw owns `/new`, `/approve`, and `/status`. ClawHub publication and marketplace listing evidence remain external maintainer steps.
