...
| `hive retry ACTION TARGET --generation N [--project NAME] [--actor WHO --reason WHY] [--json]` | Route task retry intent through the durable coordinator. `manual` is a cooldown-respecting wakeup. `repair`/`reset`, `abandon`, and `rearm`/`re-arm` are audited generation-guarded actions; non-manual actions require actor and reason. | `Hive::Commands::Retry` | [[commands/retry]] |
...
| `hive daemon SUBCOMMAND` | Auto-advance dispatcher and durable retry evaluator. Lifecycle/install/enrollment/queue subcommands are unchanged. Each tick reconciles attempts, reports terminal evidence, evaluates journal-backed absolute retry deadlines, and admits a fenced successor through normal capacity. Stale-marker helpers observe liveness only; no marker-clear budget, quarantine, or provider probe controls delayed task dispatch. | `Hive::Commands::Daemon` | [[commands/daemon]] |
...
| `hive markers clear FOLDER --name <NAME> [--match-attr KEY=VALUE] [--project NAME] [--json]` | Low-level marker repair: removes one recovery marker with an atomic write + hive commit, and rejects terminal-success markers. It does not reset the durable retry ladder or authorize redispatch; normal recovery uses the generation-guarded `hive retry` actions. | `Hive::Commands::Markers` | [[commands/markers]] |
