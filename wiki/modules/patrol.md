...
| `Hive::Patrol::FeatureBatch` | `lib/hive/patrol/feature_batch.rb` | Selects the deterministic SHA-bound rotating component batch and returns the next persistent cursor. `Commands::Patrol` records an explicit active snapshot, so it finishes the pinned SHA even when default advances or the first batch errors at cursor zero. |
...
| `Hive::Patrol::TokenBudget` | `lib/hive/patrol/token_budget.rb` | Shares measured-token, agent-launch, and per-agent dollar caps across ordinary review/fix phases and architecture discovery/action phases for one command cycle. It checks the durable current-day project total before every default spawn and records missing usage as an unmetered launch. |
...
Operators normally configure scheduling through `patrol.mode`, which [[modules/config]] resolves into cadence plus token, launch, and per-agent dollar ceilings before the daemon sees the project config. `ultrapatrol`, `high`, `medium`, and `low` deliberately receive progressively smaller spend envelopes as cadence falls; `off` resolves to `enabled: false`. Measured input, output, and cached tokens from both ordinary and architecture stages share the same project/day total. Unmetered children still consume launch quota, so a provider that cannot report token totals cannot bypass the tier. Explicit granular knobs always win over a set mode and survive the deep-merge even when no `mode:` is set.
