...
- `test/unit/gh_test.rb` covers frontmatter parsing, secret-scan fetch-failure semantics, open/merged PR lookup contracts, `pr_state` success/error parsing, branch `make-the-hive-daemon-automatically-260629-223d` `pr_metadata` parsing/error handling, `PushResult`, subprocess timeout/termination, `list_open_prs` JSON fields, failing-job log clipping, and malformed JSON failures.
