...
## ADR-032: Install channels ship the `hive-cli` gem, not Tebako binaries; one renderer, signature-gated publish

**Status:** Active (supersedes ADR-027's Tebako artifact model for the channel layer)
**Context:** ADR-027 proposed Tebako-packed self-contained binaries (`hive-<version>-<target>.tar.gz`) as the release artifact for all install channels. In practice the release converged on a single `hive-cli` rubygem attached to a signed GitHub Release; all three channels (install.sh, Homebrew, AUR) consume that one gem. Earlier hand-maintained channel metadata drifted — a stale `packaging/aur/.SRCINFO.template` still referenced tebako-era tarballs (dropped in U2).
**Decision:** Release artifact is the `hive-cli` gem + cosign-keyless-signed `SHA256SUMS{,.sig,.pem}` on the GitHub Release. A `vX.Y.Z` tag triggers `.github/workflows/release.yml`: build/smoke the gem → sign → release → `repository_dispatch` to the Homebrew tap (gated on `HOMEBREW_TAP_TOKEN`) → `aur-publish` job (gated on `AUR_SSH_PRIVATE_KEY`) that cosign-verifies the gem before rendering/pushing. Both channel templates render through one helper, `packaging/render.rb` (fail-closed ERB: an undefined template variable exits non-zero and writes nothing — no half-rendered formula/PKGBUILD ever publishes). AUR is a `-bin` package: `package()` runs `gem install` (networked build; `makechrootpkg` clean-chroot will fail — deferred "vendor deps into gem" follow-up), `arch=('any')`, `provides/conflicts=('hive')` to claim the virtual name and block `apache-hive`. `.SRCINFO` is generated via `makepkg --printsrcinfo`, never templated.
**Consequences:** Channels share one artifact (the gem) and one checksum file; the cosign-verify step in `aur-publish` is the supply-chain gate (do not loosen `--certificate-identity-regexp`). Both publish paths are idempotent, so tag re-runs are safe; missing channel secrets skip-with-audit-line rather than fail the release. The git tag itself is still unsigned — tag-protection ruleset on `v*` is the compensating control (ADR-027's known gap carries forward); `git tag --verify` remains deferred hardening. One-time human setup (tag rules, `HOMEBREW_TAP_TOKEN`, AUR account/SSH key, first manual `hive-bin` bootstrap) is documented in `docs/RELEASING.md`. `aarch64` AUR installs may source-compile native deps and are not yet validated — do not advertise AUR as aarch64-ready until tested.
...
## ADR-033: Release artifacts are the `hive-cli` rubygem; brew + AUR publish from CI

**Status:** Active (supersedes ADR-027)
...
- **AUR:** a separate `aur-publish` job runs in an `archlinux:base-devel` container (makepkg needs Arch), gated on a `release-finalize` output (a job-level `if:` cannot read `secrets.*`). It cosign-verifies the gem **with the signing identity pinned** (`--certificate-identity-regexp` + `--certificate-oidc-issuer`, mirroring install.sh), renders `PKGBUILD` via `packaging/render.rb`, regenerates `.SRCINFO` with `makepkg --printsrcinfo` (never hand-maintained — that was the drift bug), and pushes a version bump to `hive-bin.git`.
- **Single render path:** `packaging/render.rb` renders both the formula and the PKGBUILD, so a release's version/sha is substituted identically everywhere. There is intentionally no `.SRCINFO.template`.
**Consequences:** Both channels publish only after the cosign identity check passes (fail-closed). The AUR `package()` fetches dependency gems from rubygems.org at build time (works for `yay`/`paru`; not for network-isolated clean chroots) and is `arch=any` despite platform-native deps (rubygems resolves the right binary per host; `aarch64` must be validated before advertising). Two secrets are required and human-provisioned: `HOMEBREW_TAP_TOKEN` (fine-grained PAT, `homebrew-hive`-only, Contents: write — the minimum the dispatch API allows) and `AUR_SSH_PRIVATE_KEY`. When a secret is absent the matching publish path self-skips with an audit line; the release still succeeds. Setup + steady-state release flow are documented in `docs/RELEASING.md`.
**Known gap — release-tag trust (carried from ADR-027, now higher-stakes):** the workflow cosign-signs whatever a `vX.Y.Z` tag points at, and that artifact now auto-propagates to Homebrew **and** AUR users before any human review. Compensating control: a GitHub tag-protection ruleset restricting who can push `v*` tags (per `docs/RELEASING.md`). Signed git tags (`git tag --verify` against a maintainer key) remain the real fix and are still deferred.

## ADR-027: Release artifacts are Tebako-packed binaries

**Status:** Superseded by ADR-033 — tebako was dropped during v0.1.0; releases ship the `hive-cli` rubygem. Retained for history.
