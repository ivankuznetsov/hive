...
title: CLI Surface
type: api
source: bin/hive, bin/hive-e2e, bin/hv, lib/hive/cli.rb, lib/hive/runtime_identity.rb, lib/hive/cli_argv_policy.rb, lib/hive/commands/answer.rb, lib/hive/commands/task.rb, schemas/hive-answer.v1.json, schemas/hive-task-workspace.v2.json, schemas/hive-version.v1.json
created: 2026-04-25
updated: 2026-08-25
...
| `hive archive [TARGET] [--from STAGE]` | With no target, list every `9-done` task across registered projects through Status archive mode (`--json` returns a `hive-status` payload filtered to done rows). With a target, promote a completed finalized PR to done, re-run done, or use the explicit evidence-bound `--reason already_delivered|superseded` operator flow, or `--reason cancelled --attestation "reason"` to retain unwanted work without delivery evidence or terminal execution. Automatic task-bound merge closure uses an internal receipt API, not a CLI bypass flag. | no target: `Hive::Commands::Status`; target: `Hive::Commands::StageAction` / `Hive::TaskClosure` | [[commands/status]], [[commands/stage_action]] |
...
- `--json` is a `class_option` honoured by `version`, `init`, `status`, `answer`, `act`, `doctor`, `setup-agents`, `run`, `rebase-status`, `approve`, `plan-review`, `drop`, `findings`, `accept-finding`, `reject-finding`, the workflow verbs (`brainstorm`, `plan`, `develop`, `open-pr`, `review`, `artifacts`, `finalize`, `archive`), `markers clear`, `metrics`, `forget`, `prune`, `workflow new`, `connect screenote`, `disconnect screenote`, `bench submit`, `pairing`, the `daemon` subcommands (`status`, `stop`, `reload`, `install`, `enable`, `disable`, `queue`), and the `bot` lifecycle subcommands (`status`, `stop`, `reload`). `watch` rejects that single-document mode and uses `--json-lines`. Version emits `hive-version.v1`; Plan-review actions emit `hive-plan-review-action.v1`; Answer emits `hive-answer.v1`; Doctor emits `hive-doctor.v2`; setup emits `hive-setup.v1`; setup-agents emits `hive-setup-agents.v1`, never prompts in JSON mode, and requires `--yes` for mutation. Bare status selects `hive-running-status.v2`, while `--operational` selects `hive-operational-status.v4`. Superseded running-status v1, operational-status v1-v3, and act v1 contracts are removed after coordinated migration. Daemon JSON is published as `hive-daemon-status.v1` / `-stop.v1` / `-reload.v1` / `-install.v1` / `-enroll.v1` / `-queue.v1`. Bot lifecycle JSON is published as `hive-bot-status.v1` / `-stop.v1` / `-reload.v1`; pairing JSON is published as `hive-pairing-list.v1` and `hive-pairing-approve.v1`. `hive babysit` is bare-text in v1. Each command with full envelope support emits a typed JSON document on success and a structured error envelope on failure. Workflow verbs emit a single `hive-stage-action` envelope. `init` emits `hive-init.v2` on success (v1 remains loadable for pinned consumers); `drop` emits `hive-drop.v2`; `rebase-status` emits `hive-rebase-status`; and Screenote/workflow/bench keep their existing contracts.
