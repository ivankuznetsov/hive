...
| File | Covers |
|------|--------|
| `init_test.rb`, `llm_wiki_scheduler_test.rb` | `hive init` plus Linux LLM-wiki scheduling — stale `main_wiki_path` removal and rediscovery, activation-relative timers with randomized starts, one repository timer across linked worktrees, shared global lock, executable shared-runner guard, cgroup memory limits, marked and exact E2E debris cleanup, service-only partial cleanup, ambiguous-unit and disabled-schedule preservation, timer-only cleanup, non-fatal init activation, and durable stop/reload retry state. |
