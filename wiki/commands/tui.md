...
- Every `ERROR` / `REVIEW_ERROR`, including signal exits `130` / `137` / `143`, `fix_status_check_failed`, and `ensure_clean_on_exit_failed`, offers the same generation-guarded retry. The daemon may temporarily defer the retry while the current work area is dirty or awaiting operator input; after files, configuration, or agent availability changes, the next eligible retry revalidates the task from current state. `EXECUTE_STALE` remains manual because it represents unresolved findings rather than a failed attempt.
