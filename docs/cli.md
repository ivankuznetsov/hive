...
| `hive refactor-patrol-reset PROJECT --confirm [--json]` | Explicitly archive only that registered project's opaque released `v2/jobs` directory and start an empty v3 JobStore. It never reads/imports v2 records, never scans other users, and refuses non-empty v3 state. |
