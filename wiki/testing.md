...
| `test/unit/web/{web_command,app_bundle,environment,service_status}_test.rb`, `test/integration/web_packaged_bootstrap_test.rb`, `web/test/integration/production_host_authorization_test.rb` | Hive web app-dir resolution (canonical input plus warned alias), exact six-variable precedence/warnings, launchd lifecycle/readiness polling, readiness-failing install and versioned status-error contracts, release-workflow-and-tag-pinned bundle authentication, real packaged-root Bundler/Rails bootstrap without a parent checkout, behavioral production Host authorization, `db:prepare` typed failure guidance, final Rails `Kernel.exec` env/argv, and loopback/public-bind policy. |
