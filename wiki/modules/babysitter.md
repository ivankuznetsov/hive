...
| `Hive::Babysitter::DryRunEnv` | `lib/hive/babysitter/dry_run_env.rb` + `bin/hive-babysitter-stub-git` / `bin/hive-babysitter-stub-gh` | PATH overlay for agent-side dry-run `git` / `gh` stubs; allowed git reads exec with hermetic user/system/local config controls, while skipped-command audit writes refuse symlinked/non-owned/non-regular log targets and escape control bytes without decoding argv as UTF-8. |
