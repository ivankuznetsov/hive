...
title: CLI Surface
type: api
source: bin/hive, bin/hv, lib/hive/cli.rb, lib/hive/thor_usage_json.rb
created: 2026-04-25
updated: 2026-06-06
tags: [cli, api]
---
...
`bin/hive` is a thin runner that loads `lib/hive`, `lib/hive/cli`, and `lib/hive/thor_usage_json`, then calls `Hive::CLI.start(ARGV, debug: true)`, catching `Hive::Error` to render `hive: <message>` to stderr with the error's `exit_code` (default `ExitCodes::GENERIC = 1`). `debug: true` is deliberate: it makes Thor re-raise parse/usage errors instead of swallowing them internally, so the executable wrapper can preserve Hive's exit-code and JSON-envelope contracts before any command object is constructed.
...
- `bin/hive` catches `Thor::Error` around the main CLI dispatch. Without `--json`, it prints Thor's usage error and exits 64. With `--json`, it delegates to `Hive::ThorUsageJson.emit` and emits a single error document on stdout before exiting 64; known parse-error commands are routed to their registered schema (`hive-run`, `hive-stage-action`, `hive-findings`, etc.) through `Hive::ThorUsageJson::ENVELOPES`, including the schema-required `verb` extra for stage-action verbs. `Hive::ThorUsageJson` is intentionally library code rather than inline binary code so the routing table, first-non-flag command detection, `--json` detection, schema-registration guard, EPIPE swallow, and JSON-generator warning branch are unit-testable. The integration suite round-trips every parse-error-raising mapped command against its published schema, the unit suite round-trips **all** mapped rows (including the defensively-retained `forget`/`prune`/`status`/`patrol`/`daemon`/`archive` entries), and `hive pr --json` is pinned to the canonical `verb: "open-pr"`. Only `status`/`prune` additionally reach the Thor parse path via an *unknown flag* (`--bogus`); `daemon`'s unknown-subcommand error is command-local and stays on stderr (its map row is dead). An unknown command has no versioned schema, so it falls back to an unversioned `{ok:false, error_kind:"usage", error_class:"UsageError", ...}` shape.
...
**Error envelopes.** Every `--json`-supporting command (`status`, `run`, `approve`, `findings`, `accept-finding`, `reject-finding`, `markers clear`, `metrics rollback-rate`, `forget`, `prune`, `patrol`, and the workflow verbs `brainstorm` / `plan` / `develop` / `open-pr` / `review` / `artifacts` / `finalize` / `archive`) emits a `Hive::Schemas::ErrorEnvelope` document on stdout when an error is raised. Detect failure by `payload.ok == false`. The envelope carries `schema`, `schema_version`, `ok=false`, `error_class`, `error_kind` (a closed enum per command — see `Hive::Schemas::RunErrorKind` / `StatusErrorKind` / etc.), `exit_code` (matches the raised `Hive::Error`'s `exit_code` per `Hive::ExitCodes`), and `message`. Per-error structured extras (`candidates` for `AmbiguousSlug`, `id` for `UnknownFinding`, `path` for `DestinationCollision`, `stage` for `FinalStageReached`) appear automatically. `hive daemon queue --json` uses its own `hive-daemon-queue.v1` `ErrorPayload` arm for queue-command failures (`unknown_action`, `missing_request_id`, `internal`) so queue inspectors still receive one schema-specific document. Parse/usage errors raised by Thor before command code runs now follow the same stdout JSON rule for mapped commands: `Hive::ThorUsageJson` wraps Thor's message in a top-level `Hive::UsageError < Hive::Error` (USAGE/64 exit code, with `error_kind` carried on the exception) and builds the appropriate schema-specific error envelope (for example, missing `TARGET` in `hive run --json` returns `schema: "hive-run"`, `error_kind: "invalid_task_path"`, exit 64). Stage-action verbs additionally carry the schema-required `verb` extra (e.g. `hive pr --json` reports the canonical `verb: "open-pr"`), and a genuinely unknown command (no mapped schema) falls back to a schema-less `error_kind: "usage"` envelope rather than a misrouted one. `hive run --json` additionally preserves the existing dual-signal contract on `:error` / `:review_error` markers — the SuccessPayload is emitted to stdout BEFORE the `TaskInErrorState` raise, so the rescue's ErrorPayload is suppressed (one document, exit 3). Errors that fire before `bin/hive` itself loads Hive/Thor (gem-load failures, shebang errors) cannot emit JSON — those remain stderr-text + exit-code only.
