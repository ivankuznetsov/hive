...
15. **Babysitter dry-run default-deny stubs are unit-pinned but not live-agent-smoked.** Commit `889b0b65` changed `bin/hive-babysitter-stub-git` and `bin/hive-babysitter-stub-gh` from mutating-command blocklists to read-only allowlists, commit `fc8e7739` tightened `gh api` classification so payload flags (`-f`, `-F`, `--raw-field`, `--field`, `--input`) skip as implicit writes unless the method is explicitly GET, and commit `29532639` tightened the `git` stub so otherwise read-only commands that include `--output` / `--output=...` skip instead of writing files. `test/unit/babysitter/dry_run_env_test.rb` verifies global-option stripping, unknown-command skips, mutating-command skips, read-only passthrough, the implicit-POST payload guard, `git diff --name-only` passthrough, and `git diff --output...` skipping with recording fake binaries. No artifact was found showing a full `hive babysit --once PROJECT --dry-run` live-agent run after these dry-run stub changes.
