...
class HiveCommandsDaemonTest < Minitest::Test
  ...
  def test_start_daemon_writes_pid_loads_global_config_runs_dispatcher_and_cleans_pid
    ...
    refute captured.key?(:merge_watcher),
           "pipeline finalization must not install the retired live-GitHub merge watcher"
  ...
  def test_status_json_merges_service_state_fields
    ...
    report.define_singleton_method(:binary_version) { |_binary| Hive::VERSION }
  ...
  def drift_for(service_state, version: nil)
    ...
    report.define_singleton_method(:binary_version) { |_binary| version }
    report.send(:binary_state, service_state).fetch("binary_drift")
  end
  ...
  def test_binary_state_raises_when_drift_not_in_declared_states
    ...
    report = Hive::Daemon::StatusReport.new(hive_home: @home)
    report.define_singleton_method(:binary_version) { |_binary| Hive::VERSION }
    klass = Hive::Daemon::StatusReport
  ...
  # safe_payload is the in-process ($stdout-free) status snapshot the web
  # dashboard renders directly. Happy path returns the full envelope hash.
  def test_safe_payload_returns_full_status_hash_for_running_daemon
    report = Hive::Daemon::StatusReport.new(hive_home: @home)
    write_pid_payload(pid: 1234)
    File.utime(Time.now - 5, Time.now - 5, report.pid_file)
    report.define_singleton_method(:pid_alive?) { |pid| pid == 1234 }
    report.define_singleton_method(:pid_owned_by_us?) { |_payload, pid| pid == 1234 }
  ...
  def test_safe_payload_degrades_to_not_running_hash_on_probe_failure
    report = Hive::Daemon::StatusReport.new(hive_home: @home)
    report.define_singleton_method(:running_state) { raise "probe exploded" }
  ...
  def test_status_text_reports_running_daemon
    ...
    report = command.status_report
    report.define_singleton_method(:pid_alive?) { |_pid| true }
    report.define_singleton_method(:pid_owned_by_us?) { |_payload, _pid| true }
  ...
  def test_binary_version_extracts_semver_from_probe_output
    ...
    ) { report.send(:binary_version, "/x/hive") }
