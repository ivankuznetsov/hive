...
task rows, and runs an internal active-task scan every 30s as repair. It dispatches
...
| Key | Default | Purpose |
|-----|---------|---------|
| `poll_interval_sec` | 30 | Minimum completion-to-start interval between periodic active-status repair scans. Min 5. Slow periodic scans cannot chain; ancillary completion can still request an immediate follow-up scan. |
| `fast_poll_sec` | 1 | Cheap wake cadence for child reaps and a rotating batch of at most 64 tracked state-file mtime probes between active scans. Only exact changed task rows refresh; dependency-bearing rows wait for the authoritative active scan. Min 1. |
