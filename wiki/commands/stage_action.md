...
hive review --pr 197                      # create/reuse 1-review/adhoc-review-pr-197, run review
...
hive archive <slug> --reason already_delivered \
  --evidence owner/repo#42                # verify, preview, explicitly confirm, archive
hive archive <slug> --reason superseded \
...
Confirmation re-reads every mutable fact before writing `closure.json`.
...
The `pr-review` workflow maps `review` to its own `1-review` stage and `archive` to `2-done`. Development, PR creation, artifacts and finalization verbs are rejected for these tasks. JSON stage fields use the task workflow. See [[pr-review]] for migration and publication constraints.
