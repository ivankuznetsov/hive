...
## [2026-06-06T19:40:06Z] wiki — audit hive-eval command coverage after traversal fix

**Action:** Audited commit `a80cddc1584175b32b75b8df08e1ed8a2044fac8` after it touched the executable `bin/hive-eval`, reporter tests, and wiki testing coverage. Read `AGENTS.md`, `.llm-wiki/config.json`, [[index]], [[architecture]], [[decisions]], [[gaps]], and recent [[log]] entries first; `qmd search "hive-eval scenario traversal testing eval"` found the older eval-harness wiki context, and the configured master wiki path had no `hive-eval` hits. Verified the committed diff plus current `bin/hive-eval`, `Rakefile`, `test/eval/support/reporter_test.rb`, `test/eval/support/reporter.rb`, and `test/eval/scenarios/s3_noise_test.rb`. Refreshed [[testing]] so the public runner contract now states that `--scenario` is basename-only, invalid selectors exit 64 before report creation, and `s3_noise` is a passing daemon-enabled signal/noise scenario rather than an expected baseline failure; also corrected the default-suite Rakefile snippet to include `test/babysitter`. No page coverage changed, so [[index]] did not need a catalog update; no new uncertainty was found for [[gaps]]. Did not run `qmd update` or `qmd embed`.

## [2026-06-06T20:51:24Z] wiki — audit hive-eval reporter fixture cleanup

**Action:** Audited commit `010cd3867f75a20c6d4b4807f2e6d77f385011a6` after it changed `test/eval/support/reporter_test.rb` so the reporter failure-path test removes stale `intentional_failure_*_test.rb` scenario fixtures before writing a new deliberate-failure fixture. Read `AGENTS.md`, `.llm-wiki/config.json`, [[index]], [[decisions]], [[gaps]], and recent [[log]] entries first; `qmd search "hive-eval reporter intentional failure fixture"` found the eval-harness wiki context. Verified the committed diff plus current `bin/hive-eval`, `Rakefile`, `test/eval/support/reporter_test.rb`, `test/eval/support/reporter.rb`, and the current [[testing]] eval section. Refreshed [[testing]] with the self-healing fixture-cleanup behavior. No page coverage changed, so [[index]] did not need a catalog update; no new uncertainty was found for [[gaps]]. Did not run `qmd update` or `qmd embed`.
