...
Accepted images are staged in a per-composer temp directory and inserted into the prompt as `[imageN]`. The footer shows `· [N image(s)]` while attachments are staged (ASCII-only, no emoji — the prompt label width math is lipgloss-managed and emoji glyphs split unpredictably across terminals). On submit, the TUI calls `Hive::Commands::New#call!` directly: `[imageN]` tokens become `![](assets/image-N.<ext>)`, files are copied into `1-inbox/<slug>/assets/`, and the recursive commit in [[modules/git_ops]] captures `idea.md` plus assets together. The on-disk basename is `image-N.<ext>` (controlled by `ComposerStaging::ATTACHMENT_BASENAME = "image"`); earlier drafts used `bug-N` but the name presumed a bug-report use case and would mislead operators staging design screenshots or architecture diagrams, so the neutral `image` was chosen.
