...
| `managed_directory_test.rb`, `modules/migration/{bounded_file_inventory,evidence_store,shadow_comparator,shadow_decision_migration,patrols}_test.rb`, `refactor_patrol/{job_store,registered_project_migration_coordinator,installed_job_schema_migration,job_store_generation_cutover,job_schema_restore}_test.rb`, `commands/{daemon,refactor_patrol_candidate_migration,refactor_patrol_schema_restore}_test.rb`, `integration/{cli_version,refactor_patrol_command,module_migration}_test.rb` | Patrol migration storage — descriptor-relative no-follow reads, locks, directory traversal, atomic directory-to-tombstone exchange, and exact transaction-bound restore; linked managed-component, file, and lock refusal; filesystem-order-independent bounded inventories; source/archive/replacement-digest-bound restart checkpoints; v3-only runtime admission; complete per-user registered-project sweeps across two independent `HIVE_HOME` installations and custom state roots; realpath-alias deduplication; registry-digest rescan; package/first-use activation; released-format v2 conversion; corrupt/permission failure isolation and hourly retry; candidate daemon fencing; and full-v3 quarantine before exact v2 reactivation. |
