...
| `managed_directory_test.rb`, `gemspec_test.rb`, `modules/migration/{bounded_directory_entries,migration_repository,qualification_checkpoint_verifier,qualification_process_custody,qualification_scenario_evidence_collector,qualification_open_router_transport,qualification_provider_broker,qualification_provider_protocol,qualification_scenario_driver,qualification_scenario_orchestrator,qualification_scenario_process,qualification_lane_runner}_test.rb` | Compressed Patrol host qualification — entry caps apply while enumerating rather than after allocation; target reads are no-follow and descriptor-stable; checkpoint events require the complete canonical semantic envelope; host evidence collection is byte-for-byte non-mutating; direct driver fixtures use a scenario-owned usage database and module-decision recovery waits for the gated fault worker to disappear before exact lost-attempt reconciliation; process failures after spawn retain only an 8 KiB typed digest projection, reap the owned foreground process group including a same-group child, preserve exact orchestrator type, and publish a receipt-less failed-generation row; the installed lane uses packaged module sources plus a fixed-endpoint/model, one-call host broker whose PID-reuse-safe launcher ancestry, Unix-socket capability, canonical output, exact typed retry reason, transcript digest, failure-path capability cleanup, duplicate transcript-identity rejection, and absence of candidate-visible source/network/key/model are exercised through a real Bubblewrap namespace; terminal lane files are prevalidated, staged as one exact private tree, and installed with a no-replace atomic directory rename; injected interruption after every staged write and immediately before install leaves no visible partial lane and retries safely with different evidence. |
