...
| `preflight:` | Optional `Proc` invoked before each spawn (e.g., `pi` checks `auth.json` under `PI_CODING_AGENT_DIR` or the default `~/.pi/agent`). |
...
- `pi` — no permission flag, no `--add-dir` (triggers `warn_isolation_reduced` when callers pass `add_dirs:` per ADR-018). Queued commit `5c1a20e9` makes preflight and skill resolution share `PI_CODING_AGENT_DIR`, falling back to `~/.pi/agent`, and require `auth.json` there. Min version `0.70.2`. `:output_file_exists`.
