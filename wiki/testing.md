...
| `prdigest_test.rb`, `commands/digest_test.rb` | PRDigest delegation — registered GitHub identity resolution, case-insensitive subset filters, private token-free config, argv-only repeated repository forwarding, prior-London-day default, credential/binary failures, unchanged `prdigest-result` JSON, dry-run/human output, child-exit preservation, and malformed/contradictory child-result refusal. |
