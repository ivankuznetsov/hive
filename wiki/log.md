...
## [2026-06-05T19:55:00Z] patrol/status/tui/cli — refresh archive, sparse handoff, and executable parse-error coverage

**Action:** Refreshed the LLM wiki after commit `4bf726c6` removed the dead `marker_name:` archive-filter parameter, inlined the TUI folder-mtime parse helper, hardened `Hive::Patrol::ReviewHandoff#idea_text` for sparse direct `Finding.new` values, and added direct TUI display-cell formatter tests. Read `AGENTS.md`, [[index]], [[architecture]], [[decisions]], [[gaps]], and recent [[log]] entries first; `qmd search "archive marker agnostic ArchiveFilter marker_name status tui ReviewHandoff evidence Format truncate wide grapheme"` returned no indexed results, so this pass used direct source/wiki reads. Verified the committed diff plus current `lib/hive/archive_filter.rb`, `lib/hive/commands/status.rb`, `lib/hive/tui/snapshot.rb`, `lib/hive/patrol/review_handoff.rb`, `lib/hive/tui/views/format.rb`, `bin/hive`, and focused tests. Updated patrol command/module coverage for sparse `idea.md` rendering, TUI/testing coverage for display-cell formatter boundaries, CLI coverage for the current parse-error matrix and unknown-command `usage` fallback, and gaps for the remaining live-smoke uncertainty. Did not run `qmd update` or `qmd embed`.
