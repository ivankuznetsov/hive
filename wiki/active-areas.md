...
| `153bed1d`, `29d02c34`, `163ed51e`, `ccfa7c03` | Rails web architecture and resource bounds | The first three are rebased equivalents of the previously documented `96b06792` / `4455fc06` / `22d80d1b` line: mutations move onto Rails resources, status polling becomes subscriber-owned, and project filters render at the GET boundary. The branch head additionally bounds composer file inspection/retention, prevents overlapping Turbo-frame polls, and rejects oversized or chunked bodies before Rack parsing. |
...
| `fce1a3a3` | Historical daily digest | The internal engine and later PRDigest adapter have both been removed from Hive; PR digests now run through standalone PRDigest prose or agent facts. |
