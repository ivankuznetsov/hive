...
| `commands/daemon_test.rb` | `Hive::Commands::Daemon` and `Hive::Daemon::StatusReport` — lifecycle command routing, PID-file ownership/status handling, detached start/re-exec behavior, service install/enable/disable output, daemon-status service/binary fields, explicit runtime `HIVE_BIN` authority across managed-web PATH collisions, `safe_payload` web degradation, `binary_drift` classification including `unreadable` and stable-symlink equivalence, queue inspection, start-path wiring of daemon/update/answer-digest config into the dispatcher, and `daemon.auto_retry.enabled` config threading into the dispatcher. |
...
| `patrol_fix/operational_projection_test.rb`, `daemon/{patrol_fix_operational_projection,patrol_fix_runtime,operational_snapshot,status_consumer}_test.rb`, `commands/{status,watch}_test.rb`, `operational_status_test.rb`, `bot/status_watcher_test.rb`, `tui/snapshot_test.rb`, `web/test/models/patrol_overview_test.rb`, `schema_files_test.rb` | Unified Patrol repair U10 — one bounded schema-validated project projection, source-read isolation, unique-root cohort metrics, authoritative/partial latency, telemetry-only current-day token observations, daemon snapshot composition, and exact Status/Watch/TUI/bot/web pass-through without adapter source-store reads. |
| `daemon/patrol_fix_candidate_inventory_test.rb`, `patrol_fix/{admission_store,semantic_admission,task_materializer}_test.rb`, `daemon/{patrol_fix_admission_scheduler,patrol_fix_runtime,dispatcher}_test.rb`, `commands/patrol_fix_semantic_decision_test.rb`, `integration/patrol_fix_lifecycle_test.rb`, `e2e/lib/patrol_fix_qualification_test.rb` | Unified Patrol repair U11 — complete owned-manifest inventory binding with deterministic top-64 rich context; unrelated-task isolation and owned-corruption refusal; one exact leased semantic child under existing daemon capacity/restart/timeout custody; stale inventory/head/context rejection; both-source duplicate convergence; real workflow rework plus separate parked escalation successor; one fake-host publication under restart replay; a frozen historical four-gate corpus through production prompt/parser seams; and a strict observation-only source-to-PR dogfood report. Focused tests use fake providers, a local bare remote, and fake GitHub only. They do not satisfy the opt-in real-provider, natural-finding, hosted-CI, or real-PR gate. |
...
after all eight cases clear the unsafe-decision bar.
| `patrol_fix/migration/{inventory,semantic_group,reconciler,disposition_manifest}_test.rb`, `patrol/migration_inventory_test.rb`, `refactor_patrol/{migration_inventory,job_store,canonical_action_catalog}_test.rb` | Unified Patrol repair U8 migration preflight — complete snapshot-stable source cursors independent of truncated health/list projections, corrupt-row blocking, direct v4 job/action/artifact observations, provider-free exact grouping plus strict injected semantic decisions, all-state PR reconciliation without mutation, exactly-one count/root equations, deterministic schema-valid bytes, and raw-only byte-identical opaque-v3 accounting. |
| `github_publication_test.rb`, `patrol_fix/{publication_receipt,source_snapshot,task_materializer}_test.rb`, `stages/patrol_fix/publish_test.rb` | Unified Patrol repair U5 — expected-absence local-bare-remote publication; durable identity/digests without raw title/body/diff; complete paginated all-state PR reconciliation; exact marker/title/body/repository/base/branch/head adoption across open/draft/closed/merged states; foreign same-head and wrong-head branch refusal; intent-before-attempt retry versus attempted-unknown parking; lost push/create response convergence; title/body/diff secret blocking with redacted errors; immutable creation base across later main advancement; exact review/head revalidation before effects and receipt; zero-transport adoption only for a full canonical migration payload; durable `pr.md`/receipt replay; receipt-gated Done transition; tampered-cleanup-custody refusal; and non-revoking cleanup diagnostics. GitHub behavior is injected and no test uses a live account. |
| `refactor_patrol/{effect_gateway,job_store,claim_maintenance_transitions}_test.rb`, `refactor_patrol/pr_opener_test.rb`, `refactor_patrol/action_runner_test.rb`, `refactor_patrol/repository_ownership_test.rb` | Generation-scoped transition and publication recovery — one job-bound occurrence across discovery/actions, direct v4-only JobStore authority that leaves obsolete v3 bytes opaque, exact recorded transition reconciliation after a settlement crash, one-hour pacing for retryable action failures, durable diagnostic retry episodes, gateway-routed semantic mutations, one narrow claim-maintenance port, one blocking process/thread sender authority per semantic intent, local transition retry safety without a second recovery record, remote absence refusal, canonical receipt replay, action-local generation fences, append-only publication phases, trunk-drift rechecks, immutable supersession, exact proven-old-OID branch replacement, and continuation ownership. |
