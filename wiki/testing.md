...
| `lock_test.rb` | `Hive::Lock` — acquire/release, stale-PID detection, commit-lock process contention, same-thread reentrancy, fork-inherited ownership refusal, bounded timeout, exception cleanup, and kernel release after `SIGKILL`. |
