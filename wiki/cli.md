...
`--json`, `--json=true`, `--json=1`, and `--json=yes` are canonicalized to
`--json` before dispatch; leading `--no-json` / `--skip-json` are moved after
the command as negations. The help rewrite preserves any leading dash-prefixed
