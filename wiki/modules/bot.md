...
| Module | File | Purpose |
|--------|------|---------|
| `Supervisor` | `lib/hive/bot/supervisor.rb` | Long-running loop. Polls Telegram, runs status ticks, reaps child commands, handles TERM/INT/HUP, writes last-seen update IDs, and applies an interruptible one-second retry delay after a failed Telegram poll so invalid credentials or immediate transport failures cannot create a CPU/logging loop. |
| `Telegram` | `lib/hive/bot/telegram.rb` | Thin `telegram-bot-ruby` wrapper for `getUpdates`, `sendMessage`, plain length/newline splitting, typed `Update` records, media/voice metadata extraction, `getFile`, and file download. Benign `getUpdates` transport errors (`Faraday::TimeoutError`, `Faraday::ConnectionFailed`, `Net::ReadTimeout`) emit `poll_failure` at `level=debug category=noise`; other poll errors remain warnings. The wrapper exposes whether the most recent poll failed so the owning supervisor, rather than the transport adapter, controls retry pacing. |
...
| `NotificationBuilders` | `lib/hive/bot/notification_builders.rb` | Renders notification text and inline keyboards from `RowActions` descriptors. Every `ERROR` / `REVIEW_ERROR` recovery alert exposes guarded Autofix even when no diagnostic suggested action was recorded; `EXECUTE_STALE` remains manual-only. Rows whose live action is `agent_running` or `archived` suppress stale recovery alerts, but a non-null `auto_residue` summary still produces a keyboard-free FYI naming the committed paths. The builder logs stale-marker contradictions as `notification_skipped_live_agent`. `details_reply(row)` is the shared Show-details renderer for inline callbacks, `/details`, and status row details: it always includes the row summary and next-step hint, appends cached diagnostic summary/detail when present, and caps replies to Telegram's 4096-character limit. All three render sites wrap the call in a soft-degrade rescue (`render_details_reply` in the handlers, an inline rescue in `Supervisor#render_details`): a render-time fault logs `details_render_failed` and replies "Status lookup failed — try again in a moment." rather than escaping past the already-ack'd callback / poll loop and leaving the operator with no reply. The slash `resolve_status_row` degrade now also logs `status_lookup_failed`. The `ready_for_review` approval push appends a clickable PR link when `row.pr_url` is valid. Fingerprints include project, slug, stage, marker, marker attrs, and `auto_residue`, deliberately excluding `pr_url` and rendered text. |
| `BrainstormParser` | `lib/hive/bot/brainstorm_parser.rb` | **Back-compat alias for `Hive::BrainstormParser`** (`lib/hive/brainstorm_parser.rb`) — the pure parser moved to the top-level namespace so the daemon can share it for the answers-pending gate (see [[modules/daemon]] §"Brainstorm answers-pending gate"). Parses one-based `## Round`, `### Q<N>.`, and `### A<N>.` blocks. **Lenient A-header**: accepts the first A-section under each Q as that Q's answer slot regardless of A-number, so a brainstorm-agent off-by-one (e.g. `### A2.` after `### Q1.`) doesn't strand the operator. Stage markers delimit blocks but are excluded from question text/fingerprints. `parse` is total — invalid UTF-8 and a torn concurrent read degrade rather than raise — and reverses only the writer's explicit v1 structural-line encoding; legacy unmarked answers remain literal. Fingerprints use NFC rather than compatibility-folding NFKC. Also exposes the canonical answer heading helper used by the writer. |
| `BrainstormAnswerWriter` | `lib/hive/bot/brainstorm_answer_writer.rb` | Locked, first-write-wins insertion into the next answer block, with atomic rewrite. Slot location is **Q-context-aware**: walks forward from the parser-identified target Q's line, returning the first empty A-section before the next block boundary. When Q{n} is present and unanswered but has **no `### A` slot at all**, the writer **creates** the slot at the end of the Q-block and writes the answer there. The writer scrubs invalid UTF-8, normalizes lone-CR input before ordinal mapping, reversibly neutralizes structural answer lines under an explicit v1 answer header, and takes the existing marker sidecar lock around read-plus-atomic-replace so marker and answer writers cannot clobber each other. Identity-bound writes use bounded, creation-disabled marker locking; the unreachable exact-writer `:answer_slot_missing` fallback is removed. Giving up on the lock emits `:answer_lock_contention` carrying the holder metadata from the rescued `Hive::ConcurrentRunError`; the holder is captured before the deadline check, so a `try_append` slower than the whole retry budget still names the culprit instead of reporting `holder: nil`. |
| `ConversationStore` | `lib/hive/bot/conversation_store.rb` | In-memory per-chat active answer state with TTL. No sidecar file; `brainstorm.md` stays canonical. State carries `chat_id`, `project`, `slug`, displayed `question_n`, the opaque identity `binding`, `mode`, and `updated_at`; the retired Codex draft-assist fields (`history`, `draft`, `awaiting_confirm`) and `pending_confirm_count` API are gone. **A chat holds at most one active conversation: `start` deletes any other entry for the same `chat_id` before inserting**, so starting `/answer task-b` while `task-a` is open supersedes it and the slug-less lookups used by the free-text path (`FreeTextHandler`, `Router#answer_context`, slash `/done`) always resolve to the most recently started conversation instead of a stale first match. Telegram writes pass the binding through the router to `Hive::Commands::Answer`, so same-number questions from replacement rounds fail stale rather than receiving an old reply. All `@states` reads/writes are mutex-guarded because the Telegram poll thread mutates conversations while the status-poll thread calls `active_for_slug?` and prunes. `active_for_slug?(slug, project: nil)` is prune-aware and returns true when any chat has a non-expired answer conversation for that slug. Project scoping is lenient: two fully resolved different projects do not cross-suppress, but an unscoped/project-less side still suppresses to avoid reintroducing duplicate waiting alerts. `NotificationDispatcher` uses this to avoid duplicate proactive waiting pushes while the operator is already answering. |
...
| `DispatchRequestWriter` | `lib/hive/bot/dispatch_request_writer.rb` | Client of the shared runtime-control-plane dispatch repository. It validates argv before inserting a durable request row, then attempts immediate non-interactive admission; accepted attempts bind the same request row, capacity or unavailable-local cases remain queued, and unexpected failures remove only a still-unclaimed row. Recoverable-marker requests delegate to `Hive::Recovery::API`. |
...
CleanExit residue is the one suppression exception: when a ready action is
daemon-owned, or a waiting action is hidden by an active conversation, the
dispatcher sends only the keyboard-free residue FYI and keeps the ordinary
approval/input notification suppressed. The alert fingerprint includes the
bounded summary, so each new residue commit is delivered once.
...
Two other in-process paths remain. Brainstorm answer insertion is deliberately
narrow: Telegram inventories the exact slot through `Hive::Commands::Answer`,
stores its opaque binding with the conversation, and submits the final literal
reply through the bound write form. Identity, stage, generation, ordinal,
fingerprint, task-lock, and marker-sidecar-lock checks therefore match the CLI
and web surfaces. Two devices racing the same question produce one write and a
closed idempotent/conflict/stale result, not a merged or overwritten block.
Typed `/answer` targets resolve the current status row first and retain its
project identity; duplicate slugs are refused. Restart/TTL reply reattachment
has no presented binding, so it fails closed with a fresh `/answer` prompt
instead of inventing a binding for whichever question is current.
...
state, including its slot binding (or a reply-to reattach target), to the result, and the supervisor
feeds a successful transcript into `execute_answer_write`. That means the
same identity-bound command, first-write-wins behavior, auto-advance reply, and
...
`Hive::RuntimeControlPlane::DispatchRepository::ALLOWED_VERBS`
...
`Hive::RuntimeControlPlane::DispatchRepository::OUTBOX_EXPIRY_SEC` (1h) are stale and are
