...
| `hive forget NAME [--json] [--if-exists]` | Drop one named entry from the global registry (`~/Dev/hive/config.yml`). Inverse of `hive init`. The project's `.hive-state` directory on disk is not touched. Unknown name → exit 64 unless `--if-exists` is passed for retry-safe no-op success (`removed: false`). | `Hive::Commands::Forget` | [[commands/forget]] |
