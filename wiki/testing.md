...
| `daemon/status_consumer_test.rb` | `Hive::Daemon::StatusConsumer` — hidden internal task-graph parsing including canonical task `pr_url`, schema-version skew handling, strict `live_task_lock` and non-negative project hidden-count coercion, aggregate hidden-count parity, legacy project filtering, and local `state_file` mtime re-stat so daemon edit-resume decisions keep subsecond precision. |
...
| `stages/review/{ci_fix,triage,browser_test,fix_guardrail,suppression}_test.rb` | Review phase helpers — CI-fix retries, binary-labelled hosted-check output normalization before fix-prompt rendering, triage prompt/bias/custom-template/protected-file behavior, triage `review_triage` default fallback values (75 / 1800), browser-test protocol handling, fix-guardrail approval gates, and no-fix suppression fingerprint/strip/seed behavior. |
...
| `commands/status_test.rb`, `dependency_snapshot_test.rb`, `archive_filter_test.rb`, `schema_files_test.rb`, `tui/schema_correspondence_test.rb` | Shared active/archive projections, machine contract, and scan boundary — workflow/action-aware captured archive membership, one action classification per terminal candidate, exact 72/168-hour strict boundaries, `never`, all-project workflow/config generation capture before scanning, visible invalid-workflow error rows, admission derived from prepared active folders with per-project disk fallback, completion-phase project isolation, routine omission of inert terminal history, inclusion of non-inert terminal stages, exact dependency-history lookup, retention-bypassing exact task lookup, lossless dedicated archive mode, explicit ordinary/active/archive/partial projection kinds, additive schemas, required task-key correspondence, preserved `folder_mtime`, and stage-move race handling. |
| `tui/snapshot_test.rb`, `tui/state_source_test.rb`, `tui/views/{archive_pane,tasks_pane,hyperlink}_test.rb`, `tui/{bubble_model,app}_test.rb` | TUI projection consumption — producer-authoritative active rows and their matching reused admission context, admission/quota/closure and PR rendering, no public-mtime retention calculation, archive absent at boot, explicit lossless archive loading, request coalescing, active/archive publication-race preservation, isolated project degradation with last-known rows, archive-failure isolation, restart after a timed-out poller stop, active descriptor/config/task fingerprints, malformed-workflow error visibility, and archive-pane completeness. |
...
| `tui/app_test.rb`, `tui/state_source_test.rb`, `integration/tui_reactivity_perf_test.rb` | `Hive::Tui::App` / `StateSource` — charm-only backend selection, synchronous active-only startup, snapshot-poller dedup/error dispatch, HUP/WINCH handling, content-fingerprint-gated active refresh reuse, liveness-fallback reparsing, explicit archive refresh and coalescing, workflow/policy stat-fault sentinels, publication-after-fingerprint ordering, bounded state-file change latency, per-project degradation retention, and routine parse cost independent of terminal-history size. |
...
| `archive_visibility_retention_test.rb` | Fixed-clock cross-surface active/archive acceptance — one real mixed project with legacy omission, explicit `3`, `7`, and `never`; routine status, operational/daemon, TUI, and web expose only active work with compatibility archive counts at zero; explicit archive remains lossless; task shapes stay unchanged; and archive-policy edits do not invalidate an unchanged active web projection. |
