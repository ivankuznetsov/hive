...
**TLDR**: Phase 1 MVP shipped Apr-25 and the active surface is now wider than the original loop: PR-first review, `7-artifacts`, daemon auto-advance/archive, descriptor-backed custom workflows with resource controls, managed Honeycomb package install/update/activation, Claude/Codex/Pi/Grok agent profiles, `hive refactor-patrol`, a complete local operator web UI, TUI dashboard, Telegram bot, managed llm-wiki bootstrap, release/install verification, eval harness, token usage accounting, and project-global Claude tmux/headless routing are all implemented. Current deferred work is mostly depth/scale: parallel reviewers, observability exports, richer PR-comment ingestion, daemon/bot operational polish, and live-provider smoke depth.
...
Recent release/dependency/history inspected on 2026-07-19:

| Commit | Area | Notes |
|--------|------|-------|
| `edaa71314` / `3964fb8f` / `a05d85ac` | Managed workflows / Honeycomb | Installs complete immutable Honeycomb packages with exact runtime policy and runnable mapping defaults, then retires the local `architecture` and `writing` scaffolds after their managed-package flagship proof. Local scaffold creation now offers only `blank` and `research`. |
| `af71b993` / `4e3e0d51` | Hive Web / v0.6.4 | Completes the loopback operator UI across local mode, Grok login, agent-skill readiness, Telegram pairing, task recovery, and reviewed workflow lifecycle mutations; v0.6.4 clears the submitted composer before Turbo render and precreates the babysitter dry-run audit log. |
| `cea12c19` / `5be363dd` / `8f23ed5f` | Patrol reliability | Bounds ordinary review batches by live quota headroom, persists clean-prefix progress, preserves architecture reviewer evidence, analyzes dirty source checkouts through pinned detached worktrees, and accepts strict JSON followed by reviewer rationale. |
| `0835becb` | Internal consolidation | Shares command/result handling, durable state/value helpers, identity validation, scheduler/deadline policy, process probes, and presentation utilities while preserving consumer-specific failure policy. |
...
| Shared service installer | `lib/hive/commands/service_installer/{base,outcome,result_presenter}.rb`, `lib/hive/commands/{daemon,bot}/service_installer.rb`, `schemas/hive-{bot,daemon}-install.v1.json`, `docs/solutions/…cross-platform-service-installer-base…` | `ServiceInstaller::Base` is subclassed by daemon + bot and returns a `ServiceInstaller::Outcome`; `ResultPresenter` now owns their common JSON/text/exit presentation. Content-comparison drift detection (`--force` to overwrite), `unsupported` outcome on hosts with no service manager, exit codes 0/64/70, and a read-only `service_state` probe remain surfaced as `service_installed`/`service_enabled`/`unit_path` in both `bot`/`daemon status --json`. |
| Managed workflows | `lib/hive/workflows/package*.rb`, `lib/hive/commands/workflow.rb`, `schemas/honeycomb-*.json`, `templates/workflows/` | Honeycomb v2 packages resolve to immutable generations, disclose exact actor permissions and runtime policy, bind reviewed generation/config identities to activation, and preserve visible failure causes. Current local scaffolds are `blank` and `research`; retired `architecture` and `writing` names direct operators to the corresponding Honeycomb install commands. |
| Hive Web | `web/`, `lib/hive/web/`, `lib/hive/commands/web.rb`, `wiki/commands/web.md` | Connection-authenticated loopback UI covers projects, task creation/recovery, agent login and managed-skill repair, Telegram pairing, and preview/consent/receipt workflow lifecycle operations. Explicit repository/provider boundaries and command-layer validation remain in force. |
...
| Release/install | `install.sh`, `install.md`, `packaging/`, `.github/workflows/install-smoke.yml`, `.github/workflows/release.yml` | The local `v0.6.4` tag points at synchronized release source containing managed Honeycomb packages, the complete operator web surface, quota-bounded patrol recovery, retired flagship scaffolds, and the composer/audit-log fixes. Update/uninstall, release artifact verification, install-smoke `jq` provisioning, daemon service install JSON envelopes, hivebox Docker install/smoke flow, and remaining signed-tag/macOS x86_64/live-provider follow-ups are documented in [[operating]], [[commands/web]], [[testing]], and [[gaps]]. |
