...
The empty `gh` config/home directory is created once for the lifetime of `DryRunEnv.with_env`, pinned into the generated launcher, and removed when the agent command returns. Allowlisted `gh` passthroughs therefore keep the hermetic config boundary without leaving per-command directories under `/tmp` after `exec`.
