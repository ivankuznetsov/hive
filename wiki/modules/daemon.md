...
| `Hive::Daemon::DailyDigestDeliveryScheduler` | `lib/hive/daemon/daily_digest_delivery_scheduler.rb` | Separate opt-in Telegram scheduler. It walks the persisted interval sequence, waits until the configured hour in the current record's zone, and emits one `hive digest send --date D --json` child for the immediately preceding closed record. Its cursor binds the exact record ID. |
