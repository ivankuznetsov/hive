...
`Hive::DiagnosticEvidence` fills the nil-diagnostic gap in tier order: `diagnostics/red-status.md` (a prior agent verdict), then the newest meaningful `logs/*.log` line, then the current marker on the task state file. Logs are globbed from both the in-folder `logs/` **and** the inferred global per-task log dir `.hive-state/logs/<slug>/` (the primary run-log dir for coding tasks, derived from `Hive::Task::PATH_RE`). The result carries an explicit tier so the detail line is prefixed by source — `Diagnostics:` (red-status), `Log:` (a log), or `Marker:` (the state file) — instead of always saying "Log:". Symlinked evidence escaping the task/log roots is rejected before any read. That fallback emits a one-line summary plus a source path instead of a bare "no diagnostic" result.
