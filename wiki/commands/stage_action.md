...
3. **Archive idempotency check**: if the verb is `archive` and a `9-done` finalization task has both `:complete` and a durable cleanup receipt, emit a `noop`. Legacy unfinalized archived history remains compatible. A moved task without its receipt re-runs Done to finish cleanup.
...
6. **Transition validation**: archive first runs the pure-evidence finalization reconciler, then `Conditions::TransitionGuard` requires current reconciler-authored `archive_ready` before marker validation. Finalize success, `summary.md`, terminal markers, `--force`, green/readiness, disappearance, and closed-unmerged state are insufficient. Other forward advances require a terminal marker — currently `:complete`, `:execute_complete`, or `:review_complete` (the closed set is `Hive::Markers::TERMINAL_MARKER_NAMES`). The `brainstorm` verb's ordinary source behavior remains descriptor-driven.
...
For `archive` specifically: a second invocation against a fully cleaned task is a clean no-op (phase: `noop`, exit 0, `next_action.key: archived`). A crash after the one-time stage move but before cleanup receipt deliberately re-runs Done from `9-done` until exact local cleanup completes.

## Archive eligibility

`Hive::Finalization::Reconciler` appends `archive_ready` only when the current
task/job/adopted-attempt/PR/head coordinates carry explicit babysitter-observed
`MERGED` evidence, or a current guarded no-PR approval with no later re-arm.
StageAction never queries GitHub while checking the transition. Its guard is
evaluated before generic force behavior, so `hive archive --force` cannot
