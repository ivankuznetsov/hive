...
2. Stop a foreground daemon using the XDG state-home `.daemon.pid` when present.
3. Deregister platform service units:
   - macOS: unload and remove `~/Library/LaunchAgents/local.hive-daemon.plist`.
   - Linux: `systemctl --user disable --now hive-daemon`, remove `~/.config/systemd/user/hive-daemon.service`, then daemon-reload.
