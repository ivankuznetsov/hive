...
| `Hive::Daemon::ChildSupervisor` | `lib/hive/daemon/child_supervisor.rb` | Spawns `hive ...` subprocesses with `pgroup: true`; writes daemon-dispatched child output under the durable XDG state directory at `logs/daemon-children/<project>/<slug>/` (tmpdir is only the standalone-caller fallback), outside the tracked project state worktree; reaps via `Process.wait(-1, WNOHANG)`; parses JSON envelopes from child stdout; supports `terminate_all(grace_sec:)` with TERM→KILL escalation. |
