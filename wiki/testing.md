...
| `cli_usage_error_json_test.rb` | `bin/hive` pre-dispatch usage errors under `--json` — missing required targets for schema-backed commands, workflow verb `hive-stage-action` envelopes, `markers` required-option failures, and `prune` usage errors before command object dispatch. |
