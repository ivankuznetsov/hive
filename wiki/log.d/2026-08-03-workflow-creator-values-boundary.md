...
- Extended static require ownership and clean-load feature detection to
  catalog-owned `packaging/` paths, including `require_relative` and
  repository-root-prefixed `require` identities.
  ...
  fail both named checks, while lazy edges—including
  `require "lib/hive/commands/run"`—fail named static validation without widening
  the global scan.
