...
| `integration/migrate_patrol_findings_script_test.rb` | Explicit one-time historical import — active ordinary findings become Patrol Fix tasks; accepted Architecture Patrol fix/discuss dispositions become admissions while dismissals stay inert; missing target revisions bind to current default-branch state; secret-like evidence is redacted; dry runs do not write; reruns converge; matching conflicts fail closed; and unrelated malformed task metadata is ignored. |
