...
created: 2026-04-25
updated: 2026-07-19
tags: [cli, api, skills, provisioning]
---

**TLDR**: Hive exposes a Thor-based CLI through `hive`, plus an `hv` fallback entrypoint for hosts where Apache Hive shadows the `hive` name. The human workflow is `hive status` followed by stage verbs (`brainstorm`, `plan`, `develop`, `open-pr`, `review`, `artifacts`, `finalize`, `archive <target>`) that move-or-run tasks by slug. `hive review --pr <n>` reviews an existing GitHub PR. `hive doctor` is read-only managed-skill diagnosis and `hive setup-agents` is the separate consent-safe provisioning surface. Lower-level commands cover run/approve, findings, markers, metrics, auth, contribution, reporting, and background services. Versioned `--json` contracts and stable process exit codes let wrappers branch deterministically.
...
sibling shape, `setup` keeps the unversioned `hive-setup` shape, and Screenote
...
| `hive setup [--json] [--service] [--no-bootstrap] [--no-init]` | Local workstation provisioning: diagnostics, optional QMD/web-bundle bootstrap, daemon service install, optional project enrollment, and optional web-service install. `--no-bootstrap` is diagnose-only and performs no writes beyond output. | `Hive::Commands::Setup` | [[commands/setup]] |
| `hive setup-agents [--yes] [--json] [--agent NAME...] [--skill ID...]` | Preview, consent to, execute, and post-verify native provisioning for unresolved enabled manifest-managed Claude/Codex/Pi skills. JSON mutation requires `--yes`. | `Hive::Commands::SetupAgents` → `Hive::AgentSkills::Provisioner` | [[commands/setup-agents]] |
...
| `hive doctor [--json]` | Read-only shared inspection of effective managed stages/reviewers/hooks: bounded native inventory plus the real resolver, with health/evidence/remediation in `hive-doctor.v2`. Legacy tmux/QMD/config checks remain separate. Exits 0 / 65 / 78. | `Hive::Commands::Doctor` | [[commands/doctor]] |
...
- `init` accepts `--force`, `--json`, and the pre-write `--refactor-patrol` / `--no-refactor-patrol` discovery choice. Omitting the boolean keeps fresh discovery enabled; neither value enables auto-fixing or issue filing.
...
- `--json` is a `class_option` honoured by `init`, `status`, `doctor`, `setup-agents`, `run`, `rebase-status`, `approve`, `drop`, `findings`, `accept-finding`, `reject-finding`, the workflow verbs (`brainstorm`, `plan`, `develop`, `open-pr`, `review`, `artifacts`, `finalize`, `archive`), `markers clear`, `metrics`, `forget`, `prune`, `workflow new`, `connect screenote`, `disconnect screenote`, `bench submit`, `digest`, `pairing`, the `daemon` subcommands (`status`, `stop`, `reload`, `install`, `enable`, `disable`, `queue`), and the `bot` lifecycle subcommands (`status`, `stop`, `reload`). Doctor emits `hive-doctor.v2`; setup-agents emits `hive-setup-agents.v1`, never prompts in JSON mode, and requires `--yes` for mutation. Daemon JSON is published as `hive-daemon-status.v1` / `-stop.v1` / `-reload.v1` / `-install.v1` / `-enroll.v1` / `-queue.v1`; bot lifecycle JSON is published as `hive-bot-status.v1` / `-stop.v1` / `-reload.v1`; pairing JSON is published as `hive-pairing-list.v1` and `hive-pairing-approve.v1`. `hive babysit` is bare-text in v1. Each command with full envelope support emits a typed JSON document on success and a structured error envelope on failure. Workflow verbs emit a single `hive-stage-action` envelope. `init` emits `hive-init.v2` on success (v1 remains loadable for pinned consumers); `drop` emits `hive-drop.v2`; `rebase-status` emits `hive-rebase-status`; `setup` remains unversioned `hive-setup`; and Screenote/workflow/bench/digest keep their existing contracts.
