...
```ruby
{
  ...
  "honeycomb"         => { "repository" => "ivankuznetsov/honeycomb" },
  "project_name"      => nil,
  "permissions"       => "yolo",
...
`honeycomb.repository` is the GitHub `owner/repository` slug used by `hive workflow publish`; it defaults to `ivankuznetsov/honeycomb`. Config validation rejects blank, malformed, URL, and `.git` values. Publication stores its verified upstream checkout under the XDG cache path returned by `Hive::Paths.honeycomb_cache_path`, never in the author worktree.
...
10. **`validate_daemon!`** — daemon numeric bounds, booleans, and nested hashes are checked before the daemon starts. The nested `daemon.auto_retry` block must be a hash, and `daemon.auto_retry.enabled` must be boolean when present.
11. **`validate_honeycomb!`** — `honeycomb.repository` must be a GitHub-style `owner/repository` slug and must not carry a `.git` suffix.
