...
- The separately maintained `hive-site` source must mirror the native-first
  ...
  from that site, so its companion PR and rendered Jekyll/Pagefind verification
  remain external delivery evidence rather than Hive-source evidence.
...
17. **Tmux-session disappearance is unit-pinned but not live-smoked.** Commit `71db09c7` added `test/unit/claude_launcher_test.rb` coverage proving that `Hive::ClaudeLauncher.wait_for_terminal_marker` stamps `ERROR reason=tmux_session_terminated` when `runner.session_exists?` turns false before a marker arrives, and reports a pane-readability error if the tmux session check itself fails. The daemon healer now also has focused unit coverage for clearing non-review terminal agent-loss `ERROR reason=tmux_session_terminated` / `reason=agent_orphaned` markers in `2-brainstorm`, `3-plan`, `4-execute`, `7-artifacts`, and `8-finalize` with marker-id guards and unbounded cooldown retries; the `3-plan` variant additionally queues `hive plan <slug> --project <project> --from 3-plan` and logs `heal_requeued`. Branch `fix-claude-tmux-ready-detector-260629-50cc` documents the correct local-gem validation path for Claude tmux launcher-script packaging fixes in [[operating]], and source/tests show the gemspec includes `lib/hive/scripts/**/*.sh`; this refresh still found no in-tree artifact of a locally built `hive-cli` install replaying the affected Claude tmux task to `WAITING` or later. No in-tree artifact was found for the full live or integration path: kill a real managed tmux session during a Claude-backed brainstorm/plan/execute/artifacts/finalize run, observe the marker through `hive status`, let the daemon clear it, and verify repeated cooled retries eventually advance after the external fault is repaired.
...
28. **Finalize unpushed-commit auto-retry is unit-pinned but not live-smoked.** The healer change makes `Hive::Daemon::StaleAgentHealer` clear `8-finalize` `ERROR reason=unpushed_commits` rows when no task lock is live, using the universal unbounded cooldown policy so normal daemon dispatch can rerun finalize's clean-exit, auth, and push path. `test/unit/daemon/stale_agent_healer_test.rb` covers the retryable marker, live-lock skip, non-finalize skip, marker-id race guard, repeated retries beyond the former budget, and marker-clear failure logging; `test/integration/run_finalize_test.rb` covers finalize writing the marker on persistent push failure. This refresh did not find an in-tree artifact showing a live daemon observing such a finalized red row, clearing it, dispatching `hive finalize`, and eventually succeeding after the underlying push failure is repaired.
...
39. **3-plan error healer requeue is unit/integration-pinned but not live-smoked.** Commit `5f7ba051` introduced explicit plan reruns after agent-loss clears; current behavior applies that requeue to every successful `3-plan` `ERROR` clear with `requestor=healer` / `trigger=error_retry` because an empty markerless `plan.md` otherwise classifies back to `:error`. `heal_requeue_failed` distinguishes a successful marker clear from a failed queue write. This refresh did not find an in-tree live artifact showing a daemon observing a red `3-plan` row, writing and dispatching the queue request, then surfacing either recovered `WAITING`/`COMPLETE` or a fresh cooled-down error marker.
...
43. ~~**Brakeman registry-laundered task-log ignore is scanner-clean but manually justified.**~~ — closed 2026-07-20. `Tasks::LogsController#show` now resolves the registered project and task, while `Task#latest_log` owns the bounded filesystem read. Brakeman no longer reports the old controller sink, so its obsolete ignore was removed. Focused integration coverage now proves both an unknown project and a valid-shaped unknown task return 404 on `GET /tasks/:project/:slug/log` before a path can be read.
...
46. **The PR-only `hive digest` is source/test-pinned, but post-fix live provider and Telegram evidence remains external.** The canonical pipeline now resolves registered GitHub targets, selects pull requests in an explicit Europe/London day, fetches repository metadata plus complete PR body/raw-diff/file evidence, validates an evidence-to-fact-to-bullet ledger, redacts recognized secrets before both outbound boundaries, and emits only `hive-digest` v2. Focused tests cover DST bounds, filters, pagination/caps, repository-atomic failure, optional statistics, strict generation coverage, entity-aware MarkdownV2 splitting, a production-shaped two-project/five-PR payload delivered through a failed-later-chunk resume with every section and footer exactly once, empty/partial/failure orchestration, schema envelopes, owner-private stable-payload/next-unsent/in-flight checkpoints, bounded HTML parse recovery, and daemon retry-versus-permanent blocking. The observed 2026-07-23 incident supplies pre-fix production evidence of repeated partial delivery, but the opt-in `test/digest/e2e_test.rb` remains the place for real post-fix GitHub/agent/Telegram proof; this implementation run did not use live credentials or retain an in-tree artifact showing accepted Telegram message IDs for the corrected path. A retry currently repeats upstream resolution, collection, and generation before `Sender` loads the stable checkpoint; that does not replace or replay the stored payload, but it remains unnecessary work and an availability dependency. Blocked-date inspection and recovery also remain file-based rather than a dedicated operator command.
...
50. **Universal durable-error retry is source/test-pinned but not live-smoked.** The former probe-specific healer has been retired so `StaleAgentHealer` is the sole owner of every `ERROR` / `REVIEW_ERROR` clear, retry counter, baseline seed, and `3-plan` continuation. Source and focused tests pin the shared cooldown, global/project enable gates, temporary current-work-area deferral, marker-generation matching, unbounded retries with always-empty exhaustion fields, nil-`state_file_mtime` guard, non-expiring healer plan requests, and fresh-marker restoration after enqueue failure. This refresh did not find an in-tree artifact showing an installed daemon repeatedly observing a real Codex-auth or Claude-launch failure, waiting through a repaired external condition, and advancing without operator marker surgery.
...
`concurrent-ruby` 1.3.6 and Brakeman 8.0.4; release preparation synchronizes only the local
...
3. **Authenticated four-agent activation remains optional and unproven for the current candidate.** Offline release tests prove the canonical Hive operating policy, deterministic OpenClaw/Claude/Codex/Pi projections, exact manifest/archive digests, byte-for-byte projection identity, private exact-gem installation, and managed-web setup. `.github/workflows/live-agent-skills.yml` can still run all four authenticated native jobs and create an attested candidate-bound Check Run, but no provider credentials are configured and `release.yml` intentionally does not require it. Therefore local/hosted release gates prove packaging and invocation contracts, not real model discovery or behavior against provider services.
4. **Published OpenClaw v0.1.3 is verified, but the new canonical projection is not yet published or live-proven.** The current public `@ivankuznetsov/hive-cli` v0.1.3 payload completed ClawHub static, Skillspector, ClawScan, moderation, exact-version inspection, and clean-install byte comparison; its immutable version ID is `k9755za2qn954070sytzmw4e958awza0`, fingerprint `c172cc4a4b71ec712512c3ae64ba0c92f36d1432e29096df7b08ddb37b8357f0`, and published `SKILL.md` SHA-256 `f7a0981a6058a7b5e7fa0a385589edb558905cca295fa041c673d46a40395202`. This branch replaces the checked-in source with the generated projection from `skills/hive/`; that new payload has not been published, scanned, or invoked through a real installed OpenClaw. `hive doctor` treats the older ClawHub payload as stale when canonical provenance is absent. A future separately authorized skill publication must choose a new immutable skill version. Shortcut listings remain intentionally absent.
