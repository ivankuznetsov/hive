...
| `agent_git_gate_test.rb`, `managed_git_test.rb`, `patrol_fix/agent_git_isolation_test.rb` | `Hive::AgentGitGate` — clean-loadable immutable read/observation/materialization/publication/private-metadata values; closed operations including strict submodule-aware status and raw changed-path metadata; unknown-argument refusal; hook/fsmonitor/diff/filter/config/environment helper suppression; absolute credential-helper selection; repository-local executable-config refusal including upload-pack hooks; forbidden transports; real bare-remote expected-absence and expected-OID leases; independently observed before/after receipts without target leakage; ref-movement refusal; bounded reads, streaming gitlink discovery beyond 64 KiB of ordinary index entries, and remote-call deadlines with process-group cleanup; root-confined detached exact materialization/reuse; ownership-checked force-discard of dirty disposable materializations; private Git preparation, replace-free authoritative ancestry proof, no-`FETCH_HEAD` exact import, and transactional clean-descendant adoption; and real bubblewrap denial of linked/regular common-dir, ref, hook, index, pointer, global, XDG, and absent external config writes while private commits remain adoptable; detached read-only Review startup; controller alternates parent-symlink refusal; and ordinary worktree-redirection refusal. |
