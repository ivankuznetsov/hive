...
bundle exec rake test:all_users_job_schema_migration
...
| `managed_directory_test.rb`, `modules/migration/{bounded_file_inventory,evidence_store,shadow_comparator,shadow_decision_migration,patrols}_test.rb`, `refactor_patrol/{job_store_files,job_query_index,job_store,registered_project_migration_coordinator,installed_job_schema_migration,installed_users_job_schema_migration,all_users_authority,install_wide_retry_service,job_store_generation_cutover,job_schema_restore}_test.rb`, `commands/{daemon,refactor_patrol_candidate_migration,refactor_patrol_schema_restore}_test.rb`, `integration/{cli_version,refactor_patrol_all_users_migration,refactor_patrol_command,module_migration}_test.rb` | Patrol migration storage — descriptor-relative no-follow reads, locks, directory traversal, atomic directory-to-tombstone exchange, and exact transaction-bound restore; linked managed-component, file, and lock refusal; filesystem-order-independent bounded inventories; descriptor-confined v3 job/index/quarantine/lock I/O; atomic pre-lock enforcement of the 8,192-job capacity; snapshot-and-count-bound compact completion receipts; source/archive/replacement-digest-bound restart checkpoints; deterministic conversion-occurrence reconstruction and exact live-terminal-or-retirement-fence reconciliation before completion, including retained terminal proof when the bounded fence inventory is full; v3-only normal runtime admission with no constructor or CLI fallback; bounded NSS discovery over a strict bounded JSON process wire; root-owned immutable candidate custody; canonical child receipt transport at the exact 2-MiB body limit with an independent 2-KiB stderr cap; fixed identity-free root summaries whose 4,096-entry maximum remains below 4 MiB; six near-maximum receipts reaching the last profile without project-row checkpoint growth; complete per-profile registered-project sweeps across real distinct Linux UIDs with default, `HIVE_HOME`, and split-XDG roots (temporary NSS accounts on the isolated CI VM, a subordinate-id user namespace locally), including two initial projects per inactive user and a project registered later by another user; exact uid/gid/supplementary-group drop and user-owned outputs; canonical root binding plus cross-user exact, cross-kind, and ancestor/descendant collision refusal with full ancestor-custody revalidation; contradictory receipt and secret-bearing diagnostic refusal; exact-duplicate state-root fanout; registry-digest rescan on every hourly profile re-entry; explicit package/all-user activation; root-owned systemd/launchd hourly retry installation; released-format v2 conversion; corrupt/permission failure isolation and hourly host/per-user retry; candidate daemon fencing and readiness acknowledgement; and full-v3 quarantine before exact v2 reactivation. |
