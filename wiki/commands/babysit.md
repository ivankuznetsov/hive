...
The `gh api` option scanner mirrors pflag short-cluster value handling for API shorthands: `-iX POST` and `-if body=hi` skip like their separate-option forms, while explicit GET scalar query forms such as `-iX GET ... -if state=open` still pass.
