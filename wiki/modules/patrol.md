...
| `Hive::Patrol::StateStore` | `lib/hive/patrol/state_store.rb` | Defines the ordinary-patrol collections, is the sole ordinary `EffectGateway` composition root, and exposes the ordinary product recovery API over the shared occurrence journal. Its full-cycle lock is also the nonblocking daemon admission fence: recovery and reservation cannot adopt a manual worker's occurrence, while a manual mutating cycle refuses an already-reserved daemon occurrence. Read-only dry runs do not allocate journal occurrences. Feature maps use one digest-bound batch effect whose local writes can be reconciled or replayed after a partial failure, so repository size does not consume the occurrence effect budget. Fingerprint writes require the configured gateway and persist the exact canonical set/delete operation in the occurrence intent. Before any fingerprint read or suppression decision, recovery walks every recovery-active predecessor capture, reconciles or retry-safely redispatches that exact operation, and rejects multiple nonterminal predecessors for one fingerprint. A pending local attempt first adopts an exact patch receipt when present; otherwise it records a failed interrupted attempt only after `Fixer` proves the deterministic checkout disposable. Patch JSON is a subordinate result of that attempt and does not mint a nested state effect. It also projects typed publication outbox entries into one immutable fingerprint binding, writing the binding before tuple acknowledgement so crash-before-ack replay is an exact no-op. Fingerprints retain PR mapping only; the removed effect-intent maps are not a parallel retry authority. |
...
An ordinary local `attempt` remains non-retry-safe at the generic gateway.
Before a recovered cycle can continue, the ordinary product owner either
adopts its exact persisted patch or asks `Fixer` to prove that the deterministic
checkout contains no work to preserve. Only the latter proof may convert the
uncertain effect into a failed `interrupted_fix_attempt` receipt. If any effect
still cannot settle, `PatrolScheduler` retains the recovery-active occurrence
and emits a blocked diagnostic instead of asking strict finalization to raise
through the daemon tick.
...
- Agent prompts treat findings and recommendations as data; validation commands come only from project config. Each admitted finding retains its exact reviewed target SHA as provenance. Before the fixer agent starts, Hive checks out the fresh current base and, when that SHA has advanced, verifies every retained snippet against its reviewed line, then rebinds it to the same current line or one unique current occurrence. Redundant anchors may disappear only when at least one current anchor remains for every evidence role; a missing role, oversized source, or ambiguous relocation becomes a fail-closed `stale_evidence` outcome. This lets unrelated formatting and implementation movement survive without treating a moving default branch as proof that the defect vanished. Hive then runs the configured command as a clean baseline and refuses a failing baseline or one that mutates the checkout.
