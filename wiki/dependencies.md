...
tools. The v0.4.0 release-prep checkout is `0.4.0`: `lib/hive.rb`, root
`Gemfile.lock`, and `web/Gemfile.lock` all pin the local path gem as
`hive-cli (0.4.0)`. The release-prep change keeps both lockfiles synchronized
...
2.7.2, and the current local path gem as `hive-cli (0.4.0)`.
