...
| `cli_version_test.rb` | `bin/hive` wrapper contract — top-level `--version`, leading JSON relocation for status (`--json` and `--json=false` before the subcommand), and command-local help after option-bearing invocations (`hive approve --from 2-brainstorm --help`) print usage instead of validating the command arguments. |
...
| `cli_usage_error_json_test.rb` | `bin/hive` pre-dispatch usage errors — bad `run`, `approve`, and `markers clear` invocations under `--json` emit command-specific error envelopes and exit 64, including leading Thor-truthy class-option forms such as `hive --json=true run` and `hive --json=t run`. |
