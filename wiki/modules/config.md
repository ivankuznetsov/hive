...
3. **`validate_review_adhoc!`** — `review.adhoc` must be Hash-shaped; `review.adhoc.reviewers` must be nil or an Array of the same reviewer-entry shape as `review.reviewers`; `review.adhoc.fix` must be boolean. (The block-level `review.adhoc.permissions` rejection lives in the permissions validator — see item 8 — not here.)
