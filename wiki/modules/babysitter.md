...
- Dry-run is best-effort because absolute-path binary invocations can bypass the PATH overlay. Within the overlay, the `git` / `gh` stubs are default-deny: they strip leading global options, skip unknown commands, and only pass through known read-only commands to the real binary. `gh api` passes through only when it has no method and no field/body flags, or when the method is explicitly GET without `--input`; field flags such as `-f`, `-F`, `--raw-field`, and `--field` make a no-method call skip because the GitHub CLI can treat them as write payloads, and `--input` always skips because it supplies a request body. `git config` only passes through for read forms (`--get`, `--get-all`, `--list`).
