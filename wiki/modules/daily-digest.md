...
history, and PR owner observations remain source evidence. The daily store does
...
The record schema carries a monotonic sequence and `local_date`, persisted
...
fingerprints. Material facts include task creation, durable stage/state
...
Recovery retries only unresolved sources. A resolved gap and recovered facts
are an idempotent amendment, visible in later CLI/Web reads. Recovery never
changes Telegram outcome or triggers a second recap.
