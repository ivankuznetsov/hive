...
| `cli_usage_error_json_test.rb` | `bin/hive` pre-dispatch usage errors under `--json` — representative wrapper mappings for missing required targets (`run`, `approve`, `markers`), workflow verb `hive-stage-action` envelopes (`brainstorm`), and `prune` usage errors before command object dispatch. |
