...
- `test/unit/babysitter/dry_run_env_test.rb` includes PTY-backed regressions proving `git log` passthrough forces `--no-pager` and identifier-less `gh run view` / `gh workflow view` passthrough forces `GH_PROMPT_DISABLED=1` without opening a selector.
