...
`PROJECT_PATH` defaults to `Dir.pwd`. `--force` skips the clean-tree check. `--json` emits a single `hive-init.v1` success document on stdout with project metadata and the resolved prompt answers. If Thor rejects an invalid positional shape before init runs, it emits the schema's `ErrorPayload` (`error_kind: usage`, exit 64); later init precondition failures retain their legacy stderr contract. `--workflow NAME` selects the project default workflow and is validated against `Hive::Workflows::Registry`; unknown names fail before disk writes and list valid names.
