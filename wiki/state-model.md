...
`hive status --json` exposes two task timestamps from this layout: `mtime` is the current stage state-file mtime (or the folder mtime fallback when the state file is missing), serialized for the public payload at whole-second ISO8601 precision, while `folder_mtime` is always the task folder's own `File.mtime`. Daemon edit-resume decisions use the same state-file mtime concept but prefer a local `File.mtime(state_file)` re-stat in `Hive::Daemon::StatusConsumer` when the file exists, so the persisted dispatch-baseline comparison keeps subsecond precision; consumers that need directory-level aging can use `folder_mtime` without re-walking the filesystem. See [[commands/status]] and [[modules/daemon]].
