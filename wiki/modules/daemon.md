...
| `Hive::Daemon::RefactorPatrolScheduler` | `lib/hive/daemon/refactor_patrol_scheduler.rb` | Exposes oldest-first discovery/action candidates, validates exact registration and repository ownership, pins new discovery to the freshly fetched committed default branch, reuses a partial job's durable `analysis_sha`, claims discovery with generation/liveness evidence, emits job-bound result paths, and checkpoints only matching schema-valid v4 completion envelopes. Architecture review/fix shares ordinary Patrol's mode-derived daily launch allowance and project-wide lock; token totals remain telemetry. Retryable discovery normally uses a 60-second backoff, `daily_agent_spawn_limit` waits for the next UTC day, and provider `token_limit`/`turn_limit`, revoked authority, and retryable action failures use the shared fixed one-hour cooldown. |
