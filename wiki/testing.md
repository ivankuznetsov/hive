...
| `bot/{brainstorm_parser,brainstorm_answer_writer}_test.rb`, `commands/answer_test.rb` | Literal brainstorm answer boundary — physical slot order across rounds, normalized fingerprints, missing-header repair, exact ordinal writes when source numbers repeat, unique relocation, duplicate/missing match refusal, stable generation and input-epoch fencing, moved-stage/folder refusal without source recreation, identical retry idempotency, different-answer conflict, creation-disabled lock contention, 64 KiB UTF-8 stdin and shell-metacharacter literalness, schema-valid closed outcomes/errors, and final-slot completion without direct dispatch. |
