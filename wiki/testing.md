...
| `cli_test.rb` | `Hive::CLI` — command delegation and option threading for the Thor surface, including `hive workflow new`, publish-only `--version` / `--dry-run`, `hive generate-name` lookup scoping, and internal archive recovery flags. |
| `deny_patterns_test.rb`, `secret_patterns_test.rb` | Versioned honeycomb security rules, stable rule ids, binary-safe location-only findings, false-positive boundaries, and legacy secret scan/redaction compatibility. |
| `honeycomb/{manifest,package_builder,permission_summary,preflight,submission}_test.rb`, `commands/workflow_publish_test.rb` | Honeycomb package rebasing/determinism, content digests, permission/dependency disclosure, safe preflight, direct-or-fork submission argv/order, human/JSON command contracts, and the dry-run remote-side-effect boundary. All GitHub paths are fake-backed. |
...
| `workflow_publish_cli_test.rb` | Real `bin/hive workflow publish ... --dry-run` routing against a temporary initialized project, including `hive-workflow-publish.v1` validation and proof that auth/git/gh submission binaries are never invoked. |
