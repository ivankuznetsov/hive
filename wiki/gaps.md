...
15. **Babysitter dry-run default-deny stubs are unit-pinned but not live-agent-smoked.** Commit `889b0b65` changed `bin/hive-babysitter-stub-git` and `bin/hive-babysitter-stub-gh` from mutating-command blocklists to read-only allowlists, and commit `fc8e7739` tightened `gh api` classification so payload flags (`-f`, `-F`, `--raw-field`, `--field`, `--input`) skip as implicit writes unless the method is explicitly GET. The 2026-06-03 patrol follow-up keeps explicit GET field flags as query parameters but also pins `--input` as skipped even with explicit GET because it supplies a request body. `test/unit/babysitter/dry_run_env_test.rb` verifies global-option stripping, unknown-command skips, mutating-command skips, read-only passthrough, and the implicit-POST/body payload guard with recording fake binaries. No artifact was found showing a full `hive babysit --once PROJECT --dry-run` live-agent run after these dry-run stub changes.
16. **Bot suppress-while-answering behavior is unit-pinned but not live-smoked.** Commit `b293c7a3` added `ConversationStore#active_for_slug?` and injects the store into `NotificationDispatcher` so proactive `needs_input` pushes are skipped while any chat has a non-expired answer conversation for the slug. `test/unit/bot/conversation_store_test.rb` and `test/unit/bot/notification_dispatcher_test.rb` cover the TTL, same-slug suppression, different-slug non-suppression, and recovery/error non-suppression contracts. No artifact was found showing a live Telegram bot plus daemon WAITING-flap reproduction after the change.
