...
15. **Babysitter dry-run default-deny stubs are unit-pinned but not live-agent-smoked.** Commit `889b0b65` changed `bin/hive-babysitter-stub-git` and `bin/hive-babysitter-stub-gh` from mutating-command blocklists to read-only allowlists. `test/unit/babysitter/dry_run_env_test.rb` verifies global-option stripping, unknown-command skips, mutating-command skips, and read-only passthrough with recording fake binaries. No artifact was found showing a full `hive babysit --once PROJECT --dry-run` live-agent run after the default-deny change.
