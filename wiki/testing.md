...
| `commands/digest_test.rb` | `Hive::Commands::Digest` — strict `YYYY-MM-DD` parsing, default runner invocation, dry-run message output, success-only `hive-digest` JSON, merged-PR dispatch, and `hive-merged-pr-digest` envelope validation. |
| `commands/pairing_test.rb` | `Hive::Commands::Pairing` — list/approve JSON envelopes, invalid arguments, unknown/expired codes, allowlist mutation, live-bot reload signaling, approval-notice queueing, and failure behavior before code consumption. |
...
| `digest/categorizer_test.rb`, `digest/renderer_test.rb`, `digest/run_test.rb`, `digest/sender_test.rb`, `digest/merged_pr/*_test.rb` | Digest generation/delivery — model JSON mapping and fallbacks, prompt rendering with PR bodies, Telegram MarkdownV2 escaping/category ordering, empty/success/failed-notice orchestration, pending-pairing reminders, dry-run token bypass, chat-id resolution, Telegram send arguments through an injected client, and merged-PR repo resolution/collection/Hive matching/rendering. These are unit seams; no real agent, GitHub, or Telegram Bot API call is exercised. |
