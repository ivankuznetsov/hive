...
| `/autofix <id\|slug>` | Resolves the current status-v4 row and submits one generation-fenced `hive retry manual` intent. It never clears a marker, selects a stage verb, or bypasses an unexpired cooldown. Rows without a current retry projection show details/refresh guidance instead. |
...
- Recovery rows: `Autofix` appears only when the exact status-v4 `retry` projection supplies a current generation and the record is not abandoned. The callback carries that generation and submits one `hive retry manual` intent; cooldown and capacity remain coordinator/dispatcher concerns. Manual-only or projection-less rows show `Show details`. Legacy `Clear and retry` callbacks without a generation are rejected with a refresh prompt instead of reviving the marker-clear path.
