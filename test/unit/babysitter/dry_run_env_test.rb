...
class BabysitterDryRunEnvTest < Minitest::Test
  ...
  def test_with_env_pins_real_binaries_against_command_local_overrides
    with_tmp_dir do |dir|
      ...
      assert_includes real_invocations, "git -c core.fsmonitor=false -c core.askPass= --no-pager status --short"
  ...
  def expected_real_invocation(binary, *args)
    ...
    "real-git #{([ "-c", "core.fsmonitor=false", "-c", "core.askPass=", "--no-pager" ] + passthrough).join(' ')}"
  end
