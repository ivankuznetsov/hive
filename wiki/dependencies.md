...
tools. The v0.5.2 release-prep checkout is `0.5.2`: `lib/hive.rb`, root
`Gemfile.lock`, and `web/Gemfile.lock` all pin the local path gem as
`hive-cli (0.5.2)`. The release-prep change keeps both lockfiles synchronized
...
2.7.2, and the current local path gem as `hive-cli (0.5.2)`.
