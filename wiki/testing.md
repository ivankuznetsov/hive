...
Queued commit `e1c41ea0` refreshes only that ignore entry's fingerprint and
note after the controller switched to registered `Tasks::BaseController`
lookup; it does not widen the trust boundary or change runtime code.
...
Queued commits `d28377b2` and patch-equivalent `eb8f6181` change the final
browser wait to target that durable PR-gate result rather than the transient
`execute` badge: a fast daemon may
