...
- `test/unit/commands/status_test.rb` — status row collection, vanished-folder and transient duplicate stage-move races, genuine collision preservation, corrupted finalize rows, legacy dir warnings, live task-lock action override, `folder_mtime` JSON emission, old-archive hiding, and archive-mode listing.
