...
46. **The PR-only `hive digest` is source/test-pinned, but live provider and Telegram evidence remains external.** The canonical pipeline now resolves registered GitHub targets, selects pull requests in an explicit Europe/London day, fetches repository metadata plus complete PR body/raw-diff/file evidence, validates an evidence-to-fact-to-bullet ledger, redacts recognized secrets before both outbound boundaries, and emits only `hive-digest` v2. Focused tests cover DST bounds, filters, pagination/caps, repository-atomic failure, optional statistics, strict generation coverage, MarkdownV2 splitting, empty/partial/failure orchestration, schema envelopes, and daemon cursor/backoff. The opt-in `test/digest/e2e_test.rb` remains the place for real GitHub/agent/Telegram proof, but this implementation run did not use live credentials or retain an in-tree artifact showing accepted Telegram message IDs for the new PR-only path. The rare at-least-once duplication risk after a mid-stream Telegram failure also remains intentionally unmitigated across processes.
