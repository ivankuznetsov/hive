...
| `agent_test.rb`, `agent_profile_test.rb`, `agent_profiles_test.rb`, `spawn_agent_test.rb`, `claude_cli_flags_test.rb` | Agent spawning and model routing — legacy no-stage Claude argv, independent exact/coarse precedence, native Claude/Codex controls, custom-profile compatibility, unsupported capability errors, explicit raw-flag suppression, and absence of cross-profile flags. |
...
| `digest/categorizer_test.rb`, `digest/renderer_test.rb`, `digest/run_test.rb`, `digest/sender_test.rb` | Digest generation/delivery — global `models.digest` argv routing, model JSON mapping and fallbacks, prompt rendering with PR bodies, Telegram MarkdownV2 escaping/category ordering, empty/success/failed-notice orchestration, dry-run token bypass, chat-id resolution, and Telegram send arguments through an injected client. These are unit seams; no real agent or Telegram Bot API call is exercised. |
