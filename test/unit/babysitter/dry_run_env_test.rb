...
class BabysitterDryRunEnvTest < Minitest::Test
  ...
  def test_stubs_refuse_hard_linked_skip_log
    with_tmp_dir do |dir|
      target = File.join(dir, "target.log")
      link = File.join(dir, "skipped.log")
      File.write(target, "existing\n")
      ...
      assert gh_status.success?, gh_err
      assert_equal "existing\n", File.read(target)
      assert_includes git_err, "[dry-run] failed to write skip log #{link}: dry-run skip log has multiple hard links"
      assert_includes gh_err, "[dry-run] failed to write skip log #{link}: dry-run skip log has multiple hard links"
    end
  end
