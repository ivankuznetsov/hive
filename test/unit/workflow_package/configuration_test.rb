...
class WorkflowPackageConfigurationTest < Minitest::Test
  ...
  def test_apply_preserves_archive_visibility_retention_and_result_contract
    declared_result = Hive::Workflow::Result.new(
      kind: :document,
      primary_artifact: "article.md",
      capabilities: %i[media supporting_artifacts]
    )
    source = Hive::Workflow.new(
      id: workflow.id,
    ...
    assert_same declared_result, mapped.result
  end
