...
`Hive::Stages::Resolver.resolve(task, descriptor:)` dispatches on each resolved stage's internal execution-strategy key, `Workflow::Stage#execution_strategy` — never on workflow identity and never on a global stage-name table. The key is derived at descriptor construction: an explicit per-stage `runner:` pin wins (a Ruby-descriptor-only field the project-YAML parser never reads), otherwise the primitive `kind:` maps through `Workflow::GENERIC_KIND_STRATEGIES` (`agent → :agent`, `council → :council`, `controller → :controller`); every other kind derives nil. `Workflows::Coding::DESCRIPTOR` pins its nine bespoke runners with `runner:` keys, and `Workflows::PatrolFix::DESCRIPTOR` pins every stage (including terminal `done`) to its controller runner. `Resolver::RUNNERS` is keyed by strategy and lazy-requires each runner ([[stages/agent]] for `:agent`, [[stages/council]] for `:council`). Because dispatch reads only the resolved stage's own declaration, a non-coding stage that shares a name with a coding stage (e.g. content's terminal `done` agent) resolves by its own kind — no `coding_id?` guard exists. A nil strategy raises `Hive::StageError` with `no runner for stage <name>`.
