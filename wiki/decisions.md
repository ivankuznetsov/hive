...
**Decision:** After a successful fix-agent spawn, Hive checks the worktree. If the worktree was already dirty before the fix agent ran, Hive refuses to proceed with `REVIEW_ERROR phase=fix reason=fix_dirty_worktree`; pre-existing manual changes are never bundled into an autonomous fix commit. If the fix agent started from a clean tree, exited successfully, and left the worktree dirty, Hive stages all resulting worktree edits and creates a fallback commit with the same rollback-rate trailer block required of fix-agent-authored commits: `Hive-Task-Slug`, `Hive-Fix-Pass`, `Hive-Fix-Findings`, `Hive-Triage-Bias`, `Hive-Reviewer-Sources`, and `Hive-Fix-Phase: fix`. The current implementation invokes `git -c commit.gpgsign=false commit` so unattended local runs do not hang on signing prompts; repositories that require signed commits need a follow-up signing policy rather than silent unsigned fallback commits.
**Consequences:** Successful fix-agent edits no longer turn into repeated dirty-worktree recovery loops, and `hive metrics rollback-rate` can treat fallback commits exactly like agent-authored fix commits. The rollback path is the normal one for fix commits: locate the trailer-bearing commit (for example by `Hive-Task-Slug` / `Hive-Fix-Pass`) and `git revert <sha>` on the feature branch. The safety envelope is deliberately layered: the pre-fix dirty guard protects unrelated local edits, protected-file hashing keeps the agent from rewriting Hive orchestration files, ADR-020's fix guardrail scans the resulting commit diff before another review pass, and the trailers preserve attribution. Remaining risk: `git add -A` stages every fix-agent-produced path, and unsigned fallback commits can be rejected by signed-commit branch protection until the signing policy is configurable.
