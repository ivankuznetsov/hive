...
| `ready_to_run` | Dispatch a generic workflow stage once. A stage transition discards the previous stage's persisted mtime baseline so its successor remains eligible. An unchanged state-file mtime after dispatch is surfaced as `markerless_stalled`, then atomically converted to `ERROR reason=agent_exited_without_terminal_marker` for ordinary coordinated recovery. After a daemon restart, an older current mtime can recover once only when the baseline was loaded from the persisted store; same-process observations never gain that recovery authority. |
