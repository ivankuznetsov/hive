...
| `artifact_firewall_test.rb`, `protected_files_test.rb`, `secret_patterns_test.rb`, `agent_profile_modes_test.rb`, `claude_launcher_test.rb`, and affected `stages/*` tests | `Hive::ArtifactFirewall` — immutable manifest/snapshot/report contracts; no-follow add/change/delete/symlink/directory/mode/parent classification; descriptor-bound captured bytes and baseline identity; non-empty regular required outputs inside realpath-checked writable roots; immutable in-memory captures; bounded injectable redaction; snapshot/report binding; atomic verified restoration with parent-substitution refusal, including exceptional spawn exits; non-recursive and unreconstructable failure; symlink rejection in headless/tmux output polling; exact execute/open-PR/finalize/review/managed-worktree marker and result adapters; clean loading; and the production `ProtectedFiles` bypass guard. |
