...
| `Capability`, `Context` | Generate one-time launch authority, authenticate the exact worker process/task/stage, revalidate generation at the mutation boundary, expose the immutable admitted route plus process-local compatibility projections, and provide one inherited bounded diagnostic writer after transport variables are scrubbed. |
