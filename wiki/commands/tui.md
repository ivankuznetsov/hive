...
| `F` | run `hive finalize` |
| `a` | run `hive archive` |
| `Enter` | from left pane: focus right pane. From right pane: perform the row's contextual action: input editor on `needs_input`, log tail on `agent_running` and kill-class evidence, red-status detail on review/error rows, browse-first handling for legacy review-stale repairs, and suggested-command dispatch for ready rows. Autofix in red-status detail submits a generation-fenced coordinator intent and never clears a marker. |
...
If `claude_pid_alive == false`, the liveness observer terminalizes/reports the attempt. The row follows its durable retry projection; a verb key cannot bypass cooldown or create an unfenced replacement.
...
`Hive::Tui::Snapshot::Row` carries `slug`, `id`, `display_name`, `pr_url`, `mtime`, `folder_mtime`, and the optional journal-derived `retry` object from status JSON. The task list hides the slug in favor of the id/name columns, using the slug as the name fallback when display generation has not succeeded or a legacy task has not been backfilled. Detail views keep the slug visible beside `#id display_name`. Filtering still matches the slug and now also matches display name and stringified id. Retry fields are copied unchanged; snapshot code does not calculate state or deadlines.
...
Red rows show bounded diagnostic evidence plus the exact optional status-v4
`retry` projection. The detail view answers why the row is red, lists the
evidence artifacts/guidance, and shows retry state, count, and absolute deadline
without reconstructing them from marker attrs.

`cooldown` remains visibly cooling and cannot render as ready. `running` shows
the current fenced successor; `abandoned` shows the operator audit metadata and
has no automatic dispatch affordance. A `ready` row is still capacity-gated by
the normal daemon.

For review/error recovery, Enter dispatches one coordinator intent:

```text
hive retry manual <slug> --project <project> --generation <N>
```

The command may wake a due record but cannot bypass an unexpired cooldown.
After repairing the underlying condition, an operator uses `hive retry repair`
with actor/reason/generation; abandonment and re-arm use the matching audited
actions. TUI recovery contains no marker-clear plus stage-run sequence. Browse
and edit gestures for review escalation files remain available, but retry
authorization still belongs to the coordinator.

Snapshot polling keeps the view honest. If the row disappears, recovers, changes
generation, or receives a replacement retry projection, the detail/grid state is
refreshed from the same status payload; the TUI never maintains a private retry
counter or deadline.
...
- **SIGHUP:** trapped at boot in `App.run_charm`; the trap calls `runner.send(Messages::TERMINATE_REQUESTED)`, which the runner picks up at the top of the next loop tick. Update returns `Bubbletea.quit` so the runner exits cleanly. Cleanup runs in `App.run_charm`'s `ensure` (kill the polling thread, stop StateSource, restore previous HUP/WINCH handlers, `SubprocessRegistry.kill_inflight!`, reap any legacy background workers). All setup runs inside the same `begin`, so constructor failure still reaches nil-guarded cleanup.
- **Crash-time cleanup:** there is no `at_exit` hook. Workflow-verb children are spawned with `pgroup: true` and intentionally **detached** — `dispatch_background` never registers them with `SubprocessRegistry`, and the registry's `kill_inflight!` is called only from `App.run_charm`'s normal-exit `ensure` block (not from `at_exit`). A signal that bypasses that ensure (`SIGKILL` of the TUI, kernel OOM kill, etc.) leaves the children running. That is the design — long-running background agents outlive an interrupted dashboard so the user can re-attach with `hive tui`; any eventual terminal observation is handled by durable attempt reconciliation and the retry coordinator.
