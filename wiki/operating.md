...
locations. Each version probe closes stdin and is bounded by a five-second
TERM deadline followed by a one-second KILL grace; installations whose
`timeout` lacks kill-after support use the built-in process-group watchdog.
Use `HIVE_BIN_OVERRIDE` for a custom Hive CLI install path. RubyGems
