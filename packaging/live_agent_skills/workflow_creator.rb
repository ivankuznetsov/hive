...
module HiveLiveAgentProof
  ...
  WORKFLOW_CREATOR_GRAPH = {
    "valid" => true,
    "stages" => %w[research draft approval].freeze,
    "automatic_edges" => [ %w[research draft].freeze, %w[draft approval].freeze ].freeze,
    "human_outcomes" => [
      { "stage" => "approval", "name" => "approve", "complete" => true,
        "artifact" => "draft.md", "to" => nil }.freeze,
      { "stage" => "approval", "name" => "reject", "complete" => false,
        "artifact" => nil, "to" => "draft" }.freeze
    ].freeze
  }.freeze
  WORKFLOW_CREATOR_TASK = {
    "slug" => WORKFLOW_CREATOR_TASK_SLUG, "workflow" => "editorial",
