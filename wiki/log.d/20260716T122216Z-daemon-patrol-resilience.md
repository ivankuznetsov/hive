...
**Result:** One bad registered-project config is isolated by existing daemon gates instead of terminating the service and its active workers. Patrol no longer spends reviewer/fixer quota when it cannot validate a fix, while `--dry-run` remains available for review-only scans. A patrol-only CONFIG exit bypasses controller-level project drops and enters scheduler backoff without blocking ordinary tasks. Patch JSON now records `base_sha` and `head_sha`, and attempt diffstats no longer depend on a stale local default branch.
