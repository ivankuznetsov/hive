...
class WorkflowCreatorValuesTest < Minitest::Test
  ...
  def test_load_captured_core_and_invocation_paths_resist_post_load_replacement
    script = <<~'RUBY'
      ...
      BasicObject.class_eval { define_method(:__send__) { |*| raise("intercepted BasicObject#__send__") } }
      {
        ...
        String => %i[initialize encoding valid_encoding? bytesize encode each_byte byteslice << <=> % hash eql? ==],
  ...
  def test_production_file_stays_within_u1a1vi_exact_caps
    ...
    required_callables = %i[
      ...
      escape_byte append! charge! seal construct_snapshot
    ]
  ...
  def reference_canonical(value)
    normalized = reference_value(value)
    "#{JSON.generate(normalized)}\n"
  end

  def reference_value(value)
    if value.instance_of?(Hash)
      value.map { |key, nested| [key.encode(Encoding::UTF_8), reference_value(nested)] }
           .sort_by { |key, _nested| key.b }
           .to_h
    elsif value.instance_of?(Array)
      value.map { |nested| reference_value(nested) }
  ...
  def proc_block_kind(node)
    ...
    return :proc if identifiers.include?("lambda") || identifiers.include?("proc")
  end
end
