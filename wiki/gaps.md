...
- The built-in `bench` descriptor, packaged stage instructions, self-contained
  ...
  The former release-packaging uncertainty is closed at the source/tag boundary:
  the changelog records the built-in workflow in v0.4.2, the local v0.6.1 tag
  contains its descriptor and runtime, and `hive.gemspec` includes the template
  tree in the gem payload. No checked-in artifact proves a fresh standalone
  project completed a live paid-provider campaign, and the local tag/payload
  inspection does not independently prove a public registry download. Honeycomb
...
12. ~~**Local git index health affected the 2026-06-01 wiki refresh.**~~ Closed for the current checkout on 2026-07-19: `git status --short`, unstaged diff, and cached diff reads all succeed, and `plugins/zoom/CONTRIBUTING.md` is no longer indexed. The historical blob `7afa1ea39e410defd0219acecd295e3108f5f93a` is still unavailable locally, so this closes the active-checkout blocker rather than reconstructing the old missing object.
...
24. **Provider-limit classification is unit/integration-pinned but not post-fix live-smoked.** Commit `d22caf37` adds `Hive::AgentLimit` and wires headless `Hive::Agent#handle_exit` plus Claude tmux readiness, terminal-marker, and expected-output waits so provider quota/rate/usage-credit text becomes `limits reached for <agent>:` / `ERROR reason=limits_reached`. Later review-stage work stamps `REVIEW_ERROR reason=limits_reached retry_after=...` when all reviewers fail on limits, and commit `b6bba5d6` routes triage/fix phase spawn failures through the same limit helper instead of leaving provider-limit walls on terminal `triage_failed` / `fix_failed` markers. Commit `118ed2fd` narrows the old bare "limit reached/exceeded/reset" match so usage/billing/rate/time-window terms are required; healthy agent output about scroll/window/viewport/page limits should no longer classify as an account wall while real usage-credit and time-window walls should still do. `test/unit/agent_limit_test.rb`, `test/unit/agent_test.rb`, `test/unit/claude_launcher_test.rb`, `test/unit/stages/review/run_reviewers_test.rb`, `test/unit/daemon/stale_agent_healer_test.rb`, and `test/integration/run_review_test.rb` cover classifier precedence, raw-stream limit capture, headless/tmux status modes, all-reviewer limit classification, daemon cooldown clearing, triage limit vs non-limit marker behavior, and benign Claude banner/status hints. The earlier runnable-coverage concern is closed: the UI-feature and real usage/time-window tests are public methods, and `test/integration/run_review_test.rb` contains a focused fix-phase `limits_reached` regression. This refresh still found no in-tree artifact showing a fresh live headless/tmux/review run after the classifier narrowing surfacing `limits_reached` through `hive status`, the daemon, or the TUI.
...
Latest refresh (2026-07-19): the local `v0.6.4` tag points at synchronized
release source in `lib/hive.rb`, both lockfiles, README/install URLs, the
changelog, and release-facing wiki pages. Local source and tags do not by
themselves prove public signed gem/assets, Homebrew/AUR updates, or
multi-architecture hivebox images; those remain the tag-triggered `release.yml`
verification boundary. The v0.6.2 source packages quota-bounded
ordinary/architecture patrol progress, durable architecture reviewer evidence,
and detached exact-source analysis; v0.6.3 retires the locally scaffolded
Honeycomb flagships; v0.6.4 includes the hivebox composer reset and babysitter
dry-run audit-log precreation. Live post-release dogfood evidence remains open
until a published release is installed and the relevant flows complete real
runs. Historical commit `54fd3455` still provides commit-message evidence for
the native arm64 GHCR smoke against `ghcr.io/ivankuznetsov/hivebox:0.3.1`.
The former root/web Brakeman and `concurrent-ruby` mismatch is closed: both
locks resolve Brakeman 8.0.5 and `concurrent-ruby` 1.3.7. They remain
independently resolved bundles and still differ where their manifests require
it, including JSON and RuboCop versions; see [[dependencies]].
