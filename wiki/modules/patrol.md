...
| `Hive::Patrol::FindingRegistry` | `lib/hive/patrol/finding_registry.rb` | Reconciles existing finding records with merged/open fingerprint state and dismissals, suppresses terminal or same-target semantic duplicates before persistence, and supersedes an older active record only when a newer target is admitted. Exact-fingerprint and category/token indexes keep admission linear in the relevant semantic bucket, while lifecycle transitions persist the already-loaded record with one timestamp. |
