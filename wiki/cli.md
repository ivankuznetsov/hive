...
|---------|----------|-----------|------|
| `hive init [PROJECT_PATH] [--json]` | Bootstrap `.hive-state` orphan branch + worktree plus managed llm-wiki context in a git project; `--json` emits current `hive-init.v2` on success | `Hive::Commands::Init` | [[commands/init]] |
