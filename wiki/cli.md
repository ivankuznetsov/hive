...
| `hive module SUBCOMMAND [SOURCE_OR_NAME] [--json]` *(queued `071d0d71`)* | Preview-bound module install/update/enable/disable/uninstall; redacted list/inspect/status; no-repair doctor; and write-free event dry-run. The queued branch publishes lifecycle/list/status/doctor/dry-run v1 schemas but is not integrated on current default. | `Hive::Commands::Module` | [[commands/module]] |
