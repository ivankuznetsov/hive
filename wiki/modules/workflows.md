...
- `Hive::Workflow::Metadata` — optional frozen publication values: semantic
  `version`, `author`, `description`, semantic `minimum_hive_version`, optional
  resolved `readme`, and resolved explicit `assets`. The parser validates fields
  when present; the publisher enforces completeness.
...
- `shell_justification` is valid only on scoped permission maps that explicitly
  grant Bash. Reviewer and revise permission declarations retain it for
  context-aware publication preflight.
...
`hive workflow new ID` scaffolds and commits a runnable descriptor; its templates include commented publication guidance, not fake active author/version values. `hive workflow publish ID` collects stage/reviewer/revise instructions plus declared README/assets, preserves owned relative paths, rewrites references, and hashes final package bytes. See [[commands/workflow]].
