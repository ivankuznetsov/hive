...
| `cli_usage_error_json_test.rb` | `bin/hive` pre-dispatch usage errors — bad `run`, `approve`, and `markers clear` invocations under `--json` emit command-specific error envelopes and exit 64, including leading truthy class-option forms such as `hive --json=true run`. |
