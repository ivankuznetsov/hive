...
## ADR-051: OpenCode uses an invocation-owned overlay and caller-owned lifecycle

**Status:** Active
**Context:** The April 2026 OpenCode spike used `1.14.25`, found no native Compound Engineering integration, and had no simple per-spawn filesystem/config isolation contract. OpenCode `1.18.16` now exposes exact nested routes, source-backed project-config disabling, environment-controlled configuration, JSON events, and sanitized session export; Compound Engineering `3.21.4` registers OpenCode skills and commands natively.
**Decision:** Add OpenCode as an opt-in built-in profile with minimum version `1.18.16`. The provider-neutral component prepares a private config/data/cache/state overlay, performs local route/capability/auth checks, compiles the run and sanitized-export commands, normalizes captured evidence, and exposes idempotent cleanup. The caller remains responsible for spawning, streaming, timeout/cancellation, process-tree cleanup, export execution, and calling overlay cleanup in `ensure`. Hive identity remains `opencode`; requested and observed nested backend/model values remain separate. Existing defaults, councils, and fallbacks do not select OpenCode.
**Consequences:** OpenCode never inherits ambient project/global configuration or credentials unless the caller names a source. Safe permission modes use deny-first generated policy; raw trusted overrides are outside ordinary Hive configuration. Completion requires terminal-message correlation with sanitized export rather than inferring actual identity from the requested route. Component version `0.2.0` is a candidate-only boundary in this work; tagging, publishing, deploying, and downstream release remain separately authorized actions.
