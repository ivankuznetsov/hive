...
keeps the error shaped like the requested surface. Ordinary versioned commands
derive the contract from `Hive::Schemas::SCHEMA_VERSIONS` plus the published
schema's `ErrorPayload.error_kind`; aliases, multi-surface commands, and
unversioned envelopes use `Hive::CLI::JSON_USAGE_ERROR_OVERRIDES`. This covers
missing and extra positional arguments across the JSON surface, including
`forget`, `prune`, Screenote `connect`/`disconnect`, `status`, `metrics`, and
`web install`/`web status`, before the human `hive:` stderr line. Registered
schemas include `schema_version`; unversioned Screenote and web envelopes retain
their command-native shape. Workflow verbs include the `verb` extra (`pr` maps
to `open-pr`) and finding toggles include `operation`.
...
**Error envelopes.** Every full-envelope `--json` command (`status`, `run`, `approve`, `findings`, `accept-finding`, `reject-finding`, `markers clear`, `metrics rollback-rate`, `forget`, `prune`, and the workflow verbs `brainstorm` / `plan` / `develop` / `open-pr` / `review` / `artifacts` / `finalize` / `archive`) emits a command-specific error document on stdout when an error is raised. Detect failure by `payload.ok == false`. Most registered surfaces use `Hive::Schemas::ErrorEnvelope`, carrying `schema`, `schema_version`, `ok=false`, `error_class`, a closed `error_kind`, `exit_code`, and `message`; metrics retains its published error shape without `error_class`. Per-error structured extras (`candidates` for `AmbiguousSlug`, `id` for `UnknownFinding`, `path` for `DestinationCollision`, `stage` for `FinalStageReached`) appear automatically. The wrapper derives ordinary pre-dispatch contracts from schema metadata and uses CLI-owned overrides for aliases/unversioned surfaces, so agent callers do not lose JSON just because Thor rejects argv before a handler is instantiated. `hive daemon queue --json` uses its own `hive-daemon-queue.v1` `ErrorPayload` arm for queue-command failures (`unknown_action`, `missing_request_id`, `internal`) so queue inspectors still receive one schema-specific document. `hive run --json` additionally preserves the existing dual-signal contract on `:error` / `:review_error` markers — the SuccessPayload is emitted to stdout BEFORE the `TaskInErrorState` raise, so the rescue's ErrorPayload is suppressed (one document, exit 3). `hive bench submit --json` now emits a command-shaped document for Thor argv errors; later `BenchSubmit::UsageError` failures remain stderr-only. Errors that fire before the wrapper can load Hive or inspect argv (gem-load failures, shebang errors) cannot emit JSON — those remain stderr-text + exit-code only.
