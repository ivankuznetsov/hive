...
module Hive
  module Attempts
    ...
    class Entrypoint
      def initialize(store: nil, dispatcher: nil, client: nil, config_loader: Hive::Config.method(:load))
        @store = store
      ...
      def dispatch(task:, intended_stage:, argv:, request_id: SecureRandom.uuid,
        ...
        dispatcher = @dispatcher || build_dispatcher(store, cfg)
      ...
      def build_dispatcher(store, cfg)
