  ...
  module Tui
    ...
    module Help
      BINDINGS = [
        ...
        { mode: :grid, key: "Enter",     action: :open_contextual,    description: "left pane: focus right. right pane: open contextual mode: input editor on `needs_input` (completed brainstorm answers auto-run), triage on `review_findings`, log tail on `agent_running` / `error`, recover review errors; ready rows still dispatch the suggested command" },
