...
| `agent_test.rb`, `agent_profile_test.rb` | `Hive::Agent` / `AgentProfile` — spawn/wait/timeout/SIGINT forwarding, version and capability checks, configured permission/workspace-write argv, Claude model/effort and verified safe-mode `cli_flags`, process-group cleanup for timed-out version/help probes and stdout-inheriting descendants, safe-mode non-leakage to unrelated launches, and provider-limit classification before generic exit-code / expected-output failures. |
...
| `gemspec_test.rb`, `install_script_test.rb` | RubyGem/install packaging — `base64 >= 0.2` is declared for PKCE; `hv` stays out of `spec.executables`; and repeat `install.sh` runs temporarily remove only recognizable Hive-managed `hive` wrappers before RubyGems writes its binstub, restore the previous wrapper if install/binstub verification fails, and stamp the replacement wrapper with `hive-managed: install-wrapper/v1`. |
...
| `test/unit/web/status_feed_test.rb`, `web/test/models/{board,status_broadcaster}_test.rb`, `web/test/integration/status_test.rb`, `web/test/system/{kanban_board,pipeline_flow}_test.rb` | Hive web status views and broadcasting — descriptor-ordered project/workflow board bands, unknown-stage retention, visible degraded-project/unavailable-workflow states, side-effect-free Board/Grid GET routes plus signed-preference mutation, shared native card actions, project-filter preservation, stable card focus/band scroll across reorder morphs, long-name mobile board containment, registered-project snapshots, one shared scan per poll tick, emit-on-connect for independent subscribers, volatile-field dedup that keeps `mtime`/`folder_mtime` significant, refresh-only view reconciliation including failed-idea deferred replay, card-action mutation survival, redirect-handoff suppression, first-connect de-duplication, and missed-broadcast catch-up after direct or history-restored Action Cable reconnect, poller survival after snapshot errors, `StatusBroadcaster` resubscription after a raising Turbo broadcast, and deterministic browser coverage that a competing page refresh cannot abort an in-flight mutation or preserve a duplicate-ready permanent draft. |
...
| `patrol_command_test.rb`, `unit/patrol/token_budget_test.rb`, `unit/usage_db_test.rb` | `hive patrol` — JSON envelope, dry-run behavior, language-neutral default component mapping, freshly fetched exact-remote scan checkout without moving stale local main, fail-closed remote-fetch errors before reviewer work, replacement of an unmaterializable active snapshot with a fresh cursor-zero default, globally ranked selection, immutable selection-audit recording, SHA pinning across default advancement after a cursor-zero review error, scan-state recording, inbox non-interference, retry/backoff outcomes, schema validation with fake mapper/reviewer/fixer/PR opener collaborators, shared daily tokens, separate ordinary/architecture daily launch accounting, the architecture cycle/per-agent multiplier, the durable unmetered-architecture backstop across budget instances, and patrol attribution. |
