...
- `grok` — headless via `-p <prompt>`, `--always-approve`, and `--output-format streaming-json`. Preflight accepts `XAI_API_KEY`, `GROK_CODE_XAI_API_KEY`, an explicit credential file via `GROK_AUTH_PATH`, or `auth.json` under `GROK_HOME`/`~/.grok`; device login is `grok login --device-auth`. The direct path takes precedence over `GROK_HOME`, matching the CLI and allowing one refresh-token/lock domain to be mounted into isolated runners. No add-dir or budget flag. Text events are concatenated into `final_message`; unavailable token usage stays nil. Min version `0.2.90`. `:output_file_exists`. Native skill verification is not yet available.
