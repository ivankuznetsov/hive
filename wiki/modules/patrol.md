...
| `Hive::Patrol::PrOpener` | `lib/hive/patrol/pr_opener.rb` | Fail-closed secret-scans the title, body, and exact validated diff; verifies a clean exact local head, remote base, leased push, remote head, and created/existing PR identity; records fingerprint-to-PR state; and invokes `ReviewHandoff`. The ledger mapping is persisted immediately after `gh pr create` succeeds — before post-creation reconciliation — so a lookup lag or identity-assert failure cannot orphan a real open PR. Dynamic publication diagnostics are separate from closed reason codes. |
