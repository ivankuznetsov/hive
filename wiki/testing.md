...
| `patrol_command_test.rb`, `unit/patrol/{finding_registry,fixer,reviewer}_test.rb`, `unit/patrol/token_budget_test.rb`, `unit/usage_db_test.rb` | `hive patrol` — JSON envelope, dry-run behavior, language-neutral mapping, exact-remote SHA pinning, durable active/resolved/rejected/superseded finding lifecycle, all-history semantic dedup before persistence, configured validation-key admission, clean-baseline preflight, stale-target rejection, globally ranked selection, immutable audit records, retry/backoff outcomes, schema validation, shared daily tokens, separate ordinary/architecture launch accounting, architecture multipliers/backstops, and patrol attribution. |
