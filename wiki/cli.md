...
|---------|----------|-----------|------|
| `hive init [PROJECT_PATH] [--json]` | Bootstrap `.hive-state` orphan branch + worktree plus managed llm-wiki context in a git project; `--json` emits `hive-init.v2` on success | `Hive::Commands::Init` | [[commands/init]] |
...
- `init` accepts `--force` (skip clean-tree check) and `--json` (single `hive-init.v2` success document with resolved answers and project metadata; precondition failures keep the legacy stderr + exit-code contract).
