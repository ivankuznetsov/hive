...
class MigrateTest < Minitest::Test
  ...
  def test_explicit_migrate_discards_v1_refactor_patrol_scan_progress
    with_tmp_global_config do
      with_tmp_git_repo do |dir|
        capture_io { Hive::Commands::Init.new(dir).call }
        ...
        File.write(
          progress_path,
          JSON.generate(
            "schema" => "hive-refactor-patrol-reconciler-progress",
            "schema_version" => 1
          )
        )
        cutovers = 0
        ...
        refute File.exist?(progress_path)
        assert_equal 1, cutovers
        assert_includes out, "discarded obsolete Architecture Patrol scan progress"

        second_out, = capture_io do
          migrate_command(dir, daemon_cutover: -> { flunk "unexpected cutover" }).call
        end
        refute_includes second_out, "discarded obsolete Architecture Patrol scan progress"
      end
    end
  end
