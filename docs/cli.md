...
| `hive workflow publish ID [--version VERSION] [--dry-run] [--json]` | Package and lint one project workflow as a honeycomb; dry-run stops before every remote mutation, normal mode opens a registry PR. |
...
Workflow verbs (`brainstorm`, `plan`, `develop`, `open-pr`, `review`, `artifacts`, `finalize`, `archive`, `run`, `approve`), findings triage, patrol, diagnostics, registry cleanup, workflow authoring/publication (`workflow new`, `workflow publish`), `init`, and daemon control support `--json` where documented. Workflow verbs emit `hive-stage-action`; honeycomb publication emits the registered `hive-workflow-publish.v1` success/error contract, including package digest, rule versions, portable dependencies, permission summary, lint status, review-required findings, and nullable submission fields for dry-run. Schema files live under [schemas/](../schemas/). `hive tui` rejects `--json`; legacy one-shot utilities remain text-only.
