...
Pass cap (`review.max_passes`, default 2) gates ordinary re-entry to Phase 2 — exceeding it sets `REVIEW_STALE pass=NN`. A newer CI-fix HEAD is the bounded exception: Hive admits the next pass so repair work cannot escape review. `fix-success-NN.md` binds each newly completed pass to its exact HEAD, allowing that exception to survive a daemon restart. Wall-clock cap (`review.max_wall_clock_sec`, default 5400) is checked at every phase boundary and between reviewers inside `run_reviewers`; each reviewer that accepts a `deadline:` kwarg receives the full remaining wall-clock budget, while its own `timeout_sec` remains the per-reviewer cap. Shared Claude tmux readiness waits count against the current reviewer deadline. Exceeding the outer wall-clock cap sets `REVIEW_STALE reason=wall_clock`.
...
After the fix agent returns, `Hive::Stages::Review::FixGuardrail.run!` (ADR-020 / U13) takes `git diff base..head` of the new commits and walks it once, dispatching each line to the configured pattern set. Capture forces deterministic `a/` and `b/` prefixes, so repository-local `diff.noprefix` configuration cannot hide ordinary edit, addition, or deletion paths from the scanner. File paths are extracted from every header form git emits: `---`/`+++` pairs with `a/`/`b/` (or mnemonic `c/`/`i/`/`w/`/`o/`) prefixes, pure rename/copy extended headers (`rename from <old>` / `rename to <new>` / `copy from/to` — emitted with no `---`/`+++` pair when rename detection finds a no-content-change move), and C-quoted forms (`"path\tname"`) for paths containing control characters; quoted paths are unquoted and backslash-decoded before pattern matching so `git mv docs/x.yml .github/workflows/deploy.yml` trips the same patterns as an edit. Custom `:raw_diff_header` patterns also see rename/copy extended headers, mirroring the `diff --git` and mode-change lines:
...
- `secrets_pattern_match` — retains the descriptor key `detector: :secret_patterns` for config compatibility, but delegates detection to bundled Betterleaks. Added lines are batched by filename; upstream source-aware rules own classification. Findings contain redacted snippets and exact secret hashes, preserving precise waiver matching without exposing credentials.
...
- `permission_change` — executable-bit raw diff header (`new file mode 100755`, `new mode 100755`). The finding's file comes from the `diff --git` header, because mode lines precede the `---`/`+++` pair and a pure chmod emits no pair at all (previously every such finding rendered as `permission_change: :?:`). A **new** executable under a `test/`, `tests/`, `spec/`, `specs/`, `__tests__/`, `testdata/`, or `fixture(s)/` directory is exempt (`exempt_new_file_paths`) — fake CLI stubs such as `web/test/e2e/support/codex` must be executable to shadow the real binary on PATH, and tripping on them parked clean reviews in `REVIEW_WAITING` with zero escalations. A mode flip on an existing file, or a new executable anywhere else, still trips.

Per-project override via `review.fix.guardrail.patterns_override`: `false` to disable a default; Hash to add a custom (must include `regex`). Replacement is total: a Hash under any name — including `secrets_pattern_match` — installs a plain regex detector (`detector: :regex`) and never inherits the Betterleaks dispatch. Scan raises on any descriptor whose `detector:` is neither `:regex` nor `:secret_patterns`. Tripped → `REVIEW_WAITING reason=fix_guardrail matches=N head=<sha> pass=NN` and `reviews/fix-guardrail-NN.md` written. The `head=` attribute records the worktree HEAD at the moment the guardrail tripped; the approval-on-resume path enforces it. Because decoded paths and matched snippets may contain literal control bytes (a C-quoted rename target like `".github/workflows/de\nploy.yml"` decodes to a real newline), the report writer escapes every control byte at the report boundary (`escape_control_bytes`): one Match always renders as exactly one unchecked checkbox line, so a crafted path fragment such as `\n- [x] ` can neither forge a pre-checked box nor drift the checkbox count away from the marker's recorded matches and deadlock the approval round-trip.
...
1. **All active findings are `[x]`** — partial ticks keep the pause (no fall-through to Phase 4 fix re-spawn). The retired default `dependency_lockfile_change` finding no longer requires approval on resume, unless a project explicitly reinstates that rule. Lockfile updates remain covered by CI and code review; a filename alone is not a reason to stop a fix. Retired findings still count toward the recorded match count, and the existing HEAD and clean-worktree checks still apply.
...
- **`:complete`** — `reviews/fix-success-NN.md` sentinel exists AND its mtime is ≥ `escalations-NN.md`'s mtime, OR pass `N+1` reviewer files exist. `next_pass_for` advances to `N+1`. New sentinels also record the exact completion HEAD.
...
3. If `:complete` and `fix-success-NN.md` is at least as new as the escalation artifact, the pass has no pending operator edit. Ordinary action surfaces and the daemon may submit guarded recovery. A matching bound HEAD resumes Phase 5; a different bound HEAD, or a legacy sentinel without a binding, opens one fresh reviewer pass. If `escalations-NN.md` is newer than the sentinel, the operator edit is authoritative: ordinary automatic retry remains disabled, the TUI opens that file, and the explicit `r` gesture submits the post-edit coordinator request.
