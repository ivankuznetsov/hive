...
| `commands/babysit_test.rb` | `Hive::Commands::Babysit` — lifecycle command routing, PID-file ownership handling, stale-runtime status/reload warnings, foreground restart, and detached restart re-exec into canonical `start --detach` before daemonizing. |
