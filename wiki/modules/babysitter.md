...
| `Hive::Babysitter::DryRunEnv` | `lib/hive/babysitter/dry_run_env.rb` + `bin/hive-babysitter-stub-git` / `bin/hive-babysitter-stub-gh` | PATH overlay for agent-side dry-run `git` / `gh` wrapper launchers; allowed passthrough pins the parent-resolved real binary, skipped-command audit writes use the launcher-pinned worktree skip log and refuse symlinked/non-owned/non-regular log targets, including FIFOs, before a nonblocking open while escaping control bytes without decoding argv as UTF-8, `gh` reads run with fresh empty `HOME`/`GH_CONFIG_DIR` temp dirs instead of caller/user config, and git reads exec with hermetic user/system/local config controls. The overlay dir (`.hive-babysitter-dry-run-bin/`) and the worktree-root skip log (`.babysitter-dry-run-skipped.log`, plus the `.babysitter-dry-run-plan.md` plan) are gitignored at the repo root so a stage-exit [[stages/review]] `CleanExit` never `git add -A`s them into a `scope_violation` (`:error reason=ensure_clean_on_exit_failed`); `with_env` also `rm_rf`s the overlay dir on exit, while leaving the skip log in place as the dry-run's diagnostic record. |
