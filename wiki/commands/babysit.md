...
Signature-verifying history reads are skipped too: `git log`, `git show`, and `git rev-list` reject `--show-signature` plus `--format` / `--pretty` values containing `%G`, because those reads can invoke repo-local `gpg.program`.
