...
require "hive/reviewers/runtime"
...
module Hive::AgentSupport::Codex
  ...
  class Reviewer < Hive::Reviewers::Base
    ...
    def initialize(spec, ctx, cfg: nil, stage_host: Hive::Stages::Base,
                   runtime: Hive::Reviewers::Runtime)
      super(spec, ctx)
      @cfg = cfg
    ...
    def run!(deadline: nil)
      ...
      routing_arguments = @stage_host.model_routing_arguments(
        @cfg || {}, "review_reviewers", profile,
        current: @stage_host.model_routing_current(spec)
      )
      begin
    ...
    def enforce_permission_scope_gate!(profile)
      scope = @stage_host.stage_permission_scope(
        @cfg || {}, "review.reviewers", synthetic_task, profile,
        base_add_dirs: [ ctx.task_folder ],
        **@stage_host.explicit_permission_kwargs(spec)
      )
    ...
    def run_codex_review(profile, prompt, timeout_sec, routing_arguments = nil)
      ...
      @runtime.capture(
        environment: profile.subscription_environment, argv:, chdir: ctx.worktree_path,
        timeout_sec:, timeout_message: "codex review timed out after #{timeout_sec}s"
      )
    end
    ...
    def render_prompt
      template_path = @stage_host.resolve_template_path(
        spec.fetch("prompt_template"),
        hive_state_dir: @stage_host.hive_state_dir_for_task_folder(ctx.task_folder)
      )
      tag = @stage_host.user_supplied_tag
      @stage_host.render_resolved_path(
        template_path,
        @stage_host::TemplateBindings.new(
