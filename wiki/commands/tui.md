...
`Hive::Tui::Snapshot` is the sole new-idea project authority. Each immutable snapshot freezes an ordered admission list containing only captured-healthy projects with unique exact names, plus typed causes for an empty list. Duplicate names are excluded before health is considered; when duplicates are the reason nothing is selectable, the picker identifies the bounded duplicate names and directs the operator to disambiguate the registry or use `hive forget` instead of claiming that no healthy projects exist. Loading, no registered projects, and registered-but-unhealthy projects also retain distinct picker feedback.
...
Immediately before either plain-title dispatch or rich image submission, `BubbleModel` resolves the pinned exact name through the latest snapshot already installed on the model. `disappeared`, `unhealthy`, `ambiguous`, `invalid_scope`, `selection_required`, and `no_projects` all fail closed with state-specific feedback and zero dispatch. A blocked attempt clears only stale target intent, returns to the picker with no highlight, and preserves the title cursor, composition, attachments, and staged image bytes. After a later snapshot and an explicit valid selection, retry uses the selected name and the existing success path still dispatches once and resets the composer.
...
suffixes. This is an FYI only: it does not change Enter behavior or synthesize
a recovery action.
...
- **fresh completion receipt**: `reviews/fix-success-NN.md` is at least as new as `reviews/escalations-NN.md` — the pass has no pending operator edit and Hive is recovering an interrupted post-CI/review boundary.
...
Every `error` row follows one path: grid Enter opens red-status detail, and Enter in that view submits `RecoverError` through `Hive::Recovery::API` on a background worker. The TUI has no automatic recovery scheduler and gives signal exits `130` / `137` / `143` no private clear path; automatic retry belongs exclusively to the daemon. The status column shows `ERROR exit_code=N` (or `ERROR <reason>` when no exit code is set) so the operator can read why the agent failed without leaving the grid. Per-folder local dedup is tracked on `@error_recovery_inflight`; cross-process dedup and marker occurrence safety are coordinator-owned.
...
- **Ctrl+C:** `InputDecoder` preserves byte `0x03` as Bubble Tea's `KEY_CTRL_C`; `BubbleModel` translates that framework key directly to `Messages::TERMINATE_REQUESTED` before mode-specific key routing. Ctrl+C therefore quits cleanly from the grid, overlays, and editable prompts. The dashboard's detached workflow children keep running, matching the `q` and caller-death ownership contract.
- **Ctrl+Z / SIGTSTP:** Bubble Tea owns suspend/resume of the alt-screen and raw-mode toggling.
- **SIGHUP:** trapped at boot in `App.run_charm`; the trap calls `runner.send(Messages::TERMINATE_REQUESTED)`, which the runner picks up at the top of the next loop tick. Update returns `Bubbletea.quit` so the runner exits cleanly. Cleanup runs in `App.run_charm`'s `ensure` (kill the polling thread, stop StateSource, restore the previous HUP/WINCH handlers, `SubprocessRegistry.kill_inflight!`, reap inflight operator-recovery threads). All setup (StateSource boot, `Bubbletea::Runner` construction, signal hook install, poller spawn) is performed *inside* the same `begin` so a constructor failure still hits the same nil-guarded cleanup path — the StateSource thread can no longer leak when `Bubbletea::Runner.new` raises.
