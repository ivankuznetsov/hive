...
| `honeycomb_workflow_lifecycle_test.rb` | A local immutable v2 git registry fixture drives canonical `catalog.json` plus `packages/NAME/VERSION/manifest.yml` through install, no-write dry-run, update, catalog-commit task pinning, list selected/retained dimensions, remove, and offline execution of the retained task. Unit lifecycle/store coverage also pins browser-preview source/digest baselines, first-install still-unselected checks, and removal rechecks inside the mutation lock. |
