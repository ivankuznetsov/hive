...
| `Hive::Daemon::RefactorPatrolScheduler` | `lib/hive/daemon/refactor_patrol_scheduler.rb` | Exposes oldest-first merged-PR discovery/action candidates, validates exact registration and repository ownership, reuses each partial job's durable `analysis_sha`, claims work with generation/liveness evidence, emits job-bound result paths, and checkpoints only matching schema-valid v4 completion envelopes. This post-merge work does not consume scheduled discovery allowance. The Architecture module's separate `ScheduledSliceProducer` fetches/maps current `main`, claims one stable feature slice per launch, and uses its independent Architecture allowance. |
