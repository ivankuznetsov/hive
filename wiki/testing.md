...
created: 2026-04-25
updated: 2026-09-11
tags: [test, minitest, fixtures, honeycomb, agent-skills, component-boundaries, plan-review, terminal-outcomes, release-proof, bounded-storage, daily-digest, brainstorm, suggestions]
---
...
are available and can never replace a deterministic blocking row. The
...
`test/unit/running_status_test.rb` pins the compact liveness boundary: empty,
runner-only, child-only, combined, stale/reused, malformed, transition, FIFO,
oversized, scan-truncated, daemon-stopped, producer-owned daemon runtime versus
observer runtime, row/string, schema, and 64 KiB output cases.
`test/unit/pid_file_test.rb` separately pins strict-vs-legacy identity
semantics and out-of-range PID handling.

The focused brainstorm-answering checkpoint is:

```bash
bundle exec ruby -Itest test/unit/commands/answer_test.rb
bundle exec ruby -Itest test/integration/brainstorm_answering_skill_contract_test.rb
...
The repository-aware brainstorm-suggestion checkpoint is:

```bash
bin/test test/unit/brainstorm_suggestions/*_test.rb
bin/test test/unit/commands/brainstorm_suggestion_test.rb
bin/test test/unit/daemon/brainstorm_suggestion_scheduler_test.rb
bin/test test/unit/tui/brainstorm_suggestions_test.rb
bin/test test/unit/tui/brainstorm_answers_test.rb
bin/test test/integration/brainstorm_suggestion_authority_test.rb
bin/test test/integration/run_brainstorm_test.rb
```

Run the Rails projection/action coverage from `web/`:

```bash
bundle exec ruby bin/rails test test/models/task_test.rb test/integration/tasks_test.rb
...
| `work_ledger_test.rb`, `task_journal_test.rb`, `conditions/*_test.rb`, `task_projection*_test.rb`, `task_action_conditions_test.rb` | Policy-light WorkLedger plus Hive-owned generation conditions — clean loading without workflow/condition/attempt policy; structural descriptor topology; exact cursor/hash receipts; locked, fsynced, idempotent complete JSONL append with short-write completion and partial-append rollback; generic replay duplicate/malformed refusal; strict Hive journal durability/attempt integrity; task/workflow stream and stable attempt binding; first-append reader/writer locking; complete routine folds with bounded workspace detail; registry/evidence lifecycle; generation/HEAD reconciliation; deterministic supersession; terminal-publication fencing; gate/migration/shadow modes; execute boundary ordering; canonical TaskAction/status consumption; and sanitized task-1849 replay with durable-attempt metadata and live-observation sentinels. |
...
| `brainstorm_suggestions/*_test.rb`, `commands/brainstorm_suggestion_test.rb` | Advisory domain/context/transport contract — selected-manifest and suggestion bindings without HEAD freshness; exact inert envelope grammar including malformed/nested/breakout regions; owner-private bounded no-follow sidecar validation; tracked-only deterministic repository/project-wiki/main-wiki capture with staged/unstaged overlays, deletions, conflicts, symlink/race/size/secret/deadline refusal; shared read safety for persisted and provider text; controller-derived provenance; fixed-endpoint controller-owned Anthropic request with no CLI spawn, tool declaration, live mount, or auth value in the body; capability/model/key refusal; bundle permissions; bounded HTTP/cancellation/runtime cleanup and startup sweep; bounded shared projection with immediate tracked repository/wiki invalidation; binding-checked retry/restore/dismiss; and old-parser-safe idempotent cleanup with unchanged parsed answers. |
| `daemon/brainstorm_suggestion_scheduler_test.rb`, `daemon/auto_retry_safety_test.rb` | Advisory reconciliation and non-authority — pre-feature/missing records, all unanswered slots, slow capture outside the task lock, answer-wins cancellation, changed selected inputs, task-wide coalescing, per-epoch attempt/backoff exhaustion without slot starvation, restart/orphan loading recovery, late completion fencing, stage/question cleanup, bounded provider calls, and suggestion envelopes remaining empty to daemon completeness/auto-retry policy. |
| `tui/{brainstorm_answers,brainstorm_suggestions}_test.rb` | External-editor projection lease — exact current-envelope insertion, byte-identical delimiter-only adoption, full-region dismissal, untouched preservation, changed-question/new-binding stripping, Restore/Retry lifecycle actions, parser/completeness inertness, and no newer candidate written into an old editor buffer. |
...
| `completion_time_test.rb`, `commands/{approve,run}_test.rb`, `integration/migrate_test.rb` | Completion-clock durability — inert versus active terminal stamping, first-completion immutability, metadata/move/index/runner-failure/interrupt rollback, earliest credible membership-workflow Git event before state-file/folder mtime fallbacks, and explicit idempotent migration of archived legacy clocks without runtime discovery. |
...
| `git_ops_test.rb` | `Hive::GitOps` — default-branch detection, orphan worktree bootstrap, idempotent gitignore, empty-diff commit skip, central hive-state commit locking, and concurrent direct commits against one shared index. |
...
| `daemon/status_consumer_test.rb` | `Hive::Daemon::StatusConsumer` — in-process internal task-graph mapping with a no-subprocess default-path regression, exact envelope validation, bounded exact-task reads, thread-scoped nested warning propagation from malformed task metadata, workflow descriptors, deprecated config, and managed workflow selections without stderr leakage, canonical task `pr_url`, strict `live_task_lock` and non-negative project hidden-count coercion, aggregate hidden-count parity, legacy project filtering, and local `state_file` mtime re-stat so daemon edit-resume decisions keep subsecond precision. |
| `daemon/stale_agent_healer_test.rb` | `Hive::Daemon::StaleAgentHealer` — stale `AGENT_WORKING` healing, wedged `REVIEW_WORKING` lock cleanup, marker-id/live-owner guards, attempt-loss successor release, delegation of cooled durable errors/CI-stale/resolved review-stale observations, and preservation of newer operator escalation edits. |
...
| `web/test/models/{task,task_mutations,project,daemon}_test.rb`, `web/test/integration/{ideas,tasks,daemon_repairs}_test.rb`, `web/test/system/task_workspace_test.rb` | Rails resource ownership — canonical stage-run verb mapping, exact digest-bound outcome-evidence rework dispatch with malformed-command refusal, fresh-row action/stage comparison, unknown-action and queue-grammar refusal before writes, canonical recovery receipt routing and disabled lifecycle UI, Reject's prior-gate derivation, custom-workflow Approve/Drop, binding-bearing brainstorm answer/intervention writes with same-number replacement-round refusal, shared repository-aware suggestion projection, escaped candidate cards, binding-checked Retry, client-only Approve/Undo/Decline/Restore with bounded binding-keyed state across Turbo morphs and hard reloads, replacement-binding draft isolation, supplied-inventory storyboard/video capture, Send-answers-only mutation, idea capture through `Project`, path-redacted typed handling of task-capture I/O failures without partial mutation, and global daemon repair. |
...
| Patrol Fix admission/materialization/report/receipt/stage tests | Unified repair workflow — strict source snapshots, leased semantic decisions, bounded pending-index record reads, nonblocking tick contention, explicit `hive migrate` index construction and old-daemon cutover, selected-entry crash repair, idempotent finding-publication retry, task binding before acknowledgement, exact worktree generations, clean managed-output recovery after internally retried Pi provider errors and rate limits while resource exhaustion remains terminal, validation in a pristine disposable exact-HEAD checkout that excludes authoritative ignored state, authoritative pre/post custody checks, non-masking checkout/root cleanup failures, independent review, route transitions, and retry-safe task materialization. |
...
| `integration/migrate_patrol_findings_script_test.rb` | Explicit one-time historical import — active ordinary findings and accepted Architecture Patrol fix/discuss dispositions become admission records without eager workflow tasks while dismissals stay inert; missing target revisions bind to current default-branch state; secret-like evidence is redacted; dry runs do not write; reruns converge; occurrence conflicts fail closed; and unrelated malformed task metadata is never scanned. |
...
| `stages/review/{ci_fix,triage,browser_test,fix_guardrail,suppression}_test.rb` | Review phase helpers — CI-fix retries, binary-labelled hosted-check output normalization before fix-prompt rendering, hosted provider-limit short-circuit before any code-fix spawn, triage prompt/bias/custom-template/protected-file behavior, triage `review_triage` default fallback values (75 / 1800), browser-test protocol handling, fix-guardrail approval gates, and no-fix suppression fingerprint/strip/seed behavior. |
...
refusal and normal skip marker. `babysitter/passthrough_policy_test.rb` pins the
side-effect-free `GitPolicy` / `GhPolicy` classification tables and the shared
runner's skip, absolute-binary, environment-hook, handoff, and failure mapping.
...
| `bench_workflow_install_test.rb` / `workflows/bench_test.rb` | Fresh `hive init --workflow bench` installs and commits the packaged runtime under `.hive-state/bench-runtime`, creates a bench-pinned task without copying a project descriptor, and resolves the built-in stage/state contract. Legacy-upgrade coverage pins one-commit archive/runtime/config rebinding, same-process descriptor cache reset, tracked instruction archives, safe retry after a rejected commit, Ctrl-C cleanup at archive and both runtime-move boundaries, preservation after a post-commit interrupt, symlink-root and raced-target refusal, descriptor/child-name/parent-directory replacement rejection at classification, quarantine, and post-staging boundaries, dirty-config isolation, staged-index preflight, and previous-runtime retention when rollback deletion fails. The descriptor test pins Codex as the shell-control agent, campaign-sized one-hour/seven-day stage timeouts, provider-only pending generation as cooldown-aware `limits_reached` rather than manual `WAITING`, GPT-5.6 stage-profile selection to the combined `hive-bench-runner:sol` image, the opt-in same-judge Codex/OpenRouter route with secret-free argv, explicit provider provenance, child-local executable-directory PATH resolution for pinned npm runtimes, a private Codex home for OpenRouter, and an empty ephemeral/config-free/rules-free workspace for both OpenRouter and ChatGPT while ChatGPT retains its subscription auth home. It also executes the packaged Ox Alpha `max` route contract with plan review disabled, proves that sealed root-controller commands restore the target to the host UID/GID on exit, proves that redacted OpenCode logs recover token buckets from Hive's cell-local usage database, verifies that candidate Git history contains only the depth-one historical base and not a descendant gold object, and pins the provider-only network/proxy arguments plus their generation-identity receipt. The judge-stage invariant rejects a null round-two final while the incomplete cell remains eligible for retry. |
| `workflows/bench_test.rb` (judge adapter) | Pins stderr quota classification, Claude CLI's exact standalone stdout reset and usage-credits banners and structured 429 failed-result envelope, plus rejection of model-authored quota prose. |
...
| `brainstorm_answering_skill_contract_test.rb` | Guided/YOLO answer orchestration over the real literal command — read-only status inventory and preview, deterministic task traversal despite reordered refreshes, degraded project isolation and stale aggregate counts, Guided approval/replacement/pause/resume/idempotency/stale behavior, YOLO zero-write and mixed-write scans with one-at-a-time escalation, same-number cross-round targeting, relocation/ambiguity/move/generation/conflict gates, final-slot no-dispatch plus normal daemon completion eligibility, sanitized 2026-07-25 regression data, and byte-identical policy/scenario payloads across OpenClaw, Claude, Codex, and Pi. |
| `run_brainstorm_test.rb`, `brainstorm_suggestion_authority_test.rb` | Deterministic `hive run` fixture plus cross-surface advisory authority — untouched-envelope parser/writer preservation; selected context passes through Runner/Validator/Store/JSON/TUI with byte-identical editable text; generation/projection/retry/dismissal leave parsed answers, markers, stage, attempts, and dispatches unchanged; only delimiter adoption produces an operator answer and triggers normal cleanup. Real model interpretation belongs to the opt-in smoke below, not this fixture. |
...
| `run_review_test.rb`, `adhoc_review_test.rb` | `hive run` of `6-review/` and ad-hoc review — pre-flight states, reviewer/triage/fix/browser branching, no-fix suppression convergence and negative cases, manual wait and stale/error recovery, pass-head receipt recovery across CI-fix restarts (legacy fresh-pass, widened-pass resume, same-head browser resume), auto-commit/guardrail boundaries, provider-limit marker behavior for reviewers, CI-fix and triage/fix limit vs non-limit failures, classified residual triage/fix failure reasons, bounded transient triage retry recovery including wall-clock handoff to `REVIEW_STALE`, Claude/tmux fix Stop-hook fallback success for commit and whole-pass no-change evidence plus rejection for missing evidence/unresolved escalations, ad-hoc PR create/reuse/error paths, ad-hoc fix-disabled/fix-opt-in branching, and `message=` surfacing on terminal phase-agent (triage and fix) errors. |
...
| `status_test.rb` | `hive status` — bounded liveness default, explicit operational and hidden internal-graph modes, empty registry, multi-stage internal rendering, stale-lock decoration, and stage-move race behavior through the command surface. |
...
| `content_workflow_daemon_e2e_test.rb`, `content_workflow_stage_test.rb`, `content_workflow_e2e_test.rb`, `generic_workflow_daemon_e2e_test.rb` | Content and generic workflow proof layers — daemon E2Es use the production in-process `StatusConsumer` rather than a CLI/JSON mapper shim; the test-only fixture e2e advances `1-inbox -> 4-done`; the built-in `:content` tests pin per-stage slash-skill prompt rendering plus marker→runner-result mapping and drive real init/new/status/daemon/policy/approve/run to `6-done/article.md` with non-empty carried artifacts. |
...
| `refactor_patrol_command_test.rb`, `refactor_patrol/job_query_test.rb`, `refactor_patrol/job_query_index_test.rb`, `refactor_patrol/job_store_test.rb` | `hive refactor-patrol` — current v4 route envelopes, PR-scoped/manual manifest consumption, daemon-attached discovery-token recovery, exact analysis-SHA worktrees shared by mapper/reviewer, checkout isolation, partial resume and terminal replay, exhaustive zero/error envelopes, action projection, exact-registration ownership, bounded read-only v2 job queries, immutable sequence/high-water cursors, index crash/rebuild fencing, and non-mutating dry-run proof. |
...
the daemon's internal task projection preserving subsecond task mtimes; otherwise a newer answer
...
## Repository-aware suggestion outcome proof

The default suite keeps provider credentials isolated and proves the complete
capture -> validation -> sidecar -> JSON/TUI projection path through injected
controller transports. First-pass model interpretation is a separate opt-in
smoke:
...
Use `claude` instead of `codex` when that account is available. The test runs
the actual configured `2-brainstorm` producer against an unanswered round with
an untouched advisory envelope and requires WAITING, a nil parsed answer, exact
envelope preservation, no Requirements/COMPLETE, and no stage move. A skip or
provider-limit error is not proof. `run_brainstorm_test.rb` remains the
deterministic fixture regression and must not be reported as real-provider
evidence.

The corresponding real-browser presentation proof uses the checked-in
inventory/storyboard and can retain an ignored local WebM:

```bash
cd web
npm ci
HIVE_RECORD_BRAINSTORM_SUGGESTION_FLOW=1 \
  bundle exec ruby bin/rails test:system test/system/task_workspace_test.rb \
  -i /suggestion_approval/
```

The recording is written to
`web/tmp/evidence/brainstorm-suggestion-flow.webm`. The 41-assertion scenario
shows escaped candidate/source rendering, exact-body Approve, persisted-answer
inertness, hard-reload/Undo draft restoration, a replacement-binding race,
Decline/Restore, and the final Send-answers write. Keep Ruby and Node
Playwright versions aligned through `npm ci`; a host-global older driver can
hang while finalizing the video.
...
Context provenance, activity reconciliation, direct task-journal folds,
