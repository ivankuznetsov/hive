...
| Patrol Fix admission/materialization/report/receipt/stage tests | Unified repair workflow — strict source snapshots, leased semantic decisions, bounded pending-index record reads, nonblocking tick contention, explicit `hive migrate` index construction and old-daemon cutover, selected-entry crash repair, idempotent finding-publication retry, task binding before acknowledgement, exact worktree generations, clean managed-output recovery after internally retried Pi provider errors and rate limits while resource exhaustion remains terminal, validation, independent review, route transitions, and retry-safe task materialization. |
