...
class PatrolModuleIntegrationTest < Minitest::Test
  include HiveTestHelper

  NOW = Time.utc(2026, 7, 22, 16, 0, 0)
  ...
  def test_task_completed_shadow_hook_uses_durable_module_attempt_without_touching_patrol_state
    with_tmp_global_config do
      with_tmp_dir do |project_root|
        ...
        package = File.expand_path("../../modules/patrol", __dir__)
        ...
        preview = Hive::ModulePackage::Preview.build(
          ...
          settings: {
            ...
            "task_completed_workflows" => "coding", "dry_run" => false
          },
          hooks: { "setup" => false, "scheduled-scan" => false, "task-completed" => true },
        ...
        event = ledger.record(
          ...
          occurred_at: NOW, source: { "type" => "task", "id" => "42" },
          idempotency_key: "task-42", payload: { "workflow" => "coding", "task_id" => 42 },
          recorded_at: NOW
        ).event
        ...
        refute File.exist?(File.join(state, "patrol")), "shadow proof cannot initialize legacy mutator state"
      end
    end
  end
  ...
  def resolution(validation)
    ...
    Hive::ModulePackage::CatalogClient::Resolution.new(
      ...
      summary: descriptor.description, package_path: "modules/patrol/0.1.0", descriptor: descriptor
    )
  end
end
