...
5. `resolve_destination`: `--to` (long or short stage name), or auto = current stage_index + 1. Past `8-done` raises `FinalStageReached` (also exit 4).
...
- `FinalStageReached` → `stage: "8-done"`
...
|--------|-----------|-------------------------|
| `:complete`         | inert stages (1-inbox, 2-brainstorm, 3-plan, 7-finalize, 8-done) | the next stage in the pipeline (e.g. `2-brainstorm` → `3-plan`) |
| `:execute_complete` | `Stages::Execute` after the implementation phase finalizes | `4-execute` → `6-review` |
| `:review_complete`  | `Stages::Review` after the autonomous loop finalizes | `6-review` → `7-finalize` |
...
| Advancing past `8-done` | 4 (`WRONG_STAGE`) | `Hive::FinalStageReached` |
