...
| `Hive::Daemon::Quiescence` | `lib/hive/daemon/quiescence.rb` | Daemon-independent installation controller. It performs the read-only ownership precheck, closes durable admission, settles launch reservations under the exclusive launch fence, drains and identity-stops registered work within the fixed 60/25/15 deadline phases, reconciles verified interruption, checks a full SQLite checkpoint, closes the connection, and only then publishes the generation-bound finalization proof. A timeout or unverifiable inventory remains quiescing and admission-closed after closure. |
| `Hive::Daemon::FinalizationProof` | `lib/hive/daemon/quiescence.rb` | Owner-private atomic proof binding installation identity, pause generation, lifecycle revision, mutation sequence, stopped inventory, interrupted attempts, and checked checkpoint metadata. Missing, stale, mismatched, or unsafe proof cannot validate the database's paused candidate. |
| `Hive::Daemon::Resume` | `lib/hive/daemon/quiescence.rb` | Serialized inverse of quiescence. It invalidates the proof before mutation, persists resuming while admission remains closed, reconciles retained process/reservation/attempt evidence under exclusive writer ownership, and reopens only by generation CAS. Managed-service restoration happens afterward and is deadline-bounded, idempotent, and reported separately from admission reopening. |
| `Hive::RuntimeControlPlane::QuiescenceUpgrade` | `lib/hive/runtime_control_plane/quiescence_upgrade.rb` | Supervised Ruby-only conversion for the pinned pre-quiescence layout and an explicit fingerprint allowlist of tested quiescence-era revisions. It revalidates the source under operation and exclusive writer ownership, removes any paused proof before schema writes, builds exact current initializer DDL with foreign keys checked and re-enabled, and preserves a quiescence-era installation's identity, attempts, payloads, closed generation, deadline, and interrupted references while leaving it `quiescing`. Ordinary setup/startup still reject skew and no CLI migration action exists. |
