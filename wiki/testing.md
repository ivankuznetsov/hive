...
| `test/unit/web/{web_command,app_bundle,environment}_test.rb`, `test/integration/web_packaged_bootstrap_test.rb` | Hive web app-dir resolution (canonical input plus warned alias), exact six-variable precedence/warnings, authenticated managed bundle extraction and tamper refusal, package-root/GEM path dependency resolution without a parent checkout, `db:prepare` typed failure guidance, final Rails `Kernel.exec` env/argv, and loopback/public-bind policy. |
