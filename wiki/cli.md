...
| `hive brainstorm-suggestion ACTION [TARGET] [--project NAME] [--question N] [--binding SHA] [--json]` | Operate the advisory suggestion lifecycle. `cleanup` is the mandatory pre-disable/downgrade fence and strips envelopes plus sidecars before reporting `safe_to_disable`; `retry`, `restore`, and `dismiss` require the exact current candidate binding and never write an answer. | `Hive::Commands::BrainstormSuggestion` | [[stages/brainstorm]] |
