...
| `workflow_package/*_test.rb`, `commands/workflow_{install,list,remove,update,publish}_test.rb`, `workflow_lifecycle_schema_test.rb` | Legacy Honeycomb author-package canonicalization/security plus current canonical YAML/release fingerprint validation, v2 flat-catalog lifecycle and catalog-snapshot materialization, exact/wildcard domain and shell/path-hook enforcement, direct-governance negation hardening for restricted-behavior scanning, catalog/manifest/source-provenance binding, fail-closed coarse-permission admission with one lossless task-local read-only mapping, immutable store transaction/task-pin concurrency, isolated warnings for missing/malformed/tampered selected configurations, incomplete-pin cleanup refusal, pre-publication mode hardening and reuse repair with executable preservation, protected policy material, headless/tmux environment parity, lifecycle preview/consent/ownership, closed JSON envelopes, deterministic legacy packaging, and injected fork/PR seams. |
...
| `honeycomb_workflow_lifecycle_test.rb` | A local immutable v2 git registry fixture drives canonical `catalog.json` plus `packages/NAME/VERSION/manifest.yml` through install, no-write dry-run, update, catalog-commit task pinning, list selected/retained dimensions, remove, and offline execution of the retained task. |
...
| `refactor_patrol_command_test.rb`, `refactor_patrol/job_query_test.rb`, `refactor_patrol/job_query_index_test.rb`, `refactor_patrol/job_store_test.rb` | `hive refactor-patrol` — v1 compatibility plus PR-scoped/manual v2 manifest consumption, freshly fetched analysis-SHA pinning, detached exact analysis worktrees shared by mapper/reviewer/leverage, dirty or branch-switched registered-checkout isolation, contaminated-analysis rejection and cleanup, terminal replay ledgers, partial feature resume on the original SHA after default-branch advancement, exhaustive zero/error envelopes, action projection, recursive suppression, exact-registration/duplicate-owner blocking before manual metadata resolution, bounded read-only job inspection even while discovery is disabled, immutable sequence/high-water cursor membership across same-time inserts and clock rollback, selected-job-only parsing, honest namespaced and legacy-flat publication truncation metadata, claim-generation-ordered current blockers, pre-index migration, both membership/job crash windows, rebuild/write fencing, failed-rebuild generation retention, explicit index rebuild/stale-cursor rejection, selector-less jobs-v1 usage errors, and dry-run proof that current consent/ownership is evaluated while durable state, refs, branches, and external collaborators remain unchanged and any temporary analysis worktree is removed. |
