...
12. **Architecture patrol discovery** (`refactor_patrol.enabled`): review each future merged PR for high-leverage architecture refactors. Fresh terminal and web setup default to yes and persist the answer explicitly. Existing projects without the block remain disabled. This answer grants discovery only; `auto_fix.enabled` and `issue_filing.enabled` remain false until separately edited by the operator.
