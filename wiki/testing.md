...
| `workflow_package/*_test.rb`, `commands/workflow_{install,list,remove,update,publish}_test.rb`, `workflow_lifecycle_schema_test.rb` | Legacy Honeycomb author-package canonicalization/security plus current canonical YAML/release fingerprint validation, v2 flat-catalog lifecycle and catalog-snapshot materialization, exact/wildcard domain and shell/path-hook enforcement, catalog/manifest/source-provenance binding, fail-closed coarse-permission admission with one lossless task-local read-only mapping, immutable store transaction/task-pin concurrency, protected policy material, headless/tmux environment parity, lifecycle preview/consent/ownership, closed JSON envelopes, deterministic legacy packaging, and injected fork/PR seams. |
...
| `test/unit/web/status_feed_test.rb`, `web/test/models/status_broadcaster_test.rb` | Hivebox status broadcasting — registered-project snapshots, one shared scan per poll tick, emit-on-connect for independent subscribers, volatile-field dedup that keeps `mtime`/`folder_mtime` significant, poller survival after snapshot errors, targeted `card_digest` patches, the ten-card whole-band fallback, generation ordering, and `StatusBroadcaster` resubscription after a raising Turbo broadcast. |
