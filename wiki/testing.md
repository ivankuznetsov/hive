...
Queued plan-only commit `ae2c5d2d` identifies that the current threshold check
rounds the percentage to two decimals, so a near-complete numerator can display
and pass as `100.00%`. Its first required implementation step is exact
covered-line equality when the configured threshold is 100%, before any CI
parallelism or fixture optimization is accepted. That correction is not
implemented by the plan commit.
...
| File | Covers |
|------|--------|
| `config_test.rb` | `Hive::Config` — defaults, `default_workflow`, deep-merge, register/find, malformed YAML rejection, normal and patrol reviewer validation, Screenote base URL validation, the migration error for obsolete `screenote.api_token`, and queued strict project-root admission including deterministic aggregation, dynamic descriptor-stage keys, the `reviewers` migration hint, and defensive rendering when a hostile key cannot be inspected. |
| `stage_label_test.rb`, `bot/title_formatter_test.rb`, `workflow_selection_test.rb`, `content_workflow_fixture_test.rb`, `workflows/content_test.rb`, `workflows/descriptor_parser_test.rb`, `workflows/loader_test.rb`, `workflows/project_test.rb`, `commands/workflow_new_test.rb` | Shared acronym-aware stage labels, workflow selection, descriptor parsing/loading, project overlay registration (including reuse of an already resolved config), blank/research scaffold authoring, retired Architecture/Writing guidance to Honeycomb, content descriptor shape, and test-only fixture support — CLI selector validation, valid-name listing, scoped fixture registration, runtime project descriptor discovery, cache invalidation, deterministic content-agent artifact writes, registry leak guards, descriptor-level agent/model/effort parsing, `kind: council` validation, active terminal-stage parsing, and the built-in `:content` stage order/files/skills/budgets/timeouts. |
...
| `attempts/*_test.rb`, `daemon/attempt_loss_healer_test.rb` | Durable task attempts — record/schema/CAS edges, competing claim/expiry, launching-handoff acceptance, artifact-plus-dependency-verdict generation duplicates/successors including empty-list output inheritance, capacity, detached session lifecycle, terminal/lost final-frame draining, PID-reuse-safe adoption and orphan cleanup, marker/git evidence precedence, legacy backfill, mutation-free dirty capture, and restart-persistent bounded loss healing. |
...
| `task_journal_test.rb`, `conditions/*_test.rb`, `task_projection*_test.rb`, `task_action_conditions_test.rb` | Generation-scoped conditions — strict journal durability/attempt integrity including short-write completion and partial-append rollback, authenticated replay with future-schema/forged-attempt rejection, registry/evidence lifecycle, generation/HEAD reconciliation, deterministic supersession, telemetry-tail-stable snapshot validation/replay, gate/migration/shadow modes, execute boundary ordering, canonical TaskAction/status consumption, and sanitized task-1849 replay across missing/corrupt/stale snapshots with durable-attempt metadata and live-observation sentinels. |
...
| `gh_test.rb`, `gh_digest_test.rb` | `Hive::Gh` — PR frontmatter parsing, secret-scan fetch-failure semantics, open/merged PR lookup, `pr_state` success/error parsing, `pr_metadata` parsing/error handling/project `chdir:` scoping, `PushResult`, immutable exact-OID non-force push, owner-private draft body tempfiles, process-group timeout/termination including a TERM-resistant stdout-inheriting helper, shared strict GitHub host/repository validation across normal and architecture-patrol transports, exact created-PR head/base OID identity checks, `mergeStateStatus` request shape for open PR listing, merged-PR digest repo discovery/listing helpers, failing-job log clipping, and digest-specific explicit-host REST pagination/detail/raw-diff/file evidence with malformed/cap failure coverage. |
| `draft_pr_receipt_test.rb`, `stages/draft_pr_handoff_test.rb`, `managed_git_test.rb` | Managed draft-PR controller — strict atomic phases and receipt evidence; exact publication and one-attempt ambiguous push/create recovery; byte-exact UTF-8 report resume and reparsing; retried no-fix cleanup and quarantine redaction; PAT/history/binary/oversize quarantine; PII-safe projection; remote identity blocks; hardened post-agent Git that suppresses fsmonitor and attribute-selected external-diff helpers; terminal artifact repair; and no corrective/force/ready/merge/close/edit/release/publish/deploy operations. |
...
| `agent_test.rb`, `agent_profile_test.rb`, `agent_message_extractor_test.rb` | `Hive::Agent` / `AgentProfile` — spawn/wait/timeout/SIGINT forwarding, version and capability checks, configured permission/workspace-write argv, Claude model/effort and verified safe-mode `cli_flags`, bounded structured/plain message accumulation with explicit structured truncation, process-group cleanup for timed-out version/help probes and stdout-inheriting descendants, safe-mode non-leakage to unrelated launches, and provider-limit classification before generic exit-code / expected-output failures. |
| `skill_check_test.rb` | `Hive::SkillCheck` — exact Claude `/hive`, Codex `$hive`, and Pi `/skill:hive` invocation parsing/discovery plus existing plugin fallback paths, shared `PI_CODING_AGENT_DIR` auth/skill resolution, Pi package/settings/git discovery, malformed invocation hints, and deterministic `npm root -g` success/timeout coverage. |
...
| `commands/bench_submit_test.rb` | `Hive::Commands::BenchSubmit` — resolves completed `9-done` tasks from registered projects, derives the source repo from GitHub `origin`, requires `worktree.yml` + `pr.md`, aborts before PR creation on local secret findings, and surfaces missing slugs/checkouts as usage errors. Coverage includes the default scanner, extractor and PR seams, repository-relative result/ref/SHA metadata, plus temporary repositories proving success and GitHub failure restore the original branch or exact detached HEAD and the reported locator resolves at the recorded SHA; no real hive-bench validator or GitHub PR is exercised. |
...
| `commands/digest_test.rb` | `Hive::Commands::Digest` — strict `YYYY-MM-DD` parsing, Europe/London default delegation, one canonical runner, repeatable registered-repository filters, human output, and schema-valid `hive-digest` v2 dry-run, real-send, empty, warning, and error envelopes. |
| `config_test.rb`, `workflows/project_test.rb` | Project-key admission — all static project keys plus default-less `gh`, built-in and active project-workflow stage overrides, exact nested reviewer entries, deterministic aggregation of string/non-string unknown keys, the `reviewers` migration hint, custom `hive_state_path` descriptor discovery without recursive config loading, and rejection of stage-name lookalikes. |
...
| `digest/london_window_test.rb`, `digest/repo_resolver_test.rb`, `digest/collector_test.rb`, `digest/stats_test.rb` | Canonical digest collection — Europe/London ordinary/DST bounds under mismatched host timezones, registered target/filter resolution, repository-atomic complete body/diff/file evidence, explicit collection outcomes, safety ceilings, secret redaction, and independently optional metrics with partial/all-missing totals. `digest/window_test.rb` remains the separate host-local AnswerDigest contract. |
| `digest/changelog_generator_test.rb`, `digest/renderer_test.rb`, `digest/run_test.rb`, `digest/sender_test.rb` | Digest generation/delivery — exact evidence-to-fact-to-bullet and project/PR identity coverage, prompt-injection fencing, generated-text redaction, deterministic project/PR links and warnings, MarkdownV2 escaping and grapheme-safe splitting, honest empty/partial/failure orchestration, dry-run token bypass, Telegram preflight, and partial-send failure. These are unit seams; no real agent, Telegram Bot API call, or live GitHub query is exercised. |
| `daemon/digest_scheduler_test.rb` | `Hive::Daemon::DigestScheduler` — first-run no-history guard, Europe/London due calculation, one-day-at-a-time catch-up, catch-up cap logging, non-zero retry/backoff behavior, disabled mode, and DST handling. |
...
| `daemon/stale_agent_healer_test.rb` | `Hive::Daemon::StaleAgentHealer` — stale `AGENT_WORKING` healing, wedged `REVIEW_WORKING` lock cleanup, bounded daemon auto-recovery for non-limit failures, and unbounded periodic readiness attempts for review/terminal `limits_reached` markers. Quota coverage pins one-hour marker-age eligibility before a distant provider reset, reset-hint-independent recovery for missing/malformed hints, repeated attempts beyond the ordinary recovery budget, and all-stage terminal clearing. Terminal-error coverage also pins marker-id guarded clears, live-lock skips, manual repository-state skips, shared budgets across fresh marker ids, per-task budget isolation, pre-clear dispatch-baseline seeding, the `3-plan` `hive plan ... --from 3-plan` dispatch-request requeue / `heal_requeued` trace, the distinct `heal_requeue_failed` event when enqueueing fails after a successful clear, and one-shot `marker_heal_exhausted` logging for bounded paths. |
...
| `gemspec_test.rb`, `install_script_test.rb` | RubyGem/install packaging — `base64 >= 0.2` is declared for PKCE; `hv` stays out of `spec.executables`; and fault-injected repeat installs pin exact `hive`/`hv`/shim byte, mode, and symlink restoration across gem failure, missing output, shim staging, wrapper write, chmod, partial activation, and final verification failures. |
| `babysitter/dry_run_env_test.rb` | `Hive::Babysitter::DryRunEnv` and its Git/GitHub stubs — default-deny classification, Git 2.45+ lazy-fetch preflight, hosts-only per-invocation `gh` auth views, prompt suppression, environment and executable-path scrubbing, working-tree diff/verbose-status/filter guards, `gh api` value-consuming option parsing, private pre-agent skip-log creation, concurrent append serialization, and deterministic direct-stub creation/replacement race rejection. |
...
| `bot/dispatch_request_writer_durable_test.rb` (`c0c6c147`, patch-equivalent queued `72b95280` and `5c86ad25`) | Durable bot delivery/admission seam — accepted admission claims the delivery with attempt/generation references, capacity deferral leaves one pending request for daemon retry, an unexpected admission error removes the unclaimed request, and task resolution scopes the slug by project plus optional stage flag. |
| `openclaw_skills_test.rb` | OpenClaw skill metadata — only the umbrella `@ivankuznetsov/hive-cli` listing is published, setup stays visible before the CLI is installed, v0.6.4 install guidance preserves package-manager confirmation, core provisioning uses `setup --no-init` before separate interactive enrollment, setup-agents and Honeycomb pin preview/approval execution sequences, patrol dry-runs stay consent-gated, TUI token totals remain a human-only handoff, current workflow/patrol/digest/bench/TUI/web paths stay present, patrol tiers name subscription/token limits and the higher architecture allowance, direct installed-runtime/service override mutation stays absent, daemon auto-advance remains bound to prior enrollment consent, marker recovery keeps guarded clear commands, and README publishing avoids shortcut listings. |
...
| `web/test/models/{task,task_mutations,project,daemon}_test.rb`, `web/test/integration/daemon_repairs_test.rb` (`153bed1d`, patch-equivalent to `96b06792` / `2fef1f47`) | Rails resource ownership — canonical stage-run verb mapping, fresh-row action/stage comparison, unknown-action and queue-grammar refusal before writes, guarded marker-clear plus stage-rerun recovery sequences with sidecar cleanup, manual-only/markerless refusal, Reject's prior-gate derivation, custom-workflow Approve/Drop, brainstorm answer/intervention writes, idea capture through `Project`, and global daemon repair through the REST-shaped repair resource. |
| Queued `web/test/config/puma_test.rb`, `web/test/integration/ideas_test.rb`, `web/test/system/pipeline_flow_test.rb` (`ccfa7c03`) | Bounded status-page resources — real-socket 413 rejection before Rack for oversized declared bodies and all chunked bodies, bounded bodies reaching Rack, parsed upload count/size refusal without task creation, eight-image/10 MiB client limits, a 16-entry synthetic batch-inspection cap, once-per-batch transport rebuild, non-decoding glyph chips, true-disconnect file release without loss during a Turbo-permanent move, and busy-frame poll pause/resume. |
...
| `refactor_patrol/job_store_test.rb`, `refactor_patrol/pr_opener_test.rb`, `refactor_patrol/action_runner_test.rb`, `refactor_patrol/repository_ownership_test.rb` | Generation-scoped publication recovery — NUL-delimited base/commit attempt ids, append-only phase/descriptor validation, additive exact-payload legacy-receipt import, pre-create trunk drift rechecks, immutable supersession, exact proven-old-OID branch replacement, and publication phases as continuation ownership. The end-to-end regression uses a real git repository/worktree, bare Git remote, `JobStore`, `ActionRunner`, and `PrOpener`: a forked worker is hard-killed after the remote push but before its completion receipt, trunk advances, a new process owner resolves the expired dead claim and receipts/supersedes the landed patch, then the fixer replaces only the proven old remote OID and creates/verifies/hands off exactly one PR. |
...
| `daemon/refactor_patrol_merge_reconciler_test.rb`, `daemon/pr_merge_watcher_test.rb`, `daemon/refactor_patrol_scheduler_test.rb`, `daemon/patrol_arbiter_test.rb`, `daemon/child_supervisor_test.rb` | Architecture-patrol daemon boundary — first-enable baseline and exact-host paginated/equal-time merge intake, one exact timestamp-range search qualifier, frozen merge-time upper bounds and result-count restart convergence, unchanged host-bound reconciler schema v2, page-cursor and intake-index resume across restart, one absolute monotonic budget shared by origin identity discovery, catch-up, bounded watcher PR-state polling, and immediate watcher hydration, hanging-`gh` termination, slow-project isolation and fair time slicing, failure-time-based persisted retry `not_before`, non-burning pre-poll/intake deadline deferral plus consecutive intake-failure accounting, crash-leftover recovery after checkpoint-before-sidecar-unlink, manifest/directory durability and idempotent predecessor replay, missing/duplicate/continuation-owner blocking, one tick-scoped candidate ownership snapshot plus fresh reservation rechecks, candidate/reservation config-failure durability, oldest-first/fair reservation, dead-owner fencing, schema-valid completion, large job-bound result-file transport, and ordinary-patrol capacity isolation. |
...
| Queued `completion_time_test.rb`, `completed_at_backfiller_test.rb`, `task_meta_test.rb`, and terminal arms in `commands/{approve,run}_test.rb` (`16f5b059`) | Branch-only immutable completion clocks — UTC/offset parsing, malformed fail-open behavior, history and mtime discovery, first-writer-wins metadata, field-preserving rewrites, atomic terminal stamping/rollback, idempotent locked legacy backfill, and persistence-failure restoration. `8a8c9234` separately pins action-based archive membership, integer/`never` retention, strict boundary timing, shared hidden/archive projection, and invalid-terminal visibility, but contains no Status/TUI caller integration test. |
...
Queued branch-only regressions in `9c4b4d69` additionally pin shared
...
and acceptance suites. Queued head `05784893` adds patrol coverage for
reviewer-selected configured validation keys, pre-persistence registry
admission, same-target terminal suppression plus active retry reuse,
newer-target recurrence/supersession, target-bound merged/dismissed lifecycle
reconciliation, stale-target refusal, clean-base validation failure/mutation
before agent launch, v3 reason enums, and unchanged v2 compatibility shapes.
Its service-installer test requires
`WantedBy=default.target` and rejects `After=default.target`.
Later test-only head `33888cc8` covers the same branch's remaining worthy
paths: in-memory `FindingRegistry` transitions, invalid stored rows and unknown
states, ledger-free semantic comparison, fixer validation-key mismatch, generic
usage exit 64, Codex owned-config drift/mode preservation, and Pi agent-home
resolution failure. It changes no production source and supplies no hosted or
installed proof.
...
| `daemon_stale_agent_healing_test.rb` | Status-to-healer integration — real `hive status --json` rows feed `Hive::Daemon::StaleAgentHealer`, pinning stale `AGENT_WORKING` classification, on-disk healing, closed logger events (`marker_healed`, `heal_requeued`, `marker_heal_failed`), `daemon.agent_marker_grace_sec` threading, the `3-plan` terminal-loss healer writing a real allowlisted dispatch request, and a one-hour-old quota marker becoming retryable while its displayed provider reset remains five days away. |
...
| `patrol_command_test.rb`, `unit/patrol/{finding_registry,fixer,reviewer,validator}_test.rb`, `unit/patrol/token_budget_test.rb`, `unit/usage_db_test.rb` | `hive patrol` — v3 JSON envelope with retained v1/v2 compatibility, dry-run behavior, language-neutral mapping, exact-remote SHA pinning, active/resolved/rejected/superseded lifecycle, indexed semantic admission, dry-run-to-shipping and transient-failure retry reuse, newer-target recurrence, target-bound ledgers, single-read/single-timestamp transitions, shared validation-key admission, clean-baseline/stale-target preflight, globally ranked selection, immutable audits, quota accounting, and schema validation. |
| `refactor_patrol_command_test.rb`, `refactor_patrol/job_query_test.rb`, `refactor_patrol/job_query_index_test.rb`, `refactor_patrol/job_store_test.rb` | `hive refactor-patrol` — v1 compatibility plus PR-scoped/manual v2 manifest consumption, freshly fetched analysis-SHA pinning, detached exact analysis worktrees shared by mapper/reviewer/leverage, dirty or branch-switched registered-checkout isolation, contaminated-analysis rejection and cleanup, terminal replay ledgers with fresh-head repinning, partial feature resume on the original SHA after default-branch advancement, invocation-unique dry-run analysis keys, exhaustive zero/error envelopes, action projection, recursive suppression, exact-registration/duplicate-owner blocking before manual metadata resolution, bounded read-only job inspection even while discovery is disabled, immutable sequence/high-water cursor membership across same-time inserts and clock rollback, selected-job-only parsing, honest namespaced and legacy-flat publication truncation metadata, claim-generation-ordered current blockers, pre-index migration, both membership/job crash windows, rebuild/write fencing, failed-rebuild generation retention, explicit index rebuild/stale-cursor rejection, selector-less jobs-v1 usage errors, and dry-run proof that current consent/ownership is evaluated while durable state, refs, branches, and external collaborators remain unchanged and any temporary analysis worktree is removed. |
...
The queued `05784893` binary regressions prove namespaced retention values win
when both forms exist and legacy generic values warn while remaining a
fallback. Queued `9c4b4d69` artifact tests prove nested
...
Queued `22d80d1b` and patch-equivalent `163ed51e` change the Playwright layer
from cumulative project state to per-example isolation:
`ApplicationSystemTestCase` stops and clears the
...
broadcaster-replaced rail. Queued siblings `affc392f`, `22d80d1b`, and
patch-equivalent `163ed51e` remove that client-side rail replacement: Rails
renders ordinary GET links and the selected project subset, while system tests
exercise navigation and composer synchronization rather than retaining a
filter node across morphs.
...
Queued commits `e1c41ea0` and patch-equivalent `a46cd592` refresh only that
ignore entry's fingerprint and note after the controller switched to registered
`Tasks::BaseController` lookup; they do not widen the trust boundary or change
runtime code.
...
Queued commits `d28377b2`, `eb8f6181`, and patch-equivalent `1941780d` change
the final browser wait to target that durable PR-gate result rather than the
transient `execute` badge: a fast daemon may
complete execute between two Turbo renders, while the real worktree commit
proves the stage ran. Current-default integration remains unverified.
