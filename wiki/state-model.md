...
title: State Model
type: data-model
source: lib/hive/task.rb, lib/hive/markers.rb, lib/hive/config.rb, lib/hive/attempts/*, lib/hive/task_journal.rb, lib/hive/task_projection/*, lib/hive/daemon/retry_*.rb, lib/hive/lock.rb, lib/hive/worktree.rb, lib/hive/metrics.rb, lib/hive/usage_db.rb, lib/hive/bot/*, lib/hive/patrol/review_handoff.rb, lib/hive/refactor_patrol/*, lib/hive/daemon/refactor_patrol_merge_*.rb, lib/hive/daemon/display_name_backfiller.rb, lib/hive/daemon/dispatch_request_queue.rb, lib/hive/web/status_feed.rb, web/app/models/status_broadcaster.rb
created: 2026-04-25
updated: 2026-07-17
...
| `<!-- ERROR reason=limits_reached provider=<agent>? message="limits reached for <agent>: ..." retry_after=<iso8601>? marker_id=<hex16> -->` | provider account/rate/quota evidence surfaced by an agent or launcher. Historical emitters may stamp `retry_after`, but it is diagnostic compatibility data only: `FailureReporter` sanitizes the evidence and `RetryCoordinator` computes the one durable, class-agnostic deadline from `daemon.retry_backoff_sec`. | `Hive::Agent#handle_exit`, `Hive::ClaudeLauncher`, stage terminalization |
| `<!-- ERROR reason=ensure_clean_on_exit_failed residue_paths=<rel,paths> marker_id=<hex16> -->` | the clean-exit invariant could not commit or validate residue. Bounded relative paths and diagnostic detail are preserved; the operator fixes the worktree/configuration, then uses audited `hive retry repair ... --generation ... --actor ... --reason ...` rather than a marker-clear-and-rerun shortcut. | `Hive::Stages::Base#enforce_clean_exit!` via `with_stage_events` exit hook + `Stages::Finalize` entry backstop |
...
| `<!-- REVIEW_ERROR phase=... reason=... message="..." -->` | agent-level review error, provider/auth failure, or protected-file/integrity failure. The terminal-error registry preserves bounded family-specific diagnostics, including `unknown`, while every family follows the same retry ladder. | `Stages::Review` orchestrator |

`5-open-pr`, `7-artifacts`, and `8-finalize` reuse generic `COMPLETE` / `ERROR` markers with stage-specific attrs such as `pr_url=...`, `is_draft=true|false`, `idempotent=true`, and `reason=...`. Those attrs remain diagnostic inputs only. Every terminal family reports through the same coordinator, and no stage-specific marker-clear, provider probe, retry budget, quarantine, or same-stage requeue is authoritative. The exact optional status-v4 `retry` projection determines cooldown/readiness; normal durable dispatch and ordinary capacity own successor execution. `PrMergeWatcher` remains a separate merged-PR retirement workflow, not a task retry authority.
...
Low-level marker repair remains available through `hive markers clear FOLDER --name <NAME>` with `--match-attr` guards, but it neither resets the durable ladder nor authorizes delayed redispatch. Normal operator recovery uses `hive retry manual` to wake an eligible record, or the audited generation-guarded `repair`, `abandon`, and `rearm` actions. Bot, web, and TUI recovery surfaces call that coordinator command instead of sequencing marker clear plus stage run.

### Durable retry projection

The task journal's replaceable `retry` projection is keyed by canonical project,
task slug, stage, and task generation. Its value validates as
`hive-retry-record` v1 and contains the predecessor/current attempt ids, retry
count, canonical diagnostic, bounded evidence/guidance, first/last failure time,
absolute `retry_after`, state, authorization, operator audit metadata, and last
terminal event id. Attempt ids and failure class do not partition the counter.

```text
terminal failure -> cooldown -> ready -> running -> succeeded
                                      \-> abandoned (operator only)
abandoned -> ready (audited re-arm)
```

Duplicate terminal delivery replays the same idempotency key and cannot
increment twice. Restart reconstruction replays the original absolute deadline,
so time already spent cooling is not restarted. Same-stage success retains the
ladder baseline; successful stage transition, fenced generation replacement, or
audited repair/re-arm are the only reset boundaries. A stale predecessor or
stale-generation operator action is rejected without replacing current state.
...
the Telegram bot, hivebox web, and the retry coordinator's authorized due-work
path. Web paths currently write through `Hive::Bot::DispatchRequestWriter`, so
the JSON `requestor` field is commonly `bot`; recovery uses one generation-
scoped `hive retry manual` intent. Healers do not enqueue task reruns.
