...
| `BrainstormParser` | `lib/hive/bot/brainstorm_parser.rb` | **Back-compat alias for `Hive::BrainstormParser`** (`lib/hive/brainstorm_parser.rb`) — the pure parser moved to the top-level namespace so the daemon can share it for the answers-pending gate (see [[modules/daemon]] §"Brainstorm answers-pending gate"). Parses `## Round`, `### Q<N>.`, and `### A<N>.` blocks. **Lenient A-header**: accepts the first A-section under each Q as that Q's answer slot regardless of A-number, so a brainstorm-agent off-by-one (e.g. `### A2.` after `### Q1.`) doesn't strand the operator. Stage markers delimit blocks but are excluded from question text/fingerprints. `parse` is total — invalid UTF-8 and a torn concurrent read degrade rather than raise — and reverses the writer's safe structural-line encoding for literal answers. Also exposes canonical heading helpers (`question_header(n)`, `answer_header(n)`) so writer / supervisor / future renderers share one format source of truth. |
| `BrainstormAnswerWriter` | `lib/hive/bot/brainstorm_answer_writer.rb` | Locked, first-write-wins insertion into the next answer block, with atomic rewrite. Slot location is **Q-context-aware**: walks forward from the parser-identified target Q's line, returning the first empty A-section before the next block boundary. When Q{n} is present and unanswered but has **no `### A` slot at all**, the writer **creates** the slot at the end of the Q-block (before the next Q/Round/marker boundary, via the parser's canonical `answer_header`) and writes the answer there. The writer scrubs invalid UTF-8, reversibly neutralizes answer lines that resemble rounds/Q/A headers/markers, and takes the existing marker sidecar lock around read-plus-atomic-replace so marker and answer writers cannot clobber each other. `:answer_slot_missing` remains only as a defensive fallback when the Q line cannot be located; exact-slot callers preserve its distinct closed outcome. |
