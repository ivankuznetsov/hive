...
36. ~~**Root README/FAQ still mentions "why no built-in web UI".**~~ — closed
2026-07-21. The README documentation index and `docs/faq.md` now describe
`hive web` as the native local Rails UI over the same project registry and
workflow state as the TUI/CLI, with loopback, login, service, proxy, and Hivebox
details delegated to [[commands/web]].
...
Latest refresh (2026-07-20): the source version and installer references are
currently v0.6.4. This feature branch does not itself prove a public tag,
signed assets, Homebrew/AUR updates, ClawHub publication, or
multi-architecture hivebox images. The release boundary now additionally
requires the exact-SHA four-agent Check Run described below before tag
automation can consume the candidate. The live post-release dogfood evidence
...
3. **The exact-SHA four-agent gate exists, but no protected run has yet supplied release evidence.** Offline tests now prove the canonical Hive operating policy, deterministic OpenClaw/Claude/Codex/Pi projections, whole-directory safe publication, platform-specific native invocation evidence (`/hive`, `$hive`, `/skill:hive`, including Claude's typed `system/init` skill and slash-command inventories), agent-first status/watch/action contracts, deterministic candidate/attestation artifacts, secret scanning, and the executable release selector's Check Run/workflow/job/artifact/archive-digest verification. `.github/workflows/live-agent-skills.yml` builds the candidate once and requires all four authenticated native jobs plus attestation before creating a candidate-bound `live-agent-skills` Check Run; `release.yml` refuses to rebuild or publish without that exact proof. This source change has not itself dispatched the protected workflow, so no checked-in/hosted artifact yet proves all four agents against one candidate. Local skips remain diagnostic and cannot close this gap.
4. **Published OpenClaw v0.1.3 is verified, but the new canonical projection is not yet published or live-proven.** The current public `@ivankuznetsov/hive-cli` v0.1.3 payload completed ClawHub static, Skillspector, ClawScan, moderation, exact-version inspection, and clean-install byte comparison; its immutable version ID is `k9755za2qn954070sytzmw4e958awza0`, fingerprint `c172cc4a4b71ec712512c3ae64ba0c92f36d1432e29096df7b08ddb37b8357f0`, and published `SKILL.md` SHA-256 `f7a0981a6058a7b5e7fa0a385589edb558905cca295fa041c673d46a40395202`. This branch replaces the checked-in source with the generated projection from `skills/hive/`; that new payload has not been published, scanned, or invoked through a real installed OpenClaw. `hive doctor` treats the older ClawHub payload as stale when canonical provenance is absent. A future separately authorized release must choose a new immutable skill version and complete the exact-SHA four-agent gate. Shortcut listings remain intentionally absent.
