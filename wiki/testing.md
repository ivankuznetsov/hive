...
| `brainstorm_suggestions/*_test.rb`, `commands/brainstorm_suggestion_test.rb` | Advisory domain/context/worker contract — selected-manifest and suggestion bindings without HEAD freshness; exact inert envelope grammar including malformed/nested/breakout regions; owner-private bounded no-follow sidecar validation; tracked-only deterministic repository/project-wiki/main-wiki capture with staged/unstaged overlays, deletions, conflicts, symlink/race/size/secret/deadline refusal; shared read safety for persisted and provider text; controller-derived provenance; tool-less/settings-less/MCP-less Bubblewrap launch shape; profile refusal; bundle permissions; success/spawn/timeout/TERM/KILL cleanup and startup sweep; bounded shared projection/cache invalidation; binding-checked retry/restore/dismiss; and old-parser-safe idempotent cleanup with unchanged parsed answers. |
