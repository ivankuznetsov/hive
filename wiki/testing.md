...
| `workflow_package/*_test.rb`, `commands/workflow_{install,list,remove,update,publish}_test.rb`, `web/workflow_lifecycle_test.rb`, `workflow_lifecycle_schema_test.rb` | Legacy Honeycomb author-package canonicalization/security plus current canonical YAML/release fingerprint validation, v2 flat-catalog lifecycle and catalog-snapshot materialization, exact/wildcard domain and shell/path-hook enforcement, catalog/manifest/source-provenance binding, fail-closed coarse-permission admission with one lossless task-local read-only mapping, immutable store transaction/task-pin concurrency, protected policy material, headless/tmux environment parity, Hivebox-to-CLI lifecycle delegation with exact preview identity rebinding, lifecycle preview/consent/ownership, closed JSON envelopes, deterministic legacy packaging, and injected fork/PR seams. |
