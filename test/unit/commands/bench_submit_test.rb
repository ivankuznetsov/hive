...
class BenchSubmitCommandTest < Minitest::Test
  ...
  def test_local_secret_scan_uses_betterleaks_without_a_bench_scanner
    File.write(File.join(@done, "leak.md"), "token = ghp_#{"aB3dE6gH9jK2mN5pQ8sT1vW4yZ7bC0eF3hI6"}\n")
    found = cmd.send(:local_secret_scan, [ File.join(@done, "leak.md"), File.join(@done, "plan.md") ])
    assert(found.any? { |f| f.include?("github-pat") && f.include?("leak.md") })
    assert_empty cmd.send(:local_secret_scan, [ File.join(@done, "plan.md") ])
    assert_empty cmd.send(:local_secret_scan, [])
  end

  def test_local_secret_scan_fails_closed
    with_replaced_singleton_method(Hive::SecretScanner, :scan, ->(*) { raise Hive::SecretScanner::Unavailable }) do
      assert_raises(Hive::Commands::BenchSubmit::UsageError) do
        cmd.send(:local_secret_scan, [ File.join(@done, "plan.md") ])
      end
    end
    assert_raises(Hive::Commands::BenchSubmit::UsageError) do
      cmd.send(:local_secret_scan, [ File.join(@done, "missing.md") ])
    end
  end
