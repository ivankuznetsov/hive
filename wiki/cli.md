...
- `bin/hive` catches `Thor::Error` around the main CLI dispatch. Without `--json`, it prints Thor's usage error and exits 64. With `--json`, it emits a single error document on stdout before exiting 64; known commands are routed to their registered schema (`hive-run`, `hive-stage-action`, `hive-findings`, `hive-patrol`, `hive-daemon-enroll`, etc.) through `JSON_PARSE_ERROR_ENVELOPES`, while an unknown command falls back to an unversioned `{ok:false, error_kind:"invalid_task_path", ...}` shape. The parse-time `hive run --json` missing-target path is integration-tested against the published `hive-run` schema.
