...
- AE5. **Covers R11.** Given Ivan подключил hive к проекту `~/Dev/myapp-new/` с его локальным `CLAUDE.md`, `.claude/skills/`, `.claude/agents/`, when агент запускается на задаче myapp-new, then он подхватывает все эти артефакты автоматически без явной конфигурации в hive.
...
- QMD уже проиндексировал проекты (коллекции `myapp-new`, `writero`, `appcrawl`, `curriculum`, `topgreendeals`, etc. подтверждены); hive будет дописывать в новые коллекции или существующие `-learnings`.
