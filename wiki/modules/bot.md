...
hidden active-task notification polling, and child reaping. Pure
...
| `StatusWatcher` | `lib/hive/bot/status_watcher.rb` | Reads Hive's hidden active projection, validates the envelope, and returns only the typed fields used by notifications and details: identity, position, action, diagnostic, `pr_url`, and `auto_residue`, plus project-level legacy-stage warnings. It does not transport terminal rows or condition/evidence histories. |
...
hive bot start
  └─ Hive::Commands::Bot (daemonizes unless --foreground)
       ...
       └─ Hive::Bot::Supervisor.run_forever
            ...
            ├─ status loop: active task graph → NotificationDispatcher
