...
3. Select a deterministic batch of at most `max_features_per_cycle` features. The cursor and exact snapshot SHA persist across daemon cycles; if default advances mid-sweep, Hive finishes the stored snapshot before starting the newer one, bounding reviewer cost without permanently restarting at the first component.
...
8. Open a PR only when validation passed, the changed paths pass the review fix guardrail, and title, body, and the exact validated base-to-head diff pass secret scanning. The local head/cleanliness, remote base, leased branch push, remote head, and created PR identity are checked fail-closed.
...
10. Persist `last_run_at`, the batch cursor, exact reviewer errors, and closed per-attempt outcomes. `last_scanned_sha` advances only after every feature in the SHA-bound sweep has been reviewed; reviewer contract failures remain visible and leave the batch eligible for retry.
...
If patrol opens a PR but cannot create its synthetic `6-review` task, the PR URL appears in `review_handoff_errors` and the exact validated worktree/patch receipt is retained for a handoff-only retry. `review_errors` distinguishes malformed/failed feature reviews from a clean zero-finding result; `review_complete` becomes true only after the full snapshot sweep succeeds. Fix, publication, and skip reasons are closed enums, with dynamic diagnostics separated into nullable detail fields. Selection decisions are also written as immutable `runs/selection-*.json` audit records.
