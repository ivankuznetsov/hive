...
15. **Babysitter dry-run default-deny stubs are unit-pinned but not live-agent-smoked.** Commit `889b0b65` changed `bin/hive-babysitter-stub-git` and `bin/hive-babysitter-stub-gh` from mutating-command blocklists to read-only allowlists, commit `fc8e7739` tightened `gh api` classification so payload flags (`-f`, `-F`, `--raw-field`, `--field`, `--input`) skip as implicit writes unless the method is explicitly GET, commit `29532639` tightened the `git` stub so otherwise read-only commands that include `--output` / `--output=...` skip instead of writing files, and commit `b5f53f98` extended the git guard across the whole original argv for exec/write-capable flags (`-c`, `--config-env`, `-O`, `--open-files-in-pager`, `--output`) that can bypass a stripped-subcommand allowlist. `test/unit/babysitter/dry_run_env_test.rb` verifies global-option stripping, unknown-command skips, mutating-command skips, read-only passthrough, the implicit-POST payload guard, `git diff --name-only` and `git grep needle` passthrough, `git diff --output...` skipping, and regression skips for `diff.external`, `core.pager`, `--config-env`, and grep pager vectors with recording fake binaries. No artifact was found showing a full `hive babysit --once PROJECT --dry-run` live-agent run after these dry-run stub changes.
