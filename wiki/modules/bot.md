...
| `WaitingRows` | `lib/hive/bot/waiting_rows.rb` | Pure selector + button builder for the human-input-blocked subset, shared by the `/waiting` reply and `hive answer-digest`. `select` keeps only the needs-input `RowActions` kinds (brainstorm/plan/review/execute/finalize/generic), drops daemon-auto-approved plan pauses, and sorts by urgency; `button_for` builds the one primary keyboard button. `ROLE_EMOJI`, `NEEDS_INPUT_KINDS`, and `URGENCY_RANK` are validated against the closed `RowActions::ROLES`/`KINDS` at load so drift fails loud. `Supervisor#status_action_button`/`#status_action_emoji` delegate here, so `/status`, `/waiting`, and the digest render the same conceptual button. A malformed row is dropped per-row (logging `status_button_failed` / `poll_failure`), never aborting the keyboard. |
...
or `#id Title… — — — Stage`. `/waiting` is the narrowed pull surface that
shows only the human-input-blocked subset via `Hive::Bot::WaitingRows.select`
(urgency-sorted, daemon-auto-approved plan pauses hidden), replying
"Nothing is waiting on you." when empty; `hive answer-digest` is its
scheduled push twin. They use Telegram HTML parse mode so valid
