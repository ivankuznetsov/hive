...
| `Hive::Daemon::RecoverableErrorHealer` | `lib/hive/daemon/recoverable_error_healer.rb` | Runs after `StaleAgentHealer` and before normal dispatch. It auto-clears only the fixed v1 recoverable terminal-error allowlist (`implementer_failed` with a Codex 401 missing bearer/basic-auth signature, and `claude_launch_failed`) after the work area is safe, the dependency health signal changed or the fallback window elapsed, backoff/budget allow it, and health probes pass. Clears are equivalent to manual `hive markers clear`; `3-plan` clears also enqueue `hive plan --from 3-plan`. The global kill-switch is `daemon.auto_retry.enabled: false`. **Scoping limitation (v1, intentional):** the health gate (`HealthProbe` doctor probe + `HealthSignal` fingerprint) is built with `project_root: nil` and the daemon-global config, so it ignores a parked task's per-project `.hive-state/config.yml`, reviewer inventory, `.llm-wiki`, and agent overrides. This is acceptable for the v1 allowlist because both recoverable reasons (Codex auth, Claude launcher) are global-CLI dependency outages, the doctor gate is one of several gates, and the blast radius is bounded (a wrongly-cleared row just re-runs the same stage within the 2-retry/backoff budget, all audited). Per-project scoping (thread the parked row's `project_root` + merged config into both `HealthProbe`/`HealthSignal` and key the per-tick caches by `[tick_id, project, category]`) is deferred to the follow-up that expands the allowlist to project-specific failures. The `HealthSignal` fingerprint is row-independent (depends only on category/config/env), so the healer memoizes it once per category per tick to avoid re-running `claude --version` + in-process `hive doctor` for every parked row. Audit routing is asymmetric: only `auto_retry` and `auto_retry_skipped` are in `Hive::Events::EVENT_TYPES`, so only those two reach the task `events.jsonl` channel; the exhausted path emits its task event as `auto_retry_skipped` (not `auto_retry_exhausted`), and a non-allowlisted/unknown reason is suppressed from the task channel entirely (daemon-log audit only, to avoid a "not retried" line on every unrelated domain failure). All four event names — `auto_retry`, `auto_retry_skipped`, `auto_retry_exhausted`, `auto_retry_failed` — reach the daemon log. |
